...
function _renderCalPreview(ds){
  ...
  var _planHtml='';
  if(new Date(ds+'T12:00:00').getDay()===6){try{_hydrateYP();}catch(e){}var _lp=litFor(ds)||{};var _ph=_dayLabel(ds);var pb=[];if(_ph)pb.push(['전례시기',_ph]);if(_lp.optype)pb.push(['운영구분',_lp.optype]);if(_lp.form)pb.push(['교리',_lp.form]);if(_lp.owner)pb.push(['교안담당',_lp.owner]);if(_lp.detail)pb.push(['교리내용',_lp.detail]);if(_lp.liturgyTeam)pb.push(['전례부',_lp.liturgyTeam]);if(_lp.choir)pb.push(['성가대',_lp.choir]);if(_lp.progress)pb.push(['진행',_lp.progress]);if(pb.length){_planHtml='<div style="background:var(--primary-light);border-radius:8px;padding:8px 10px;margin-bottom:7px"><div style="font-size:10px;font-weight:800;color:var(--primary-dark);margin-bottom:3px">📋 연간 운영 계획</div>'+pb.map(function(b){return '<div style="font-size:12px;padding:1px 0;display:flex;gap:6px"><span style="color:var(--text-light);width:56px;flex-shrink:0">'+b[0]+'</span><span style="font-weight:600;color:var(--text-sub)">'+_esc(b[1])+'</span></div>';}).join('')+'</div>';}}
  rows=_planHtml+rows;
