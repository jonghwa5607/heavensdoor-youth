...
var cc=document.getElementById('choir-week-card');if(cc){var crows=[['입당',c.ipdang],['봉헌',c.bonghen],['성체1',c.seongche1],['성체2',c.seongche2],['파견',c.pagyeon]].filter(function(r){return r[1];});var cEmpty=!crows.length&&!c.teuksong;
  var h='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px"><div><div style="font-size:10.5px;letter-spacing:.5px;color:#C9785F;font-weight:700">WEEKLY CHOIR</div><div style="font-size:16px;font-weight:800;color:var(--text);margin-top:1px">이번 주 성가곡</div></div>'+monthBtn()+'</div>';
...
var lc=document.getElementById('liturgy-week-card');if(lc){var lrows=[['제1독서',l.reading1,l.r1],['제2독서',l.reading2,l.r2],['복음',l.gospel,l.rg]].filter(function(r){return r[1];});var lEmpty=!lrows.length&&!l.note&&!l.team;
  var h2='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px"><div><div style="font-size:10.5px;letter-spacing:.5px;color:#4B5DB6;font-weight:700">WEEKLY LITURGY</div><div style="font-size:16px;font-weight:800;color:var(--text);margin-top:1px">이번 주 전례</div></div>'+(canEdit?'<button onclick="openLitPage()" style="background:var(--bg);border:1px solid var(--border-light);border-radius:8px;padding:4px 11px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--text-sub)">✏️ 편집</button>':'')+'</div>';
