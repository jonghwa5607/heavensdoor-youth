  ...
  el.innerHTML=sats.map(function(ds){
    ...
    return '<div class="card" style="padding:13px 14px;margin-bottom:10px">'
      +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:9px">'
        ...
        +'<input class="form-input" style="flex:1;font-size:12px;padding:6px 9px" placeholder="연중 제○주일" value="'+_esc(h.label||_dayLabel(ds))+'" oninput="setHymn(\''+ds+'\',\'label\',this.value)">'
      +'</div>'
...
function saveHymnMonth(){
  if(_hymnBuf){
    Object.keys(_hymnBuf).forEach(function(ds){
      ...
      _propagateLabel(ds,b.label,litData,'lt',{});
    });
    _hymnBuf=null;
  }
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function renderLitGrid(){
  ...
  el.innerHTML=sats.map(function(ds){
    ...
    return '<div class="card" style="padding:13px 14px;margin-bottom:10px">'
      +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:9px">'
        +'<span style="font-size:14px;font-weight:800;flex-shrink:0">'+(+d[1])+'월 '+(+d[2])+'일</span>'
        +'<input class="form-input" style="flex:1;font-size:12px;padding:6px 9px" placeholder="연중 제○주일" value="'+_esc(h.label||_dayLabel(ds))+'" oninput="setLit(\''+ds+'\',\'label\',this.value)">'
      +'</div>'
...
function saveLitMonth(){
  if(_litBuf){
    Object.keys(_litBuf).forEach(function(ds){
      ...
      if(!h){h={id:'lt'+ds,date:ds};litData.push(h);}
      h.label=b.label;h.reading1=b.reading1;h.reading2=b.reading2;h.gospel=b.gospel;h.note=b.note;h.roles=Object.assign({},b.roles||{});
      _propagateLabel(ds,b.label,hymnData,'hy',{items:{}});
    });
    _litBuf=null;
  }
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function openYearPlan(){if(G.role!=='teacher'){showToast('교사 전용이에요');return;}yearPlanYear=(typeof calYear!=='undefined'?calYear:new Date().getFullYear());var sel=document.getElementById('yp-year');if(sel){var ys='';for(var y=new Date().getFullYear()+1;y>=2025;y--)ys+='<option value="'+y+'">'+y+'년</option>';sel.innerHTML=ys;sel.value=yearPlanYear;}renderYearPlan();openModal('yearplan-modal');}
function renderYearPlan(){var sel=document.getElementById('yp-year');if(sel)yearPlanYear=+sel.value;var el=document.getElementById('yp-body');if(!el)return;var edit=_canEditPlan();var sats=[];for(var m=0;m<12;m++)sats=sats.concat(hymnSatsOf(yearPlanYear,m));
  var head='<div style="display:flex;gap:5px;padding:8px 6px;font-size:10px;font-weight:800;color:var(--text-light);border-bottom:2px solid var(--border-light);position:sticky;top:0;background:var(--card);z-index:1"><span style="width:42px;flex-shrink:0">날짜</span><span style="flex:1.5">전례시기</span><span style="flex:1.5">교리</span><span style="flex:1">진행</span><span style="width:28px;flex-shrink:0;text-align:center">📝</span></div>';
  var rows=sats.map(function(ds){var l=litFor(ds)||{};var d=ds.split('-');
    function cell(key,val,flex,ph){return edit?'<input value="'+_esc(val)+'" onchange="setPlan(\''+ds+'\',\''+key+'\',this.value)" placeholder="'+ph+'" style="flex:'+flex+';min-width:0;font-size:11px;padding:5px 6px;border:1px solid var(--border-light);border-radius:6px;font-family:inherit;background:var(--card)">':'<span style="flex:'+flex+';font-size:11px;color:var(--text-sub);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(val||'')+'</span>';}
    var mn=(resources||[]).find(function(r){return r.cat==='minutes'&&r.mdate===ds;});
    var mnCell=mn?'<button onclick="closeModal(\'yearplan-modal\');openMinutesViewer(\''+mn.id+'\')" style="width:28px;background:none;border:none;cursor:pointer;font-size:14px">📝</button>':(edit?'<button onclick="newMinutesForDate(\''+ds+'\')" style="width:28px;background:none;border:none;cursor:pointer;font-size:15px;color:var(--text-light)">＋</button>':'<span style="width:28px;display:inline-block"></span>');
    return '<div style="display:flex;gap:5px;align-items:center;padding:6px"><span style="width:42px;flex-shrink:0;font-size:11px;font-weight:700;color:var(--text-sub)">'+(+d[1])+'/'+(+d[2])+'</span>'+cell('label',l.label||_dayLabel(ds),'1.5','전례시기')+cell('form',l.form,'1.5','교리')+cell('progress',l.progress,'1','진행')+'<span style="width:28px;flex-shrink:0;text-align:center">'+mnCell+'</span></div>';}).join('');
  el.innerHTML='<div style="font-size:11px;color:var(--text-light);margin-bottom:10px;line-height:1.5">'+(edit?'✏️ 교감·교무·관리자는 바로 입력할 수 있어요. 전례시기는 전례·성가 편집과 연동됩니다.':'👁 열람 전용 (작성은 교감·교무·관리자)')+'</div>'+head+'<div>'+rows+'</div>';}
