...
function _applyReserveIfDue(){try{var r=appConfig&&appConfig.planReserve;if(!r)return;if(_today()<_termStart(r.yr))return;var prev={yr:r.yr-1,start:_termStart(r.yr-1),end:_termEnd(r.yr-1)};_autoPromote=true;_promoteExemptFrom=r.at||'';try{startNewSchoolYear(prev);}catch(e){}_autoPromote=false;_promoteExemptFrom='';var arr=_planTerms();var t=arr.find(function(x){return x.yr===r.yr;});if(!t){t=_defTerm(r.yr);arr.push(t);}t.locked=true;delete appConfig.planReserve;try{if(window.flushCfg)window.flushCfg();}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}}catch(e){}}
...
function savePlanTerm(lock){if(!_canEditPlan())return;if(lock){var prev={yr:_planViewYr-1,start:_termStart(_planViewYr-1),end:_termEnd(_planViewYr-1)};if(!startNewSchoolYear(prev))return;}var arr=_planTerms();var t=arr.find(function(x){return x.yr===_planViewYr;});if(!t){t={yr:_planViewYr};arr.push(t);}t.locked=!!lock;try{if(window.flushCfg)window.flushCfg();}catch(x){}renderYearPlan();showToast(lock?'🔒 '+_planViewYr+'학년도를 시작했어요':'💾 저장했어요');}
