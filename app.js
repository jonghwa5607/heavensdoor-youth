...
function openYearPlan(){if(G.role!=='teacher'){showToast('교사 전용이에요');return;}yearPlanYear=(typeof calYear!=='undefined'?calYear:new Date().getFullYear());renderYearPlan();openModal('yearplan-modal');}
function planYearShift(d){yearPlanYear+=d;renderYearPlan();}
function renderYearPlan(){var yl=document.getElementById('yp-year-label');if(yl)yl.textContent=yearPlanYear+'년';var el=document.getElementById('yp-body');if(!el)return;var edit=_canEditPlan();var sats=[];for(var m=0;m<12;m++)sats=sats.concat(hymnSatsOf(yearPlanYear,m));
  var head='<div style="display:flex;gap:6px;padding:9px 8px;font-size:11px;font-weight:800;color:var(--text-sub);border-bottom:2px solid var(--border);position:sticky;top:0;background:var(--card);z-index:5;box-shadow:0 3px 6px rgba(0,0,0,.05)"><span style="width:46px;flex-shrink:0">날짜</span><span style="flex:1.5">전례시기</span><span style="flex:1.5">교리</span><span style="flex:1">진행</span><span style="width:30px;flex-shrink:0;text-align:center">📝</span></div>';
  ...
  el.innerHTML='<div style="max-width:760px;margin:0 auto"><div style="font-size:12px;color:var(--text-light);margin-bottom:10px;line-height:1.5">'+(edit?'✏️ 교감·교무·관리자는 바로 입력할 수 있어요. 전례시기는 전례·성가 편집에도 반영됩니다.':'👁 열람 전용 (작성은 교감·교무·관리자)')+'</div>'+head+'<div>'+rows+'</div></div>';}
