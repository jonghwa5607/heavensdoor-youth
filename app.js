...
var lc=document.getElementById('liturgy-week-card');if(lc){var lrows=[['제1독서',l.reading1,l.r1],['제2독서',l.reading2,l.r2],['복음',l.gospel,l.rg]].filter(function(r){return r[1];});var lEmpty=!lrows.length&&!l.note&&!l.title&&!l.team;var h2='<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:'+((l.title||!lEmpty)?'12':'0')+'px"><div style="font-size:14px;font-weight:800;color:var(--primary-dark)">'+(l.title?E(l.title):((typeof _dayLabel==='function'&&_sat&&_dayLabel(_sat))?('<span style=\'font-weight:800\'>'+E(_dayLabel(_sat))+'</span>'):'<span style=\'color:var(--text-sub);font-weight:700\'>✝️ 이번 주 독서</span>'))+'</div>'+(canEdit?'<button onclick="openLitPage()" style="background:var(--bg);border:1px solid var(--border-light);border-radius:8px;padding:4px 11px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--text-sub)">✏️ 편집</button>':'')+'</div>';if(lEmpty){h2+='<div style="font-size:12px;color:var(--text-light);padding:4px 0">아직 등록된 독서가 없어요'+(canEdit?' · 편집을 눌러 입력하세요':'')+'</div>';}else{h2+='<div style="display:flex;flex-direction:column;gap:9px">'+lrows.map(function(r){return '<div style="display:flex;gap:12px;font-size:13px;align-items:baseline"><span style="min-width:56px;color:var(--text-light);font-weight:700;font-size:12px">'+r[0]+'</span><span style="font-weight:600;flex:1">'+E(r[1])+'</span>'+(r[2]?'<span style="font-size:11px;color:var(--primary-dark);font-weight:700;flex-shrink:0;margin-left:6px">'+E(r[2])+'</span>':'')+'</div>';}).join('')+'</div>';if(l.team)h2+='<div style="margin-top:11px;padding-top:11px;border-top:1px dashed var(--border-light);font-size:12px"><span style="color:var(--text-light);font-weight:700">전례부원</span> '+E(l.team)+'</div>';if(l.note)h2+='<div style="margin-top:9px;font-size:12px;color:var(--text-sub)">📌 '+E(l.note)+'</div>';}lc.innerHTML=h2;}}catch(e){}}
