...
function onMinKeydown(e){
  ...
  if(e.key==='Backspace'&&_minAtStart()){
    ...
    if(prev){
      ...
      try{ var rg=document.createRange(),sel2=getSelection();
        var node=ptx.firstChild||ptx; var off=Math.min(plen,(node.textContent||'').length);
        if(node.nodeType===3){rg.setStart(node,off);}else{rg.selectNodeContents(ptx);rg.collapse(false);}
        rg.collapse(true); sel2.removeAllRanges(); sel2.addRange(rg);
...
function appConfirm(opt){return new Promise(function(res){var o=(typeof opt==='string')?{title:opt}:(opt||{});var g=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};g('ac-icon',o.icon||'💬');g('ac-title',o.title||'');g('ac-desc',o.desc||'');var ok=document.getElementById('ac-ok');if(ok){ok.textContent=o.okText||'확인';ok.style.background=o.danger?'var(--coral)':'';}window.__acCb=res;openModal('app-confirm-modal');});}
function _isFullAdmin(){return G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);}
function deleteMinutes(){if(!currentMinutesId)return;var r=resources.find(x=>x.id===currentMinutesId);if(!r)return;if(!(_isFullAdmin()||r.authorId===G.id||r.createdById===G.id)){showToast('작성자 또는 교감·교무·관리자만 삭제할 수 있어요');return;}appConfirm({icon:'🗑',title:'휴지통으로 옮길까요?',desc:(r.title||'회의록')+'\n휴지통에서 다시 복구할 수 있어요.',okText:'옮기기'}).then(function(ok){if(!ok)return;_doDeleteMinutes(r);});return;}
...
function purgeMinutes(id){if(!_isFullAdmin()){showToast('교감·교무·관리자만 영구 삭제할 수 있어요');return;}var r=resources.find(x=>x.id===id);if(!r)return;appConfirm({icon:'⚠️',title:'영구 삭제할까요?',desc:(r.title||'회의록')+'\n삭제하면 복구할 수 없어요.',okText:'영구 삭제',danger:true}).then(function(ok){if(!ok)return;_doPurgeMinutes(id,r);});return;}
...
function openAvatarFull(src){if(!src)return;var m=document.getElementById('avatar-full-modal'),im=document.getElementById('avatar-full-img');if(!m||!im)return;var pvOpen=document.getElementById('profile-view-modal');window._avatarFrom=(pvOpen&&pvOpen.classList.contains('open'))?'profile-view-modal':null;if(window._avatarFrom)closeModal(window._avatarFrom);im.src=src;m.style.display='flex';m.classList.add('open');try{history.pushState({avatarFull:1},'');window._avatarHist=true;}catch(e){}}
function closeAvatarFull(){var m=document.getElementById('avatar-full-modal');if(m){m.style.display='none';m.classList.remove('open');}if(window._avatarFrom){openModal(window._avatarFrom);window._avatarFrom=null;}if(window._avatarHist){window._avatarHist=false;try{history.back();}catch(e){}}}
window.addEventListener('popstate',function(){var m=document.getElementById('avatar-full-modal');if(m&&m.style.display==='flex'){window._avatarHist=false;m.style.display='none';m.classList.remove('open');if(window._avatarFrom){openModal(window._avatarFrom);window._avatarFrom=null;}}});
...
function toggleLitLock(){
  ...
  var lock=litLocked();
  if(lock){
    appConfirm({icon:'🔓',title:'잠금을 해제할까요?',desc:'해제하면 연간계획을 편집·저장할 수 있어요.',okText:'해제'}).then(function(ok){if(!ok)return;appConfig.litLocked=false;showToast('🔓 잠금 해제됨 · 편집 가능');try{if(window.flushCfg)window.flushCfg();}catch(e){}try{renderLitLockUI();}catch(e){}try{renderYearPlan();}catch(e){}});return;
