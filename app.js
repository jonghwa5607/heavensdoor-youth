...
function selStudentCat(cat,btn){selCatTab(btn);currentBoardCat=cat;show('fab-board',cat==='free');if(cat==='event')renderBoardList(cat,posts.filter(p=>p.cat==='event'));else renderBoardList(cat,posts.filter(p=>p.cat===cat&&(p.target==='all'||(p.target==='student'&&(p.grade==='all'||p.grade==='all-s'||!p.grade||p.grade===G.gradeKey)))));}
