...
function openPostDetail(id){const p=posts.find(x=>x.id===id);if(!p)return;currentPostId=id;document.getElementById('pd-title').textContent=p.title;document.getElementById('pd-meta').innerHTML=(CAT_LABEL[p.cat]||p.cat)+' · <span onclick="openProfileView(\''+(p.authorId||'')+'\')" style="cursor:pointer;color:var(--primary-dark);font-weight:700">'+_esc(p.authorName||'익명')+'</span> · '+p.date+(p.edited?' (수정됨)':'');var _pdc=document.getElementById('pd-content');if(_pdc){if(p.id&&p.id.indexOf('wn-')===0){_pdc.innerHTML='';try{_renderNoticeCards(p.content||'',_pdc);}catch(e){_pdc.textContent=p.content||'';}}else{_pdc.textContent=p.content||'';}}document.getElementById('pd-imgs').innerHTML=postImgsHTML(p);document.getElementById('pd-attach').innerHTML=postAttachHTML(p);
renderPdComments(p);
