...
function submitWeeklyNotice(){
  ...
  if(ex){ ex.title=title; ex.content=body; ex.edited=true; ex.ts=Date.now(); }
  else{
    posts.unshift({id:id,ts:Date.now(),pushed:false,title:title,content:body,cat:'notice',target:'all',grade:'all',
  ...
  try{
    ...
    notifications.unshift({pushed:false,id:'nt-'+id,text:'📢 '+title+'이(가) 올라왔어요',time:'방금',ts:Date.now(),
      readBy:[],forRole:'all',tap:{type:'post'}});
    updateNotifDot();
  }catch(e){}
...
function startNewSchoolYear(term){if(!_autoPromote&&!(G.type==='principal'||G.type==='admin'||G.isAdmin)){showToast('교감·교무·관리자만 실행할 수 있어요');return;}if(!_autoPromote&&!confirm('새 학년도를 시작할까요?\n\n보관: 올해 출석·등급 기록 → 학생별 연혁\n진급: 전 학생 한 학년 승급, 고3은 졸업 처리\n새로 시작: 출석, 등급, 쿠폰, 결석·연락 기록, 방학 지정\n유지: 회원 계정, 게시물, 다이어리, 왕관 이력\n\n실행 후 되돌릴 수 없습니다.'))return false;if(!_autoPromote&&!confirm('새 학년도를 시작합니다. 마지막 확인입니다.'))return false;const yr=new Date().getFullYear();
  ...
  const _term=(term?term.yr:(_from?_from.slice(0,4):String(yr)))+'학년도';
...
const ATTEND_LEVELS=[{n:1,l:'🌱 씨앗'},{n:5,l:'🌿 새싹',r:'간식 교환권'},{n:10,l:'🌸 꽃망울',r:'음료 교환권'},{n:20,l:'☀️ 햇살',r:'문화상품권 5천원'},{n:40,l:'👑 왕관',r:'문화상품권 1만원'}];
...
function checkLevelCoupons(u){u.earnedLevels=u.earnedLevels||[];ATTEND_LEVELS.forEach(L=>{if((u.attendTotal||0)>=L.n&&!u.earnedLevels.includes(L.n)){u.earnedLevels.push(L.n);if(L.r){generateCoupon(u,L);if(u.id===G.id)nudgePushOnReward();}}});}
...
function goScreen(s){document.querySelectorAll('.screen,.auth-screen').forEach(el=>el.classList.remove('active'));const t=document.getElementById('screen-'+s);if(t)t.classList.add('active');}
...
function switchTab(tab){if(tab==='activity'&&G.role==='student'&&G.graduated){openMyCard();return;}document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));const sc=document.getElementById('screen-'+tab);var _ord=['home','board','attend','diary','activity','teacher','admin','my','calendar'],_ni=_ord.indexOf(tab);var _dir=(window._tabIdx==null||_ni<0)?1:(_ni>=window._tabIdx?1:-1);if(_ni>=0)window._tabIdx=_ni;try{localStorage.setItem('hd-last-tab',tab);}catch(e){}if(sc){sc.style.setProperty('--sx',_dir>0?'26px':'-26px');sc.classList.remove('tab-enter');void sc.offsetWidth;sc.classList.add('active');sc.classList.add('tab-enter');setTimeout(function(){sc.classList.remove('tab-enter');},700);}document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));const nv=document.getElementById('nav-'+tab);if(nv)nv.classList.add('active');try{if(tab==='diary')renderDiaryList();if(tab==='calendar'){
    ...
    }if(tab==='activity')renderDeptInfo();if(tab==='home'){checkImportantNotices();try{updateVerseEditUI();}catch(e){}try{checkThursdayNotice();}catch(e){}renderHomeSchedule();try{renderTeacherWeek();}catch(e){}try{renderHomeMinutes();}catch(e){}applySeason(appConfig.season||'ordinary');try{renderBdayBannerAuto();}catch(e){}}if(tab==='teacher'){autoArchiveMinutes();renderResourceList();}}catch(e){console.error('switchTab render error:',e);}try{maybeCoach(tab);}catch(e){}}
...
function attendGuard(){if(_inAdaptation()){showToast('🌱 새 학년도 준비기간이에요 · 출석은 3월부터 시작해요');return null;}if(gradGuard())return null;const sat=currentSaturday();if(isVacationDate(sat)){showToast('이번 주는 방학이라 출석체크를 진행하지 않아요');return null;}if(!qrState.code||qrState.week!==sat){showToast('아직 이번 주 QR이 생성되지 않았어요');return null;}G.attendedWeeks=G.attendedWeeks||[];if(G.attendedWeeks.includes(sat)){showToast('이미 이번 주 출석 처리되었어요');return null;}return sat;}
...
function stopQRScan(){
  ...
  if(btn)btn.textContent='📷 카메라로 스캔하기';
}
function _qrLoop(){
...
function doRegister(){
  ...
  const user={name,baptism,phone,id,pwh:'',role:G.regRole,approved:false,joinedAt:toDateStr(new Date()),birthYear:by,birthMonth:bm,birthDay:bd,feastMonth:fm,feastDay:fd};
  if(G.regRole==='student'){const s=document.getElementById('reg-school').value;const g=(document.getElementById('reg-grade-num').value||'').replace('학년','');if(!s||!g){showToast('학교급과 학년을 선택해주세요');return;}user.gradeKey=s==='middle'?'m'+g:'h';user.gradeLabel=s==='middle'?'중'+g:'고'+g;user.attendTotal=0;user.streak=0;}
  else if(G.regRole==='parent'){const children=[];document.querySelectorAll('#children-list .child-row').forEach(row=>{const inputs=row.querySelectorAll('input');const cn=(inputs[0]?.value||'').trim();const cb=(inputs[1]?.value||'').trim();if(cn)children.push({name:cn,baptism:cb});});if(!children.length){showToast('자녀 이름을 입력해주세요');return;}if(children.some(c=>!c.baptism)){showToast('자녀 세례명도 입력해주세요(동명이인 구분용)');return;}user.children=children;user.isJabumo=document.getElementById('reg-jabumo').checked;}
...
function startSession(u){
  ...
  show('event-banner-section',isS||isT);show('reward-section',isS);
  ...
  if(isP&&G.isJabumo)show('nav-jabumo-tab',true);
...
async function resetMemberPw(uid){
  ...
  notifications.unshift({id:'nt-pwreset-'+u.id,text:'🔑 비밀번호가 초기화되었어요. 교사에게 받은 임시 비밀번호로 로그인한 뒤 새 비밀번호를 설정해주세요.',time:'방금',ts:Date.now(),pushed:false,readBy:[],forStudentId:u.role==='student'?u.id:undefined,forTeacherId:u.role!=='student'?u.id:undefined});
  updateNotifDot();
...
function filterBoardStudent(gk){currentBoardCat='notice';show('fab-board',false);renderBoardList('notice',posts.filter(p=>p.cat==='notice'&&(p.target==='all'||(p.target==='student'&&(p.grade==='all'||p.grade==='all-s'||!p.grade||p.grade===gk)))));}
function filterBoardParent(){currentBoardCat='notice';show('fab-board',false);renderPosts(posts.filter(p=>p.cat==='notice'&&(p.target==='all'||p.target==='parent')));}
function selParentCat(cat,btn){selCatTab(btn);currentBoardCat=cat;show('fab-board',cat==='jabumo'&&!!G.isJabumoPresident);if(cat==='jabumo')renderBoardList(cat,posts.filter(p=>p.cat==='jabumo'));else renderBoardList(cat,posts.filter(p=>p.cat===cat&&(p.target==='all'||p.target==='parent')));}
function filterBoardTeacher(){currentBoardCat='notice';renderBoardList('notice',posts.filter(p=>p.cat==='notice'));}
function selStudentCat(cat,btn){selCatTab(btn);currentBoardCat=cat;show('fab-board',cat==='free');renderBoardList(cat,posts.filter(p=>p.cat===cat&&(p.target==='all'||(p.target==='student'&&(p.grade==='all'||p.grade==='all-s'||!p.grade||p.grade===G.gradeKey)))));}
...
function renderGalleryGrid(arr){const el=document.getElementById('post-list');if(!el)return;if(!arr||!arr.length){el.innerHTML='<div class="empty"><div class="empty-emoji">🖼️</div><div class="empty-title">아직 사진이 없어요</div></div>';return;}el.innerHTML=`<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px">${arr.map(p=>{const img0=(p.images&&p.images[0])||null;const cover=(img0&&img0.src)||p.image||'';const multi=p.images&&p.images.length>1;const clickAttr=p.photoId?`onclick="openStory('${p.photoId}')"`:'';return `<div ${clickAttr} style="position:relative;aspect-ratio:1;cursor:pointer;overflow:hidden;background:var(--border-light)">${cover?`<img src="${cover}" style="width:100%;height:100%;object-fit:cover">`:'<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:28px">📷</div>'}${multi?`<span style="position:absolute;top:6px;right:6px;color:white;font-size:14px;text-shadow:0 1px 3px rgba(0,0,0,.6)">▣</span>`:''}<div style="position:absolute;left:6px;bottom:6px;display:flex;align-items:center;gap:3px;color:white;font-size:11px;font-weight:700;text-shadow:0 1px 3px rgba(0,0,0,.6)">❤️ ${(img0&&img0.likes?img0.likes.length:0)}</div></div>`;}).join('')}</div>`;}
...
function openWriteModal(){if(gradGuard())return;if(currentBoardCat==='gallery'){if(G.role!=='teacher'){showToast('갤러리는 교사만 게시할 수 있어요');return;}const gt=document.getElementById('gallery-top');if(gt)gt.value='all';show('gallery-mid-wrap',false);const gm=document.getElementById('gallery-mid');if(gm)gm.innerHTML='<option value="">선택</option>';resetAttachBuf('gallery');openModal('gallery-write-modal');}else if(currentBoardCat==='event'){if(G.role!=='teacher'){showToast('이벤트는 교사만 게시할 수 있어요');return;}resetAttachBuf('event');openModal('event-write-modal');}else{if(G.role==='student'&&currentBoardCat!=='free'){showToast('학생은 자유게시판에만 글을 작성할 수 있어요');return;}const lbl=document.getElementById('write-cat-label');if(lbl)lbl.textContent='카테고리: '+(CAT_LABEL[currentBoardCat]||currentBoardCat);const hint=document.getElementById('write-student-hint');if(hint)hint.textContent='📌 '+(CAT_LABEL[currentBoardCat]||currentBoardCat)+' 카테고리로 자동 등록됩니다';const isT=G.role==='teacher';show('write-teacher-cats',isT);show('write-student-cat',!isT);if(isT){const isFull=G.type==='principal'||G.type==='admin'||G.isAdmin;show('write-push-wrap',isFull);const top=document.getElementById('write-top');if(top)top.value='';show('write-mid-wrap',false);const mid=document.getElementById('write-mid');if(mid)mid.innerHTML='<option value="">선택</option>';show('write-top-wrap',currentBoardCat!=='jabumo');}resetAttachBuf('write');openModal('write-modal');}}
...
function submitGalleryPost(){const modal=document.getElementById('gallery-write-modal');const editId=modal.dataset.editId;const title=(document.getElementById('gallery-title').value||'').trim()||'갤러리 사진';const content=(document.getElementById('gallery-content').value||'').trim();const target=document.getElementById('gallery-top')?.value||'all';let grade='all';if(target==='student'||target==='parent'){grade=document.getElementById('gallery-mid')?.value||'';if(!grade){showToast('하위 카테고리를 선택해주세요');return;}}const images=attachBuf.gallery.imgs.map(e=>typeof e==='object'?e:{src:e,comments:[],likes:[]});if(!images.length){showToast('사진을 추가해주세요');return;}if(_imgsTooBig(images))return;if(editId){const p=posts.find(p=>p.id===editId);if(p){p.title=title;p.content=content;p.target=target;p.grade=grade;p.edited=true;p.images=images;p.image=images[0].src;const ph=photosData.find(ph=>ph.id===p.photoId);if(ph){ph.title=title;ph.content=content;ph.images=images;ph.image=images[0].src;}}delete modal.dataset.editId;document.getElementById('gallery-modal-title').textContent='🖼️ 갤러리 게시';document.getElementById('gallery-submit-btn').textContent='등록하기';showToast('갤러리가 수정되었습니다');}else{const now=new Date();const date=now.getFullYear()+'.'+(now.getMonth()+1).toString().padStart(2,'0')+'.'+now.getDate().toString().padStart(2,'0');const phId='ph'+Date.now();posts.unshift({id:'p'+Date.now(),ts:Date.now(),pushed:false,title,content,cat:'gallery',target,grade,date,authorId:G.id,authorName:G.displayName,comments:[],edited:false,image:images[0].src,images,photoId:phId});photosData.unshift({id:phId,title,content,date:now.getFullYear()+'-'+(now.getMonth()+1).toString().padStart(2,'0')+'-'+now.getDate().toString().padStart(2,'0'),image:images[0].src,images,readBy:[]});showToast('갤러리에 등록되었습니다!');}closeModal('gallery-write-modal');document.getElementById('gallery-title').value='';document.getElementById('gallery-content').value='';resetAttachBuf('gallery');document.getElementById('gallery-top').value='all';show('gallery-mid-wrap',false);document.getElementById('gallery-img-input').value='';if(G.role==='teacher')selTeacherCat('gallery',document.querySelector('#board-teacher-top .tab-btn.active')||document.querySelector('#board-teacher-top .tab-btn'));renderStoryRow();}
function submitEventPost(){const modal=document.getElementById('event-write-modal');const editId=modal.dataset.editId;const title=(document.getElementById('event-title').value||'').trim();if(!title){showToast('제목을 입력해주세요');return;}const deadline=document.getElementById('event-deadline').value;const content=(document.getElementById('event-content').value||'').trim();const img=attachBuf.event.imgs.length?bufSrc(attachBuf.event.imgs[0]):'';
  if(editId){
...
function buildEventCardHtml(e){
  ...
  if(_ec){
    return `<div class="event-slide" onclick="openEventDetail('${e.id}')" style="${dim}"><div class="event-slide-img"><img src="${_ec}"></div><div class="event-slide-info"><div class="event-slide-title">${e.title}</div>${badge}</div></div>`;
  }
  return `<div class="event-slide" onclick="openEventDetail('${e.id}')" style="${dim}">${buildEventPoster(e)}${badge?`<div class="event-slide-info" style="background:none;padding:10px">${badge}</div>`:''}</div>`;
}
...
function renderStoryRow(){pruneStoryPhotos();const el=document.getElementById('story-row');if(!el)return;if(!photosData.length){el.innerHTML='<div style="font-size:12px;color:var(--text-light);padding:10px 0">아직 사진이 없어요</div>';return;}const byDate=[...photosData].sort((a,b)=>new Date(b.date)-new Date(a.date));let show=byDate;if(byDate.length>15){const excess=byDate.length-15;const drop=new Set();const seenOld=byDate.filter(p=>(p.readBy||[]).includes(G.id)).sort((a,b)=>new Date(a.date)-new Date(b.date));for(const p of seenOld){if(drop.size>=excess)break;drop.add(p.id);}if(drop.size<excess){const restOld=byDate.filter(p=>!drop.has(p.id)).sort((a,b)=>new Date(a.date)-new Date(b.date));for(const p of restOld){if(drop.size>=excess)break;drop.add(p.id);}}show=byDate.filter(p=>!drop.has(p.id));}el.innerHTML=show.map(p=>{const isRead=(p.readBy||[]).includes(G.id);const _cv=photoCover(p);const bg=_cv?`style="background-image:url('${_cv}');background-size:cover;background-position:center"`:'';return `<div class="story-item" onclick="openStory('${p.id}')"><div class="story-ring${isRead?' seen':''}"><div class="story-inner" ${bg}>${_cv?'':'📷'}</div></div><span style="font-size:10px;color:var(--text-sub);max-width:58px;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.title}</span></div>`;}).join('');}
...
function encDoc(arr,ix){
  ...
  if(!d.i&&d.data){
    ...
    try{
      if(n===1){
        FB.save('files',fid,{id:fid,name:nm,size:sz,n:1,d:data});
      }else{
        FB.save('files',fid,{id:fid,name:nm,size:sz,n:n});   /* 목록(매니페스트) */
        for(var k=0;k<n;k++){
          var cid=fid+'_c'+k;
          FB.save('files',cid,{id:cid,d:data.slice(k*FILE_CHUNK,(k+1)*FILE_CHUNK)});
        }
      }
...
function refreshImgButtons(pid){const{img}=getCurImg(pid);const likeBtn=document.getElementById('like-btn-'+pid);if(likeBtn&&img){const liked=(img.likes||[]).includes(G.id);likeBtn.querySelector('.like-icon').textContent=liked?'❤️':'🤍';likeBtn.querySelector('.like-count').textContent=(img.likes||[]).length;}const cEl=document.getElementById('comment-count-'+pid);if(cEl){var _p=photosData.find(x=>x.id===pid);cEl.textContent=_p?photoCmtCount(_p,(window._curImgIdx&&window._curImgIdx[pid])||0):0;}}
...
function editPost(){closeModal('post-action-modal');const p=posts.find(p=>p.id===currentPostId);if(!p)return;if(p.cat==='gallery'){document.getElementById('gallery-title').value=p.title||'';document.getElementById('gallery-content').value=p.content||'';const gt=document.getElementById('gallery-top');if(gt)gt.value=p.target||'all';onGalleryTopChange();const gm=document.getElementById('gallery-mid');if(gm&&p.grade)gm.value=p.grade;loadAttachBuf('gallery',p);document.getElementById('gallery-modal-title').textContent='🖼️ 갤러리 수정';document.getElementById('gallery-submit-btn').textContent='수정하기';document.getElementById('gallery-write-modal').dataset.editId=currentPostId;openModal('gallery-write-modal');return;}if(p.cat==='event'){document.getElementById('event-title').value=p.title||'';document.getElementById('event-content').value=p.content||'';document.getElementById('event-deadline').value=p.deadline||'';loadAttachBuf('event',p);document.getElementById('event-modal-title').textContent='🎉 이벤트 수정';document.getElementById('event-submit-btn').textContent='수정하기';document.getElementById('event-write-modal').dataset.editId=currentPostId;openModal('event-write-modal');return;}if(p.cat==='activity'){openActivityWrite();document.getElementById('aw-title').value=p.title||'';document.getElementById('aw-content').value=p.content||'';document.getElementById('aw-dept').value=p.dept||'choir';document.getElementById('aw-cat').value=p.subcat||'notice';loadAttachBuf('aw',p);document.getElementById('activity-write-modal').dataset.editId=currentPostId;return;}document.getElementById('write-title').value=p.title||'';document.getElementById('write-content').value=p.content||'';const lbl=document.getElementById('write-cat-label');if(lbl)lbl.textContent='카테고리: '+(CAT_LABEL[p.cat]||p.cat);const isT=G.role==='teacher';show('write-teacher-cats',isT);show('write-student-cat',!isT);if(isT){const isFull=G.type==='principal'||G.type==='admin'||G.isAdmin;show('write-push-wrap',isFull);show('write-top-wrap',p.cat!=='jabumo');const top=document.getElementById('write-top');if(top){top.value=p.target||'all';onWriteTopChange();}const mid=document.getElementById('write-mid');if(mid&&p.grade)mid.value=p.grade;}document.getElementById('write-modal').dataset.editId=currentPostId;loadAttachBuf('write',p);openModal('write-modal');}
...
function postImgsHTML(p){const imgs=p.images?p.images.map(im=>typeof im==='object'?im.src:im):(p.image?[p.image]:[]);return imgs.length?`<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">${imgs.map((s,i)=>`<img src="${s}" onclick="openImgFull('${p.id}',${i})" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);background:var(--bg);cursor:pointer">`).join('')}</div>`:'';}
...
if(p.docs&&p.docs.length)h+=`<div class="attach-label" style="margin-bottom:6px">📎 첨부파일 ${p.docs.length}개</div>`+p.docs.map((d,ix)=>`<div class="attach-file-item" style="flex-direction:column;align-items:stretch;gap:8px"><div style="display:flex;align-items:center;gap:6px"><span>${_docIcon(d)}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600;color:var(--text)">${d.name}</span>${_docSizeLabel(d)?`<span style="color:var(--text-light);font-size:10px">${_docSizeLabel(d)}</span>`:''}</div><div style="display:flex;gap:6px">${_canPreview(d)?`<button class="btn btn-sm btn-outline" style="width:auto;flex:1" onclick="previewPostDoc('${p.id}',${ix})">👁 미리보기</button>`:''}<button class="btn btn-sm btn-primary" style="width:auto;flex:1" onclick="downloadPostDoc('${p.id}',${ix})">⬇ 다운로드</button></div></div>`).join('');
return h;}
...
function openPostDetail(id){const p=posts.find(x=>x.id===id);if(!p)return;currentPostId=id;document.getElementById('pd-title').textContent=p.title;document.getElementById('pd-meta').textContent=(CAT_LABEL[p.cat]||p.cat)+' · '+(p.authorName||'익명')+' · '+p.date+(p.edited?' (수정됨)':'');document.getElementById('pd-content').textContent=p.content||'';document.getElementById('pd-imgs').innerHTML=postImgsHTML(p);document.getElementById('pd-attach').innerHTML=postAttachHTML(p);
renderPdComments(p);
...
function openResourceDetail(id){const r=resources.find(r=>r.id===id);if(!r)return;currentResourceId=id;const canEdit=r.authorId===G.id||G.type==='principal'||G.type==='admin'||G.isAdmin;show('rd-edit-btn',canEdit);show('rd-delete-btn',canEdit);document.getElementById('rd-title').textContent=r.title;document.getElementById('rd-meta').textContent=RESOURCE_CAT_LABEL[r.cat]+' · '+r.authorName+' · '+r.date;document.getElementById('rd-content').textContent=r.content||'';var _lk=document.getElementById('rd-link');if(_lk)_lk.innerHTML=r.link?('<a href="'+_esc(r.link)+'" class="btn btn-sm btn-primary" style="width:100%;text-decoration:none;display:flex;align-items:center;justify-content:center">'+_linkIcon(r.link)+' 문서 열기 ↗</a>'):'';document.getElementById('rd-images').innerHTML=(r.images||[]).map(im=>`<img src="${_srcOf(im)}" style="width:100%;border-radius:var(--radius-sm)">`).join('');document.getElementById('rd-docs').innerHTML=(r.docs||[]).map((d,ix)=>`<div style="background:var(--bg);border-radius:var(--radius-sm);padding:12px 14px"><div style="display:flex;align-items:center;gap:8px;font-size:13px"><span>${_docIcon(d)}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600">${d.name}</span>${_docSizeLabel(d)?`<span style="color:var(--text-light);font-size:10px">${_docSizeLabel(d)}</span>`:''}</div><div style="display:flex;gap:6px;margin-top:9px">${_canPreview(d)?`<button class="btn btn-sm btn-outline" style="width:auto;flex:1" onclick="previewResDoc('${r.id}',${ix})">👁 미리보기</button>`:''}<button class="btn btn-sm btn-primary" style="width:auto;flex:1" onclick="downloadResDoc('${r.id}',${ix})">⬇ 다운로드</button></div></div>`).join('');openModal('resource-detail-modal');}
...
const RESOURCE_CAT_LABEL={form:'양식',doctrine:'교리',liturgy:'전례·성가',camp:'행사',minutes:'회의록',
...
var RES_TABS=[{k:'form',l:'양식'},{k:'doctrine',l:'교리'},{k:'liturgy',l:'전례·성가'},{k:'camp',l:'행사'}];
...
function _resCard(r){
  if(r.cat==='minutes'){
    ...
    return `<div class="resource-card" onclick="openMinutesViewer('${r.id}')"><div class="resource-info"><div class="resource-title">${r.title}</div><div class="resource-meta">${badge}${r.authorName} · ${r.updatedAt||r.date}${preview?' · '+preview+(r.content.length>50?'...':''):''}</div></div></div>`;
  }
...
function onMinKeydown(e){
  ...
  if(e.key==='Backspace'&&_minAtStart()){
    ...
    if(prev&&!c2){ e.preventDefault(); blk.remove(); _minRenumber(); _minCaretEnd(prev.querySelector('.mb-txt')); onMinutesInput(); return; }
  }
}
...
function onMinutesInput(){if(!_minEditing)return;clearTimeout(_minSaveTimer);_minSaveTimer=setTimeout(_saveMinutesNow,1500);}
...
function _writeLock(){var id=currentMinutesId;if(!id||!(window.FB&&FB.enabled()))return;try{FB.save('minutesLocks',id,{id:id,uid:G.id,name:((G.name||'')+' '+(G.baptism||'')).trim(),ts:Date.now()});}catch(e){}}
...
let attachBuf={write:{imgs:[],docs:[]},aw:{imgs:[],docs:[]},gallery:{imgs:[],docs:[]},event:{imgs:[],docs:[]},rw:{imgs:[],docs:[]}};
...
function renderHomeNotices(){const el=document.getElementById('home-notices');if(!el)return;const notices=sortPostsNewest(posts.filter(p=>p.cat==='notice')).slice(0,5);if(!notices.length){el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px">📋</div><div class="empty-title" style="font-size:13px">아직 공지사항이 없어요</div></div>';return;}el.innerHTML=notices.map(p=>{const k=noticeKind(p);return `<div class="notice-item" onclick="openPostDetail('${p.id}')"${k?` style="border-left:3px solid ${k.fg};padding-left:9px"`:''}><div class="notice-unread"></div><div class="notice-info"><div class="notice-title">${noticeChip(p)}${p.title}</div><div class="notice-meta">${p.authorName} · ${p.date}</div></div></div>`;}).join('');}
function renderParentChildCards(children){const el=document.getElementById('parent-child-cards');if(!el||!children.length)return;const students=pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated);el.innerHTML=children.map(c=>{const nm=c.name||c;const cb=c.baptism||'';const pinned=c.sid?students.find(s=>s.id===c.sid):null;const matches=students.filter(s=>s.name===nm&&(!cb||s.baptism===cb));const linked=pinned||(matches.length===1?matches[0]:null);const ambiguous=!pinned&&matches.length>1;const statusText=linked?(linked.gradeLabel||''):ambiguous?'⚠️ 확인 필요':'미가입';return `<div class="card" style="margin-bottom:8px"><div style="display:flex;align-items:center;gap:12px${linked?';margin-bottom:12px':''}"><div style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--lavender));display:flex;align-items:center;justify-content:center;color:white;font-weight:800">${nm.charAt(0)}</div><div><div style="font-size:14px;font-weight:700">${nm}${cb?' '+cb:''}</div><div style="font-size:11px;color:${ambiguous?'var(--coral)':'var(--text-light)'}">${statusText}</div></div></div>${linked?`<div style="display:grid;grid-template-columns:1fr 1fr 1fr;border-top:1px solid var(--border-light);padding-top:10px"><div style="text-align:center"><div style="font-size:18px;font-weight:800;color:var(--primary)">${linked.attendTotal||0}</div><div style="font-size:10px;color:var(--text-light)">누적</div></div><div style="text-align:center;border-left:1px solid var(--border-light);border-right:1px solid var(--border-light)"><div style="font-size:18px;font-weight:800;color:var(--mint)">${monthAttendCount(linked)}</div><div style="font-size:10px;color:var(--text-light)">이번 달</div></div><div style="text-align:center"><div style="font-size:18px;font-weight:800;color:var(--yellow)">${linked.streak||0}</div><div style="font-size:10px;color:var(--text-light)">연속</div></div></div>`:ambiguous?`<div style="font-size:11px;color:var(--coral);margin-top:8px;line-height:1.6;background:var(--coral-light);padding:9px 11px;border-radius:8px">이름과 세례명이 같은 학생이 <b>${matches.length}명</b> 있어요.<br>담당 교사에게 문의해주시면 연결해드립니다.</div>`:''}</div>`;}).join('');}
...
function renderAdminGrid(isFull){const el=document.getElementById('admin-grid');if(!el)return;const isAdminUser=G.type==='principal'||G.isAdmin;const canApprove=G.type==='principal'||G.type==='admin'||G.isAdmin;show('admin-settings-tabbtn',isAdminUser);const students=pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated&&(isFull||u.gradeKey===G.type));const absentCount=students.filter(u=>computeAbsentStreak(u)>=3).length;const items=[
    {icon:'👥',title:'회원 관리',sub:'승인·역할·권한',badge:pendingList.filter(u=>!u.approved).length+pendingList.filter(u=>u.posRequest&&_canApprovePos(u.posRequest)).length,action:"showAdminTab('members')"},
    {icon:'🎟️',title:'쿠폰 관리',sub:'인증번호·사용 현황',badge:coupons.filter(c=>!c.used).length,action:"showAdminTab('coupons')"},
    {icon:'✅',title:'출석 관리',sub:'오늘 출석 체크',action:"showAdminTab('attend')"},
    {icon:'📊',title:'출석 통계',sub:'학년별 현황·결석알림',badge:absentCount,action:"showAdminTab('stats')"},
    {icon:'📖',title:'신앙 다이어리',sub:'공유된 다이어리·답장',action:"showAdminTab('diary')"},
    {icon:'🎒',title:'학생 카드',sub:'학생 상세 정보',action:"showAdminTab('students')"},
    ...((G.type==='principal'||G.isAdmin)?[{icon:'💌',title:'졸업 편지',sub:'고3에게 마음 전하기',action:"openGradLetters()"}]:[])
  ];
  if(isAdminUser)items.push({icon:'⚙️',title:'앱 설정',sub:'제목·컬러·말씀 수정',action:"showAdminTab('settings')"});el.innerHTML=items.map(it=>`<div class="admin-card" onclick="${it.action}"><div class="admin-card-icon">${it.icon}</div><div class="admin-card-title">${it.title}</div><div class="admin-card-sub">${it.sub}</div>${it.badge?`<div class="admin-badge" style="background:var(--coral-light);color:var(--coral)">${it.badge}건</div>`:''}</div>`).join('');}
function showAdminTab(tab){['main','attend','members','students','diary','coupons','stats','settings'].forEach(t=>{const el=document.getElementById('admin-'+t+'-tab');if(el)el.style.display=t===tab?'':'none';});const bb=document.getElementById('admin-back-btn');if(bb)bb.style.display=tab==='main'?'none':'flex';const bar=document.querySelector('#screen-admin .tab-bar');if(bar)bar.querySelectorAll('.tab-btn').forEach(b=>b.classList.toggle('active',b.getAttribute('onclick').includes(`showAdminTab('${tab}')`)));if(tab==='pending')updatePendingUI();if(tab==='attend')renderAttendList();if(tab==='members')renderMembersList();if(tab==='students')renderStudentCards('all');if(tab==='diary')renderAdminDiaryList();if(tab==='main')renderGovSection();if(tab==='settings')renderSeasonBtns();if(tab==='reminder')renderReminderList();if(tab==='coupons')renderAdminCouponList();if(tab==='settings')loadAppConfigForm();if(tab==='stats'){renderAttendStats();renderAbsentAlerts();}window.scrollTo(0,0);}
...
function computeAbsentStreak(u){const sat=currentSaturday();if((u.attendedWeeks||[]).includes(sat))return 0;const weeks=getSaturdays(60).filter(d=>!isVacationDate(d));const jn=u.joinedAt||null;let streak=0;for(const w of weeks){if(jn&&w<jn)break;if(!(u.attendedWeeks||[]).includes(w))streak++;else break;}return streak;}
let absentNotifiedKeys={};
function checkAbsentNotifications(){const sat=currentSaturday();pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated).forEach(u=>{const n=computeAbsentStreak(u);if(n<3)return;if(u.absentAckWeek===sat)return;const key=u.id+'|'+sat;if(absentNotifiedKeys[key])return;absentNotifiedKeys[key]=true;const lv=absentLevel(n);const HEAD={1:'⚠️',2:'🚨 [주의]',3:'🚨 [경고]',4:'🆘 [긴급]'};notifications.unshift({pushed:false,id:'nt'+Date.now()+Math.random().toString(36).slice(2,6),text:`${HEAD[lv]} <b>${u.name} ${u.baptism}</b> (${u.gradeLabel||''}) 학생이 ${n}주 연속 결석했어요. 연락이 필요합니다.`,time:'방금',ts:Date.now(),readBy:[],forRole:'teacher-grade-'+u.gradeKey,absentUid:u.id,tap:{type:'absent'}});/* 결석 푸시는 서버가 일요일 오전 10시에 발송 */});updateNotifDot();}
/* ══════════ FCM 푸시 알림 ══════════ */
...
function calcAttendTotal(u){return (u.attendedWeeks||[]).length-0.5*((u.halfWeeks||[]).length);}
...
function setAttendStatus(uid,st){const sat=currentSaturday();if(isVacationDate(sat)){showToast('이번 주는 방학이라 출석을 진행하지 않아요');return;}const u=pendingList.find(u=>u.id===uid);if(!u)return;u.attendedWeeks=(u.attendedWeeks||[]).filter(d=>d!==sat);u.halfWeeks=(u.halfWeeks||[]).filter(d=>d!==sat);if(st==='full')u.attendedWeeks.push(sat);else if(st==='half'){u.attendedWeeks.push(sat);u.halfWeeks.push(sat);}u.attendTotal=calcAttendTotal(u);if(st!=='absent'){u.absentAckBy=null;checkLevelCoupons(u);}u.streak=computeStreak(u);if(G.id===u.id){G.attendTotal=u.attendTotal;G.streak=u.streak;G.attendedWeeks=u.attendedWeeks;G.halfWeeks=u.halfWeeks;}saveMemberNow(u);checkAbsentNotifications();window._lastAttendUid=uid;renderAttendList();window._lastAttendUid=null;renderAbsentAlerts();renderAttendStats();renderAdminGrid(G.type==='principal'||G.type==='admin'||G.isAdmin);showToast(st==='full'?'✅ 출석 처리':st==='half'?'🌗 반일출석 처리':'결석 처리');}
function generateCoupon(u,level){const code=String(Math.floor(100000+Math.random()*900000));const coupon={id:'cp'+Date.now()+Math.random().toString(36).slice(2,4),studentId:u.id,studentName:u.name+' '+u.baptism,badgeLabel:level.l,reward:level.r||'',code,used:false,createdAt:new Date().toLocaleDateString('ko-KR')};coupons.push(coupon);notifications.unshift({pushed:false,id:'nt'+Date.now()+'t',text:`🎟️ ${u.name} ${u.baptism} 학생이 [${level.l}] 등급을 달성했어요! 인증번호: ${code}`,time:'방금',ts:Date.now(),readBy:[],forTeacher:true,tap:{type:'coupon-admin'}});notifications.unshift({pushed:false,id:'nt'+Date.now()+'s',text:`🎉 축하해요! <b>[${level.l}]</b> 등급 달성으로 <b>${level.r||'보상'}</b> 쿠폰이 발급되었어요. 선생님께 확인받고 사용하세요!`,time:'방금',ts:Date.now(),readBy:[],forStudentId:u.id,tap:{type:'coupon'}});if(u.manualReg)notifications.unshift({pushed:false,id:'nt'+Date.now()+'mr',text:'📵 '+u.name+' '+u.baptism+' 학생(수동등록)에게 새 쿠폰이 발급됐어요. 학생카드에서 확인 후 직접 전달해주세요!',time:'방금',ts:Date.now(),readBy:[],forRole:'teacher-grade-'+u.gradeKey,tap:{type:'diary-shared',sid:u.id}});updateNotifDot();showToast('🎉 '+level.l+' 달성! 쿠폰 인증번호가 교사에게 전송되었습니다');renderNotifList();}
...
function notifMatch(n){if(!n)return false;if((n.hiddenBy||[]).indexOf(G.id)>=0)return false;
  var _jt=_joinTs();if(_jt&&n.ts&&n.ts<_jt)return false;if(n.forStudentId)return n.forStudentId===G.id;if(n.forTeacherId)return G.role==='teacher'&&n.forTeacherId===G.id;if(n.forRole){if(n.forRole==='all')return true;if(n.forRole==='teacher-parent')return G.role==='teacher'||G.role==='parent';if(n.forRole.indexOf('teacher-grade-')===0){const gk=n.forRole.slice(14);return G.role==='teacher'&&(G.type===gk||G.type==='principal'||G.type==='admin'||G.isAdmin);}return n.forRole===G.role;}return G.role==='teacher'&&!!n.forTeacher;}
...
function openMemberDetail(uid){
  ...
  if(u.role==='student'){
    ...
    const noResp=evs.filter(e=>!(e.responses||{})[u.id]).length;
    ...
    rows+=_mdRow('자부모회장',u.isJabumoPresident?'🎗️ 회장':'-');
  }
...
function saveMemberEdit(){const u=pendingList.find(u=>u.id===currentEditMemberId);if(!u)return;
  ...
  if(u.role==='student'){if(u.manualReg){const nn=(document.getElementById('me-mname')||{}).value,bb=(document.getElementById('me-mbaptism')||{}).value;if(nn&&nn.trim())u.name=nn.trim();u.baptism=(bb||'').trim();coupons.forEach(c=>{if(c.studentId===u.id)c.studentName=u.name+' '+u.baptism;});}const g=document.getElementById('me-grade').value;if(g.charAt(0)==='h'){u.gradeKey='h';u.gradeLabel='고'+g.charAt(1);}else{u.gradeKey=g;u.gradeLabel={m1:'중1',m2:'중2',m3:'중3'}[g]||g;}}else if(u.role==='teacher'){const pos=document.getElementById('me-position').value;if((pos==='principal'||pos==='admin')&&pos!==u.teacherType){const holder=pendingList.find(x=>x.role==='teacher'&&x.approved&&!x.hidden&&x.teacherType===pos&&x.id!==u.id);if(holder){const lbl=pos==='principal'?'교감':'교무';if(!confirm(lbl+'은 현재 '+holder.name+' '+holder.baptism+' 선생님입니다.\n\n'+u.name+' 선생님에게 '+lbl+' 직책을 이전할까요?\n(기존 '+holder.name+' 선생님은 "기타"로 자동 변경됩니다)'))return;holder.teacherType='etc';holder.gradeLabel='기타';notifications.unshift({id:'nt'+Date.now()+'pt',text:'👤 '+lbl+' 직책이 이전되었어요: '+holder.name+' → '+u.name+' 선생님',time:'방금',ts:Date.now(),pushed:false,readBy:[],forRole:'teacher'});updateNotifDot();}}
...
function renderDetailAttend(){
  ...
  el.innerHTML='<div class="card" style="padding:14px">'
    +'<div style="display:flex;gap:6px;margin-bottom:12px">'
      +'<div style="flex:1;text-align:center;background:var(--bg);border-radius:10px;padding:8px 2px"><div style="font-size:16px;font-weight:800;color:var(--text)">'+(u.attendTotal||0)+'</div><div style="font-size:10px;color:var(--text-light);margin-top:2px">출석</div></div>'
...
function saveHymnMonth(){
  if(_hymnBuf){
    Object.keys(_hymnBuf).forEach(function(ds){
      ...
      h.items=b.items;h.note=b.note;h.teuksong=b.teuksong;
    });
    _hymnBuf=null;
  }
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function sendLitNotify(ds){
  ...
  Object.keys(byStu).forEach(function(sid){
    ...
    notifications.unshift({pushed:false,id:'nt'+Date.now()+'lit'+cnt+Math.random().toString(36).slice(2,5),
      ...
      time:'방금',ts:Date.now(),readBy:[],forStudentId:sid});
    cnt++;
  });
  updateNotifDot();
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function submitCalEvent(){if(G.role!=='teacher'){showToast('교사만 일정을 등록할 수 있어요');return;}const title=(document.getElementById('cal-ev-title').value||'').trim();const date=document.getElementById('cal-ev-date').value;if(!title||!date){showToast('제목과 날짜를 입력해주세요');return;}const time=document.getElementById('cal-ev-time').value;const place=(document.getElementById('cal-ev-place').value||'').trim();const visibility=document.getElementById('cal-ev-visibility').value;const content=(document.getElementById('cal-ev-content').value||'').trim();
  ...
  const _ceid='ce'+Date.now();calEvents.push({id:_ceid,title,date,time,place,content,visibility,authorId:G.id,authorName:G.displayName,checkedBy:[],isRecurring:false,responses:{}});if(visibility!=='private'){notifications.unshift({pushed:false,id:'nt-cal-'+_ceid,noPush:true,text:'📅 새 일정: <b>'+title+'</b>'+(date?(' ('+date+(time?' '+time:'')+')'):''),time:'방금',ts:Date.now(),readBy:[],hiddenBy:[],forTeacher:true,tap:{type:'cal-vote',date:date}});updateNotifDot();}closeModal('cal-event-modal');selectedCalDate=date;renderCalendar();try{renderHomeSchedule();}catch(x){}showToast('일정이 등록되었습니다!');}
function toggleCalCheck(eid){const e=calEvents.find(e=>e.id===eid);if(!e)return;if(!e.checkedBy)e.checkedBy=[];const idx=e.checkedBy.indexOf(G.id);if(idx>=0)e.checkedBy.splice(idx,1);else e.checkedBy.push(G.id);renderCalDayEvents(selectedCalDate);}
...
function setCalResponse(eid,val){const e=calEvents.find(e=>e.id===eid);if(!e)return;if(!e.responses)e.responses={};e.responses[G.id]=val;renderCalDayEvents(selectedCalDate);showToast(val==='yes'?'참석으로 응답했어요':'불참으로 응답했어요');}
function openRespModal(eid,kind){
  const e=calEvents.find(e=>e.id===eid);if(!e)return;
  const teachers=getApprovedTeachers();const R=e.responses||{};
...
function notifyUnrespondedTeachers(){
  ...
  const evs=calEvents.filter(e=>e.isRecurring&&e.date>today&&e.date.indexOf(ym)===0&&!isVacationDate(e.date)).sort((a,b)=>a.date<b.date?-1:1);
  ...
  const targets=teachers.filter(t=>evs.some(e=>!(e.responses||{})[t.id]));
  ...
  let sent=0;
  teachers.forEach(t=>{
    const miss=evs.filter(e=>!(e.responses||{})[t.id]);
    if(!miss.length)return;
    const label=miss.map(e=>{const q=e.date.split('-');return (+q[1])+'월 '+(+q[2])+'일';}).join(', ');
    const nid='nt-vote-'+t.id+'-'+ym;
    notifications=notifications.filter(x=>x.id!==nid);
...
function _canEditPlan(){return G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);}
var _planViewYr=null;
var _autoPromote=false,_promoteExemptFrom='';
var __TEST_DATE=null;
function _today(){return __TEST_DATE||toDateStr(new Date());}
/* ===== TEST-START (배포 전 제거) ===== */
function _nextGrade(u){var m={m1:'중2',m2:'중3',m3:'고1',h:'고2/고3/졸업'};return m[u.gradeKey]||'?';}
function _dryRunSummary(){var r=appConfig&&appConfig.planReserve;var L=['[드라이런] 오늘='+_today()];if(!r){L.push('예약 없음 — 먼저 예약하세요');return L.join('\n');}L.push('예약: '+r.yr+'학년도 · 예약일 '+r.at);L.push('3월 도래: '+(_today()>=(r.yr+'-03-01')?'예(실행대상)':'아직'));var pr=[],ex=[];pendingList.filter(function(u){return u.role==='student'&&u.approved&&!u.hidden&&!u.graduated;}).forEach(function(u){if(r.at&&u.joinedAt&&u.joinedAt>=r.at)ex.push(u.name+'('+(u.gradeLabel||'')+')');else pr.push(u.name+' '+(u.gradeLabel||'')+'→'+_nextGrade(u));});var tr=pendingList.filter(function(u){return u.role==='teacher'&&u.approved&&!(u.teacherType==='principal'||u.teacherType==='admin'||u.isAdmin);}).map(function(u){return u.name;});L.push('\n■ 승급('+pr.length+')\n'+(pr.join(', ')||'없음'));L.push('\n■ 진급면제-신규('+ex.length+')\n'+(ex.join(', ')||'없음'));L.push('\n■ 교사 담당초기화('+tr.length+')\n'+(tr.join(', ')||'없음'));return L.join('\n');}
function _planTest(){var d=prompt('테스트 가상 날짜 (YYYY-MM-DD). 비우면 실제 날짜로 복귀:',__TEST_DATE||'2026-02-05');if(d===null)return;__TEST_DATE=(d.trim()||null);var act=prompt('동작 (숫자):\n1 = 화면 새로고침만\n2 = 드라이런(미리보기, 데이터 안 바뀜)\n3 = 실제 자동진급 실행(데이터 바뀜!)','1');if(act==='2'){alert(_dryRunSummary());}else if(act==='3'){if(confirm('⚠️ 실제로 학생 진급·기록보관·교사초기화가 실행됩니다. 진행?'))_applyReserveIfDue();}try{renderYearPlan();}catch(e){}showToast('🧪 가상날짜: '+(__TEST_DATE||'실제 날짜'));}
/* ===== TEST-END ===== */
function _inAdaptation(){var r=appConfig&&appConfig.planReserve;return !!(r&&_today()<(r.yr+'-03-01'));}
function reservePlanTerm(){if(!_canEditPlan())return;var yr=_planViewYr;appConfig.planReserve={yr:yr,at:toDateStr(new Date())};var arr=_planTerms();if(!arr.find(function(x){return x.yr===yr;}))arr.push(_defTerm(yr));try{if(window.flushCfg)window.flushCfg();}catch(x){}renderYearPlan();showToast('🌱 '+yr+'학년도 예약됨 · 3월 1일 자동 시작 · 그때까지 출석 중지, 신규 가입 학생은 진급 면제');}
...
function _applyReserveIfDue(){try{var r=appConfig&&appConfig.planReserve;if(!r)return;if(_today()<(r.yr+'-03-01'))return;var prev=_termOf(r.yr-1);_autoPromote=true;_promoteExemptFrom=r.at||'';try{startNewSchoolYear(prev);}catch(e){}_autoPromote=false;_promoteExemptFrom='';var arr=_planTerms();var t=arr.find(function(x){return x.yr===r.yr;});if(!t){t=_defTerm(r.yr);arr.push(t);}t.locked=true;delete appConfig.planReserve;try{if(window.flushCfg)window.flushCfg();}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}}catch(e){}}
function _curSchoolYr(){var n=new Date();return n.getMonth()>=2?n.getFullYear():n.getFullYear()-1;}
...
function _defTerm(yr){return {yr:yr,start:yr+'-03-01',end:(yr+1)+'-02-'+pad2(new Date(yr+1,2,0).getDate()),locked:false};}
function planNav(d){_planViewYr+=d;renderYearPlan();}
function savePlanTerm(lock){if(!_canEditPlan())return;if(lock){var prev=_termOf(_planViewYr-1);if(!startNewSchoolYear(prev))return;}var def=_defTerm(_planViewYr);var arr=_planTerms();var t=arr.find(function(x){return x.yr===_planViewYr;});if(!t){t={yr:_planViewYr};arr.push(t);}t.start=def.start;t.end=def.end;t.locked=!!lock;try{if(window.flushCfg)window.flushCfg();}catch(x){}renderYearPlan();showToast(lock?'🔒 '+_planViewYr+'학년도를 시작했어요':'💾 저장했어요');}
function unlockPlanTerm(){if(!_canEditPlan())return;var t=_termOf(_planViewYr);if(t){t.locked=false;try{if(window.flushCfg)window.flushCfg();}catch(x){}renderYearPlan();}}
function _checkPlanEnd(){try{var terms=_planTerms(),td=toDateStr(new Date());for(var i=0;i<terms.length;i++){var t=terms[i];if(t.locked&&t.end<td&&!terms.some(function(x){return x.yr===t.yr+1;})){if(appConfig.planEndNotified===t.yr)return;appConfig.planEndNotified=t.yr;(pendingList||[]).filter(function(u){return u.approved&&u.role==='teacher'&&!u.hidden&&(u.teacherType==='principal'||u.teacherType==='admin'||u.isAdmin);}).forEach(function(u){notifications.unshift({pushed:false,id:'nt-planend-'+t.yr+'-'+u.id,text:'📋 <b>'+t.yr+'학년도 운영 기간이 끝났어요.</b> 일정 → 연간계획에서 새 학년도 계획을 시작해 주세요.',time:'방금',ts:Date.now(),readBy:[],forTeacherId:u.id});});try{if(window.flushCfg)window.flushCfg();}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}updateNotifDot();return;}}}catch(e){}}
function openYearPlan(){if(G.role!=='teacher'){showToast('교사 전용이에요');return;}_planTerms();_planViewYr=_curSchoolYr();renderYearPlan();openModal('yearplan-modal');}
...
function renderYearPlan(){var edit=_canEditPlan();
  ...
  var yr=_planViewYr,term=_termOf(yr),draft=term||_defTerm(yr);
  var locked=!!(term&&term.locked),editing=edit&&!locked;
  var dispStart=draft.start,dispEnd=draft.end;
  var ended=term&&toDateStr(new Date())>term.end;
  ...
  if(per){var fmt=function(x){var p=x.split('-');return p[0]+'.'+p[1]+'.'+p[2];};
    per.innerHTML='<div style="max-width:760px;margin:0 auto;display:flex;align-items:center;gap:6px;flex-wrap:wrap"><button onclick="planNav(-1)" style="background:none;border:none;font-size:16px;cursor:pointer;color:var(--text-sub);padding:0 4px">‹</button><span style="font-weight:800;font-size:13px;min-width:76px;text-align:center">'+yr+'학년도</span><button onclick="planNav(1)" style="background:none;border:none;font-size:16px;cursor:pointer;color:var(--text-sub);padding:0 4px">›</button>'
      ...
      +(editing?'<button onclick="savePlanTerm(false)" style="background:var(--bg);border:1px solid var(--border-light);border-radius:7px;padding:6px 10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">'+(term?'저장됨':'💾 저장')+'</button>'+((appConfig&&appConfig.planReserve&&appConfig.planReserve.yr===yr)?'<span style="font-size:11px;color:#2D9E8F;font-weight:800">🌱 예약됨(3월 자동시작)</span><button onclick="cancelReserve()" style="background:none;border:none;color:var(--coral);font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-decoration:underline">취소</button>':(_today()>=(yr+'-02-01')?'<button onclick="reservePlanTerm()" style="background:var(--primary);color:#fff;border:none;border-radius:7px;padding:6px 10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🌱 새 학년도 예약</button>':'<span style="font-size:10px;color:var(--text-light)">예약은 '+yr+'.2월부터</span>')):(edit?'<button onclick="unlockPlanTerm()" style="background:#fff3cd;color:#9A6A00;border:none;border-radius:7px;padding:6px 10px;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit">🔓 잠금해제</button>':''))
      +'</div>'+(!term?'<div style="max-width:760px;margin:6px auto 0;font-size:11px;color:var(--text-light)">아직 계획이 없는 학년도예요. 저장(미리 작성) 또는 시작(잠금)하세요. 기간은 자동입니다.</div>':(ended?'<div style="max-width:760px;margin:6px auto 0;font-size:11px;color:#e57373;font-weight:700">⚠️ 이 학년도가 끝났어요. › 로 다음 학년도를 시작해 주세요.</div>':''));
    /* ===== TEST-START (배포 전 제거) ===== */
    if(_canEditPlan())per.innerHTML+='<div style="max-width:760px;margin:8px auto 0"><button onclick="_planTest()" style="background:#eee;border:none;border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;font-family:inherit;color:#888">🧪 테스트</button> <span style="font-size:10px;color:#bbb">'+(__TEST_DATE?('가상날짜 '+__TEST_DATE):'실제 날짜')+'</span></div>';
    /* ===== TEST-END ===== */
  }
  var thead=document.getElementById('yp-thead');
  if(thead)thead.innerHTML='<div style="max-width:760px;margin:0 auto;display:flex;gap:6px;padding:9px 8px;font-size:11px;font-weight:800;color:var(--text-sub)"><span style="width:46px;flex-shrink:0">날짜</span><span style="flex:1.5">전례시기</span><span style="flex:1.5">교리</span><span style="flex:1">진행</span><span style="width:30px;flex-shrink:0;text-align:center">📝</span></div>';
...
function setPlan(ds,key,val){var r=litFor(ds);if(!r){r={id:'lt'+ds,date:ds};litData.push(r);}r[key]=(val||'').trim();try{if(typeof flushSync==='function')flushSync();}catch(e){}}
...
function confirmResetQR(){closeModal('qr-reset-confirm-modal');qrState.code='';qrState.resetUsed=true;try{appConfig.qr={week:qrState.week||currentSaturday(),code:'',resetUsed:true};}catch(e){}syncQRUI();showToast('QR이 초기화되었습니다. 새 QR을 생성할 수 있어요');}
...
function renderMyPosts(filter){const el=document.getElementById('my-posts-list');if(!el)return;let mine=posts.filter(p=>p.authorId===G.id);if(filter==='board')mine=mine.filter(p=>!p.isActivity&&!p.isResource);else if(filter==='activity')mine=mine.filter(p=>p.isActivity);else if(filter==='resource')mine=mine.filter(p=>p.isResource);else if(filter==='jabumo')mine=mine.filter(p=>p.cat==='jabumo');if(!mine.length){el.innerHTML='<div class="empty" style="padding:32px"><div class="empty-emoji" style="font-size:32px">📄</div><div class="empty-title" style="font-size:13px">작성한 게시물이 없어요</div></div>';return;}el.innerHTML=mine.map(p=>`<div class="post-card" style="cursor:pointer" onclick="openMyPostDetail('${p.id}')">${p.image?`<img src="${p.image}" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);margin-bottom:8px;background:var(--bg)">`:''}<div class="post-title">${p.title}</div><div class="post-meta"><span class="post-meta-chip">${CAT_LABEL[p.cat]||'-'}</span><span>${p.date}</span><span style="margin-left:auto;color:var(--primary);font-weight:700">보기 ›</span></div></div>`).join('');}
