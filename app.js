...
function openProfileView(uid){var u=(pendingList||[]).find(function(x){return x.id===uid;})||(uid===G.id?(_meRec()||G):null);if(!u)return;var av=document.getElementById('pv-avatar');if(av){av.onclick=u.avatar?function(){openAvatarFull(u.avatar);}:null;av.style.cursor=u.avatar?'zoom-in':'default';if(u.avatar){av.style.backgroundImage="url('"+u.avatar+"')";av.innerHTML='';}else{av.style.backgroundImage='none';av.innerHTML='<svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.92)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="3.4"/><path d="M4.5 20a7.5 7.5 0 0 1 15 0"/></svg>';}}document.getElementById('pv-name').textContent=((u.name)||'')+' '+((u.baptism)||'');document.getElementById('pv-role').textContent=(u.role==='student'?((u.gradeLabel)||'학생'):u.role==='parent'?'학부모':'교사');var _pvs=document.getElementById('pv-status');if(_pvs){if(u.statusMsg){_pvs.textContent=u.statusMsg;_pvs.style.display='';}else{_pvs.textContent='';_pvs.style.display='none';}}openModal('profile-view-modal');}
