...
function _imgActions(src,name){var s=(src||'').replace(/'/g,"\\'");var dl='<button class="ico-btn" title="다운로드" onclick="dlImg(\''+s+'\',\''+(name||'')+'\')">'+_svgDl()+'</button>';var sh=_canShareFiles()?'<button class="ico-btn" title="공유" onclick="shareImg(\''+s+'\',\''+(name||'')+'\')">'+_svgShare()+'</button>':'';return '<div style="display:flex;gap:8px;justify-content:flex-end;margin:4px 0 10px">'+dl+sh+'</div>';}
...
function postImgsHTML(p){const imgs=p.images?p.images.map(im=>typeof im==='object'?im.src:im):(p.image?[p.image]:[]);return imgs.length?`<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">${imgs.map((s,i)=>`<div><img src="${s}" onclick="openImgFull('${p.id}',${i})" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);background:var(--bg);cursor:pointer">${_imgActions(s,(p.title||'image'))}</div>`).join('')}</div>`:'';}
...
function openResourceDetail(id){const r=resources.find(r=>r.id===id);if(!r)return;currentResourceId=id;const canEdit=r.authorId===G.id||G.type==='principal'||G.type==='admin'||G.isAdmin;show('rd-edit-btn',canEdit);show('rd-delete-btn',canEdit);document.getElementById('rd-title').textContent=r.title;document.getElementById('rd-meta').textContent=RESOURCE_CAT_LABEL[r.cat]+' · '+r.authorName+' · '+r.date;document.getElementById('rd-content').textContent=r.content||'';var _lk=document.getElementById('rd-link');if(_lk)_lk.innerHTML=r.link?('<a href="'+_esc(r.link)+'" class="btn btn-sm btn-primary" style="width:100%;text-decoration:none;display:flex;align-items:center;justify-content:center">'+_linkIcon(r.link)+' 문서 열기 ↗</a>'):'';document.getElementById('rd-images').innerHTML=(r.images||[]).map(im=>`<img src="${_srcOf(im)}" style="width:100%;border-radius:var(--radius-sm)">${_imgActions(_srcOf(im),(r.title||'image'))}`).join('');document.getElementById('rd-docs').innerHTML=(r.docs||[]).map((d,ix)=>`<div style="background:var(--bg);border-radius:var(--radius-sm);padding:12px 14px"><div style="display:flex;align-items:center;gap:8px;font-size:13px"><span>${_docIcon(d)}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600">${d.name}</span>${_docSizeLabel(d)?`<span style="color:var(--text-light);font-size:10px">${_docSizeLabel(d)}</span>`:''}</div>${_docBtns('Res',r.id,ix,d)}</div>`).join('');openModal('resource-detail-modal');}
