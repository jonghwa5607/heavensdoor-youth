...
function closeAvatarFull(fromPop){var m=document.getElementById('avatar-full-modal');if(!m||!m.classList.contains('open'))return;closeModal('avatar-full-modal');if(!fromPop&&window._afHist){window._afHist=false;try{history.back();}catch(e){}}else{window._afHist=false;}}
