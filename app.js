...
function submitWeeklyNotice(){
  ...
  if(ex){ ex.title=title; ex.content=body; ex.edited=true; ex.ts=Date.now(); if(_wimgs.length){ex.images=_wimgs.map(function(s){return {src:s};});ex.image=_wimgs[0];}else{delete ex.images;ex.image='';} }
  else{
    var _np={id:id,ts:Date.now(),pushed:false,title:title,content:body,cat:'notice',target:'all',grade:'all',
