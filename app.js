...
function attendGuard(){if(gradGuard())return null;const sat=currentSaturday();if(isVacationDate(sat)){showToast('이번 주는 방학이라 출석체크를 진행하지 않아요');return null;}if(!qrState.code||qrState.week!==sat){showToast('아직 이번 주 QR이 생성되지 않았어요');return null;}G.attendedWeeks=G.attendedWeeks||[];if(G.attendedWeeks.includes(sat)){showToast('이미 이번 주 출석 처리되었어요');return null;}return sat;}
...
var _planEditMode=false,_planDraft=null;
function confirmPlanTerm(){if(!_canEditPlan())return;var s=(document.getElementById('yp-s')||{}).value,e=(document.getElementById('yp-e')||{}).value;if(!s||!e||s>=e){showToast('기간을 올바르게 선택해 주세요');return;}appConfig.planStart=s;appConfig.planEnd=e;appConfig.planEndNotified='';_planEditMode=false;_planDraft=null;try{if(window.flushCfg)window.flushCfg();}catch(x){}renderYearPlan();showToast('✅ 학년도 기간이 확정됐어요');}
function startNewPlanTerm(){if(!_canEditPlan())return;var pe=(appConfig&&appConfig.planEnd)||'';var sy=pe?parseInt(pe.slice(0,4),10):new Date().getFullYear();var ns=sy+'-03-01',ne=(sy+1)+'-02-'+pad2(new Date(sy+1,2,0).getDate());_planDraft={start:ns,end:ne};_planEditMode=true;renderYearPlan();}
function _checkPlanEnd(){try{var pe=appConfig&&appConfig.planEnd;if(!pe)return;if(toDateStr(new Date())<=pe)return;if(appConfig.planEndNotified===pe)return;appConfig.planEndNotified=pe;(pendingList||[]).filter(function(u){return u.approved&&u.role==='teacher'&&!u.hidden&&(u.teacherType==='principal'||u.teacherType==='admin'||u.isAdmin);}).forEach(function(u){notifications.unshift({pushed:false,id:'nt-planend-'+pe+'-'+u.id,text:'📋 <b>학년도 운영 기간이 끝났어요.</b> 일정 → 연간계획에서 새 학년도 기간을 설정해 주세요.',time:'방금',ts:Date.now(),readBy:[],forTeacherId:u.id});});try{if(window.flushCfg)window.flushCfg();}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}updateNotifDot();}catch(e){}}
function openYearPlan(){if(G.role!=='teacher'){showToast('교사 전용이에요');return;}_planEditMode=false;_planDraft=null;renderYearPlan();openModal('yearplan-modal');}
function _planRange(){var s=(appConfig&&appConfig.planStart)||'',e=(appConfig&&appConfig.planEnd)||'';if(!s||!e){var n=new Date();var sy=n.getMonth()>=2?n.getFullYear():n.getFullYear()-1;s=sy+'-03-01';var ey=sy+1;e=ey+'-02-'+pad2(new Date(ey,2,0).getDate());}return {start:s,end:e};}
...
function setPlanRange(key,val){if(!_canEditPlan())return;appConfig[key]=val||'';try{if(window.flushCfg)window.flushCfg();}catch(e){}renderYearPlan();}
function renderYearPlan(){var edit=_canEditPlan();
  var termSet=!!(appConfig&&appConfig.planStart&&appConfig.planEnd);
  var ended=termSet&&toDateStr(new Date())>appConfig.planEnd;
  var editing=edit&&(_planEditMode||!termSet);
  var rg=_planRange();
  var dispStart=termSet?appConfig.planStart:rg.start,dispEnd=termSet?appConfig.planEnd:rg.end;
  var sVal=(_planDraft&&_planDraft.start)||(appConfig&&appConfig.planStart)||rg.start;
  var eVal=(_planDraft&&_planDraft.end)||(appConfig&&appConfig.planEnd)||rg.end;
  var termYr=(parseInt(dispStart.slice(5,7),10)>=3?parseInt(dispStart.slice(0,4),10):parseInt(dispStart.slice(0,4),10)-1);
  var per=document.getElementById('yp-period');
  if(per){var fmt=function(x){var p=x.split('-');return p[0]+'.'+p[1]+'.'+p[2];};var di='font-size:12px;padding:5px 8px;border:1px solid var(--border-light);border-radius:6px;font-family:inherit';
    per.innerHTML='<div style="max-width:760px;margin:0 auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-weight:800;font-size:13px">'+termYr+'학년도</span>'
      +(editing?'<span style="font-size:12px;color:var(--text-light)">기간</span><input type="date" id="yp-s" value="'+sVal+'" style="'+di+'"><span style="color:var(--text-light)">~</span><input type="date" id="yp-e" value="'+eVal+'" style="'+di+'"><button onclick="confirmPlanTerm()" style="background:var(--primary);color:#fff;border:none;border-radius:7px;padding:6px 12px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✅ 확정</button>'
        :'<span style="font-size:12px;color:var(--text-sub)">'+fmt(dispStart)+' ~ '+fmt(dispEnd)+'</span>'+(ended&&edit?'<button onclick="startNewPlanTerm()" style="background:#fff3cd;color:#9A6A00;border:none;border-radius:7px;padding:6px 12px;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit">🎓 새 학년도 시작</button>':''))
      +'</div>'+(ended&&!editing?'<div style="max-width:760px;margin:6px auto 0;font-size:11px;color:#e57373;font-weight:700">⚠️ 학년도 기간이 끝났어요. 새 학년도를 시작해 주세요.</div>':'');}
