...
function _canStudentSee(p,gk){if(!p)return false;if(p.cat==='event')return true;var t=p.target||'all';if(t==='all')return true;if(t==='student'){if(p.cat==='notice'){return p.grade==='all'||p.grade==='all-s'||!p.grade||p.grade===gk;}return true;}return false;}
function _canParentSee(p){if(!p)return false;if(p.cat==='event')return true;var t=p.target||'all';return t==='all'||t==='parent';}
function filterBoardStudent(gk){currentBoardCat='notice';show('fab-board',false);renderBoardList('notice',posts.filter(p=>p.cat==='notice'&&_canStudentSee(p,gk)));}
function filterBoardParent(){currentBoardCat='notice';show('fab-board',false);renderBoardList('notice',posts.filter(p=>p.cat==='notice'&&_canParentSee(p)));}
function selParentCat(cat,btn){selCatTab(btn);currentBoardCat=cat;show('fab-board',cat==='jabumo'&&!!G.isJabumoPresident);if(cat==='jabumo')renderBoardList(cat,posts.filter(p=>p.cat==='jabumo'));else renderBoardList(cat,posts.filter(p=>p.cat===cat&&_canParentSee(p)));}
