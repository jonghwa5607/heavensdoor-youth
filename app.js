...
function renderStudentGrowth(){try{
  ...
  if(stg){stg.innerHTML=ATTEND_LEVELS.map(function(L,i){var sp=L.l.split(' ');var em=sp[0];var nm=sp[1]||'';var ach=t>=L.n;var cur=(pv===L.n);var arrow=(i<ATTEND_LEVELS.length-1)?'<div style="flex:0 0 auto;align-self:center;color:var(--border);font-size:10px;padding-top:8px">···</div>':'';return '<div style="flex:1 1 0;min-width:0;text-align:center;padding:'+(cur?'6px 2px':'6px 2px')+';border-radius:12px;background:'+(cur?'var(--primary-light)':'transparent')+'"><div style="font-size:24px;line-height:1;filter:'+(ach?'none':'grayscale(1)')+';opacity:'+(ach?'1':'.45')+'">'+em+'</div><div style="font-size:10px;margin-top:4px;font-weight:'+(cur?'800':'600')+';color:'+(cur?'var(--primary-dark)':'var(--text-light)')+'">'+nm+'</div></div>'+arrow;}).join('');}
  ...
  if(ag){var curLv=null;ATTEND_LEVELS.forEach(function(L){if(t>=L.n)curLv=L;});var cLabel=curLv?curLv.l:'🌱 씨앗';var mth=(typeof monthAttendCount==='function')?monthAttendCount(G):0;
    ag.innerHTML='<div class="card" style="padding:16px;text-align:left;background:linear-gradient(120deg,var(--mint-light),var(--primary-light))">'
      +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px"><div style="min-width:0"><div style="font-size:17px;font-weight:800;color:var(--text)">안녕, '+_esc(((G.name?G.name+' ':'')+(G.baptism||'')).trim())+'!</div><div style="font-size:12px;color:var(--text-sub);margin-top:4px">오늘도 꾸준히 성장하고 있어요</div></div><span style="flex-shrink:0;font-size:11px;font-weight:800;color:var(--primary-dark);background:#fff;padding:5px 11px;border-radius:20px">'+cLabel+'</span></div>'
