...
function deleteMinutes(){if(!currentMinutesId)return;var r=resources.find(x=>x.id===currentMinutesId);if(!r)return;if(!(_isFullAdmin()||r.authorId===G.id||r.createdById===G.id)){showToast('작성자 또는 교감·교무·관리자만 삭제할 수 있어요');return;}if(!confirm('이 회의록을 휴지통으로 옮길까요?\n\n· '+(r.title||'회의록')+'\n\n휴지통에서 복구할 수 있어요.'))return;const id=currentMinutesId;_minEditing=false;clearInterval(_minHbTimer);_releaseLock();r.deleted=true;r.deletedAt=Date.now();r.deletedBy=G.displayName;closeModal('minutes-viewer-modal');renderResourceList();try{renderMinutesHub();}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}showToast('휴지통으로 옮겼어요 · 복구 가능');}
...
function purgeMinutes(id){if(!_isFullAdmin()){showToast('교감·교무·관리자만 영구 삭제할 수 있어요');return;}var r=resources.find(x=>x.id===id);if(!r)return;if(!confirm('⚠️ 영구 삭제하면 복구할 수 없어요.\n\n· '+(r.title||'회의록')+'\n\n정말 삭제할까요?'))return;resources=resources.filter(x=>x.id!==id);try{if(window.FB&&FB.enabled()&&FB.remove)FB.remove('resources',id);}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}openMinutesTrash();renderResourceList();showToast('영구 삭제했어요');}
...
function openAvatarFull(src){if(!src)return;var m=document.getElementById('avatar-full-modal'),im=document.getElementById('avatar-full-img');if(!m||!im)return;im.src=src;m.style.display='flex';m.classList.add('open');try{history.pushState({avatarFull:1},'');window._avatarHist=true;}catch(e){}}
function closeAvatarFull(){var m=document.getElementById('avatar-full-modal');if(m){m.style.display='none';m.classList.remove('open');}if(window._avatarHist){window._avatarHist=false;try{history.back();}catch(e){}}}
window.addEventListener('popstate',function(){var m=document.getElementById('avatar-full-modal');if(m&&m.style.display==='flex'){window._avatarHist=false;m.style.display='none';m.classList.remove('open');}});
