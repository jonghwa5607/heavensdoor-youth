...
function openYoutube(id){if(!id)return;try{window.open('https://www.youtube.com/watch?v='+id,'_blank');}catch(e){location.href='https://www.youtube.com/watch?v='+id;}}
...
function buildEventCardHtml(e){
  ...
  const clickAttr=yid?`onclick="openYoutube('${yid}')"`:`onclick="openEventDetail('${e.id}')"`;
  ...
  if(_ec){
    return `<div class="event-slide" ${clickAttr} style="${dim}"><div class="event-slide-img" style="position:relative"><img src="${_ec}">${play}</div><div class="event-slide-info"><div class="event-slide-title">${yid?'▶ ':''}${e.title}</div>${badge}</div></div>`;
  }
