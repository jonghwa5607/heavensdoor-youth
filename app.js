...
function renderDeptPosts(dept,sub){if(sub!==undefined)deptSubFilter[dept]=sub;const f=deptSubFilter[dept];try{var _tw=document.getElementById('dept-'+dept);if(_tw){var _tabs=_tw.querySelectorAll('.tab-bar .tab-btn');_tabs.forEach(function(b){var oc=b.getAttribute('onclick')||'';b.classList.toggle('active',oc.indexOf("'"+dept+"','"+f+"'")>=0);});}}catch(e){}const el=document.getElementById(dept+'-posts');if(!el)return;const arr=posts.filter(p=>p.cat==='activity'&&p.dept===dept&&(f==='all'||p.subcat===f));var _ico=dept==='choir'?'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="18" r="2.6"/><circle cx="17" cy="16" r="2.6"/><path d="M8.6 18V5.2l11-2v12.6"/><path d="M8.6 8.6l11-2"/></svg>':'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="M7 8h10"/></svg>';if(!arr.length){el.innerHTML='<div class="empty"><div class="empty-emoji">'+_ico+'</div><div class="empty-title">아직 게시물이 없어요</div></div>';try{_deptFit(dept);}catch(e){}return;}const canAdmin=G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);el.innerHTML=arr.map(p=>{const isOwner=p.authorId===G.id;const moreBtn=(isOwner||canAdmin)?`<button class="post-more-btn" onclick="event.stopPropagation();openPostActions('${p.id}',${isOwner},true)">⋯</button>`:'';const subChip=`<span class="chip chip-blue">${p.subcat==='notice'?'공지':dept==='choir'?'악보/자료':'전례자료'}</span>`;return `<div class="post-card" data-pid="${p.id}" style="cursor:pointer"><div class="post-head"><div class="post-tags">${subChip}</div>${moreBtn}</div><div class="post-title">${p.title}</div><div class="post-meta"><span class="post-meta-chip">${p.authorName||'익명'}</span><span>${p.date}</span>${p.edited?'<span class="post-edited">(수정됨)</span>':''}</div>${p.image?`<img src="${p.image}" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);margin:8px 0;background:var(--bg)">`:''}${p.content?`<div class="post-preview">${p.content.slice(0,60)}${p.content.length>60?'...':''}</div>`:''}<div class="post-footer"><span class="post-stat">💬 ${(p.comments||[]).length}</span>${p.docs&&p.docs.length?`<span class="post-stat">📎 ${p.docs.length}</span>`:''}</div></div>`;}).join('');try{_deptFit(dept);}catch(e){}}
