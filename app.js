...
function openAvatarFull(src){if(!src)return;var im=document.getElementById('avatar-full-img');if(!im)return;im.src=src;openModal('avatar-full-modal');try{history.pushState({af:1},'');window._afHist=true;}catch(e){}}
function closeAvatarFull(fromPop){var m=document.getElementById('avatar-full-modal');if(!m||!m.classList.contains('open'))return;closeModal('avatar-full-modal');try{var pv=document.getElementById('profile-view-modal');if(pv&&!pv.classList.contains('open'))openModal('profile-view-modal');}catch(e){}if(!fromPop&&window._afHist){window._afHist=false;try{history.back();}catch(e){}}else{window._afHist=false;}}
window.addEventListener('popstate',function(){var m=document.getElementById('avatar-full-modal');if(m&&m.classList.contains('open')){closeAvatarFull(true);}});
