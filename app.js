...
function openYearPlan(){if(G.role!=='teacher'){showToast('교사 전용이에요');return;}renderYearPlan();openModal('yearplan-modal');}
...
function renderYearPlan(){var rg=_planRange();var edit=_canEditPlan();
  var sy=parseInt(rg.start.slice(0,4),10);var termYr=(parseInt(rg.start.slice(5,7),10)>=3?sy:sy-1);
  ...
  if(per){var fmt=function(x){var p=x.split('-');return p[0]+'.'+p[1]+'.'+p[2];};
    per.innerHTML='<div style="max-width:760px;margin:0 auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-weight:800;font-size:13px">'+termYr+'학년도</span>'
      +(edit?'<span style="font-size:12px;color:var(--text-light)">기간</span><input type="date" value="'+rg.start+'" onchange="setPlanRange(\'planStart\',this.value)" style="font-size:12px;padding:5px 8px;border:1px solid var(--border-light);border-radius:6px;font-family:inherit"><span style="color:var(--text-light)">~</span><input type="date" value="'+rg.end+'" onchange="setPlanRange(\'planEnd\',this.value)" style="font-size:12px;padding:5px 8px;border:1px solid var(--border-light);border-radius:6px;font-family:inherit">'
        :'<span style="font-size:12px;color:var(--text-sub)">'+fmt(rg.start)+' ~ '+fmt(rg.end)+'</span>')+'</div>';}
  ...
  var el=document.getElementById('yp-body');if(!el)return;var sats=_satsInRange(rg.start,rg.end);
