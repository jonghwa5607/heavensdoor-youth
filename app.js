...
function openWeeklyNotice(){
  ...
  if(lock){
    ...
    lock.innerHTML=!_canWriteWeekly()
      ? '<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> 주간공지는 <b>교감·교무</b>가 작성합니다'
      : (ex?('<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> <b>'+_esc(ex.authorName||'')+'</b> 님이 이미 올렸어요 · 수정은 작성자만 할 수 있어요'):'');
  }
...
function submitWeeklyNotice(){
  ...
  if(_wsdt){
    ...
    try{appConfig.wnScheduled=sat;if(window.flushCfg)window.flushCfg();}catch(e){}
    try{notifications=notifications.filter(function(x){return !(x.id&&x.id.indexOf('nt-wnrem-'+sat+'-')===0);});updateNotifDot();}catch(e){}
  ...
  try{if(appConfig.wnScheduled===sat){delete appConfig.wnScheduled;if(window.flushCfg)window.flushCfg();}}catch(e){}
  try{
    notifications=notifications.filter(function(x){return x.id!=='nt-'+id&&!(x.id&&x.id.indexOf('nt-wnrem-'+sat+'-')===0);});
...
function checkThursdayNotice(){
  try{
    ...
    targets.forEach(function(u){
      ...
      notifications.unshift({pushed:false,id:nid,text:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg> <b>'+_wnTitle(sat)+'</b>를 올려주세요 (목요일)',
...
var _ORDINARY_ART='data:image/svg+xml;utf8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 150"><circle cx="322" cy="58" r="60" fill="white" opacity="0.3"/><g fill="white" opacity="0.92"><polygon points="296,80 336,50 376,80"/><rect x="302" y="80" width="68" height="60" rx="3"/><polygon points="327,42 336,29 345,42"/><rect x="330" y="42" width="12" height="24"/></g><g stroke="#5E8C74" stroke-width="2.6" stroke-linecap="round" opacity="0.85"><line x1="336" y1="13" x2="336" y2="30"/><line x1="329" y1="20" x2="343" y2="20"/></g><rect x="327" y="104" width="18" height="36" rx="9" fill="#86B79B" opacity="0.55"/><circle cx="336" cy="92" r="6" fill="#86B79B" opacity="0.5"/><g opacity="0.92"><path d="M258 141 q-5 -30 5 -48 q9 14 3 48 z" fill="#7FC9A0"/><path d="M270 141 q7 -25 20 -33 q-3 17 -12 33 z" fill="#9BDAB6"/><path d="M384 141 q6 -26 -6 -40 q-7 14 -1 40 z" fill="#7FC9A0"/></g></svg>');
...
const img=(appConfig.seasonImgs||{})[k]||S.builtImg||(k==='ordinary'?_ORDINARY_ART:'');
...
function uploadLogo(inp){var f=inp.files&&inp.files[0];if(!f){return;}compressImg(f,640,0.92,'image/png').then(function(data){if(!data){showToast('이미지를 불러오지 못했어요');return;}appConfig.logo=data;applyLogo();showToast('앱 로고가 변경되었어요');});inp.value='';}
...
const GRADE_LABEL={m1:'중1',m2:'중2',m3:'중3',h:'고등','all-s':'전체','all-p':'전체',all:'전체'};
...
const ATTEND_LEVELS=[{n:1,l:'🌱 씨앗',r:'축하 스티커'},{n:5,l:'🌿 새싹',r:'간식 교환권'},{n:10,l:'🪴 어린나무',r:'음료 교환권'},{n:20,l:'🌳 큰나무',r:'문화상품권 5천원'},{n:35,l:'🌸 꽃',r:'문화상품권 1만원'},{n:45,l:'🍎 열매',r:'문화상품권 2만원'}];
function crownCount(u){if(!u)return 0;let c=((u.history)||[]).filter(x=>(x.attendTotal||0)>=45).length;if((u.attendTotal||0)>=45)c++;return c;}
function crownMark(u){const c=crownCount(u);return c<=0?'':' '+'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>'.repeat(Math.min(c,5));}
function _treeByLabel(label,size){if(!label)return '';for(var i=0;i<ATTEND_LEVELS.length;i++){var nm=ATTEND_LEVELS[i].l.split(' ')[1];if(nm&&label.indexOf(nm)>=0)return _treeSVG(i,size||26);}return '';}
function _labelNoEmoji(label){return (label||'').replace(/^\S+\s+/,'');}
...
if(!G.graduated){const cur=`<div class="card" style="margin-bottom:8px;cursor:pointer;border-left:4px solid var(--mint)" onclick="showHistoryWeeks(-1)"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;font-weight:700">${yr}년 ${G.grade?'('+G.grade+')':''} <span class="chip chip-mint" style="margin-left:4px">진행 중</span></div><span class="chip chip-blue">${t>0?lv:'-'}</span></div><div style="font-size:11px;color:var(--text-light);margin-top:5px">출석 ${t}회${t>=45?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg> 열매 달성!':''} · 눌러서 출석일 보기</div></div>`;el.innerHTML='<div style="font-size:13px;font-weight:800;margin-bottom:8px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg> 나의 연혁</div>'+cur+hist.slice().reverse().map((x,i)=>`<div class="card" style="margin-bottom:8px;cursor:pointer" onclick="showHistoryWeeks(${hist.length-1-i})"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;font-weight:700">${x.year}년 ${x.grade?'('+x.grade+')':''}</div><span class="chip chip-blue">${x.level}</span></div><div style="font-size:11px;color:var(--text-light);margin-top:5px">출석 ${x.attendTotal}회${(x.attendTotal||0)>=45?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg> 열매의 해!':''} · 눌러서 출석일 보기</div></div>`).join('')+'<div style="height:8px"></div>';return;}
const entries=[{year:yr,grade:G.grade||'',attendTotal:t,level:t>0?lv:'🌱 씨앗',weeks:G.attendedWeeks||[],cur:!G.graduated,idx:-1},...hist.slice().reverse().map((x,i)=>({...x,idx:hist.length-1-i}))];const totalAll=entries.reduce((s,x)=>s+(x.attendTotal||0),0);const crowns=entries.filter(x=>(x.attendTotal||0)>=45).length;
const summary=`<div style="background:linear-gradient(135deg,var(--lavender-light),var(--primary-light));border-radius:var(--radius-lg);padding:14px 10px;margin-bottom:14px;display:grid;grid-template-columns:repeat(3,1fr);text-align:center">${G.graduated?`<div style="grid-column:1/4;font-size:12px;font-weight:800;color:var(--primary-dark);margin-bottom:8px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg> ${G.name} ${G.baptism}의 주일학교 여정</div>`:''}<div><div style="font-size:20px;font-weight:800;color:var(--primary-dark)">${entries.length}</div><div style="font-size:10px;color:var(--text-sub)">함께한 해</div></div><div style="border-left:1px solid rgba(255,255,255,.6);border-right:1px solid rgba(255,255,255,.6)"><div style="font-size:20px;font-weight:800;color:var(--mint)">${totalAll}</div><div style="font-size:10px;color:var(--text-sub)">총 출석</div></div><div><div style="font-size:20px;font-weight:800">${crowns?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>×'+crowns:'–'}</div><div style="font-size:10px;color:var(--text-sub)">열매 달성</div></div></div>`;
el.innerHTML='<div style="font-size:13px;font-weight:800;margin-bottom:10px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg> 나의 신앙 발자취</div>'+summary+'<div style="position:relative;padding-left:28px"><div style="position:absolute;left:10px;top:12px;bottom:12px;width:2px;background:linear-gradient(var(--mint),var(--lavender))"></div>'+entries.map(x=>{const at=x.attendTotal||0;const pct=Math.min(100,Math.round(at/45*100));const emoji=(x.level||'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg> 씨앗').split(' ')[0];return `<div style="position:relative;margin-bottom:12px"><div style="position:absolute;left:-28px;top:12px;width:22px;height:22px;border-radius:50%;background:white;border:2px solid ${x.cur?'var(--mint)':'var(--lavender)'};display:flex;align-items:center;justify-content:center;font-size:11px;box-shadow:0 1px 4px rgba(0,0,0,.08)">${emoji}</div><div class="card" style="cursor:pointer${x.cur?';border:1.5px solid var(--mint)':''}" onclick="showHistoryWeeks(${x.idx})"><div style="display:flex;justify-content:space-between;align-items:center;gap:6px"><div style="font-size:13px;font-weight:800">${x.year}년${x.grade?' · '+x.grade:''} ${x.cur?'<span class="chip chip-mint" style="font-size:9px">진행 중</span>':''}</div><span class="chip chip-blue" style="flex-shrink:0">${x.level}</span></div><div style="margin-top:9px;height:6px;background:var(--bg);border-radius:4px;overflow:hidden"><div style="width:${pct}%;height:100%;background:linear-gradient(90deg,var(--mint),var(--primary));border-radius:4px"></div></div><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-light);margin-top:5px"><span>출석 ${at}회</span><span>${at>=45?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg> 열매 달성!':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>까지 '+(45-at)+'회'}</span></div></div></div>`;}).join('')+'</div><div style="font-size:10px;color:var(--text-light);text-align:center;margin:2px 0 8px">카드를 누르면 그해의 출석 발자국을 볼 수 있어요 👣</div>';}
...
function renderGovSection(){const el=document.getElementById('gov-section');if(!el)return;const h=adminHolder();const isMe=h&&G.id===h.id;let voteHtml='';if(adminVote){const cand=pendingList.find(u=>u.id===adminVote.candidateId);const elig=eligibleVoters();const need=Math.floor(elig.length/2)+1;const voted=adminVote.votes.includes(G.id);const canVote=!isMe&&!voted&&elig.some(u=>u.id===G.id);voteHtml=`<div style="background:var(--coral-light);border-radius:var(--radius);padding:14px;margin-bottom:8px"><div style="font-size:13px;font-weight:800;margin-bottom:4px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> 관리자 교체 투표 진행 중</div><div style="font-size:12px;color:var(--text-sub);line-height:1.6">후보: <b>${cand?cand.name+' '+cand.baptism:'?'}</b> · 발의: ${adminVote.proposerName}<br>찬성 <b>${adminVote.votes.length}</b> / 정족수 <b>${need}</b> (교사 ${elig.length}명 중 과반)</div><div style="display:flex;gap:8px;margin-top:10px">${canVote?`<button class="btn btn-sm" style="background:var(--coral);color:white;flex:1" onclick="castAdminVote()"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 21s7-4.3 7-10V6l-7-3-7 3v5c0 5.7 7 10 7 10z"/></svg> 찬성하기</button>`:voted?'<span class="chip chip-gray" style="align-self:center"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 찬성 완료</span>':''}${adminVote.proposerId===G.id?`<button class="btn btn-sm btn-outline" style="flex:1" onclick="withdrawAdminVote()">발의 철회</button>`:''}</div></div>`;}
el.innerHTML=voteHtml+`<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 6px"><span style="font-size:11px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> 관리자: <b style="color:var(--text-sub)">${h?h.name+' '+h.baptism+(isMe?' (나)':''):'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h16.9a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> 공석'}</b></span>${isMe?`<button onclick="openAdminTransfer()" style="background:none;border:none;color:var(--text-light);font-size:10px;cursor:pointer;font-family:inherit;padding:4px;text-decoration:underline">이임</button>`:(!adminVote?`<button onclick="openAdminVoteProposal()" style="background:none;border:none;color:var(--text-light);font-size:10px;cursor:pointer;font-family:inherit;padding:4px;text-decoration:underline">교체 발의</button>`:'')}</div>`;}
...
function checkAdminVoteResult(){if(!adminVote)return;const elig=eligibleVoters();const need=Math.floor(elig.length/2)+1;if(adminVote.votes.length>=need){const cand=pendingList.find(u=>u.id===adminVote.candidateId);adminVote=null;if(cand){transferAdminTo(cand,'교사 과반 투표');showToast('과반 찬성! 관리자가 교체되었습니다');}}}
...
function renderDetailCoupons(u){dedupeBdayCoupons();const el=document.getElementById('detail-coupon-list');if(!el)return;const hint=document.getElementById('detail-coupon-hint');if(hint)hint.textContent=u.manualReg?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.8 12.8 0 0 0 2.5.4A2 2 0 0 1 22 16.92v3A2 2 0 0 1 20 22 18 18 0 0 1 3.06 6a2 2 0 0 1 2-2h.09"/><line x1="23" y1="1" x2="1" y2="23"/></svg> 수동등록 학생 · 교사가 대신 확인·사용 처리':'';const mine=coupons.filter(c=>c.studentId===u.id);if(!mine.length){el.innerHTML='<div class="card" style="text-align:center;font-size:12px;color:var(--text-light);padding:16px">보유한 쿠폰이 없어요</div>';return;}el.innerHTML=mine.slice().reverse().map(c=>`<div class="card" style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:12px;font-weight:700;display:flex;align-items:center;gap:5px">${_treeByLabel(c.badgeLabel,22)}${(_treeByLabel(c.badgeLabel)?_labelNoEmoji(c.badgeLabel):c.badgeLabel)} 쿠폰 <span style="font-size:11px;color:var(--text-light);font-weight:500">· <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg> ${c.reward||'보상'}</span></div><span class="chip ${c.used?'chip-gray':'chip-coral'}">${c.used?'사용완료':'미사용'}</span></div><div style="font-size:10px;color:var(--text-light);margin-top:6px">발급 ${c.createdAt}${c.used&&c.usedAt?' · 사용 '+c.usedAt:''}${c.used?'':' · 인증번호 <b style="color:var(--primary-dark);letter-spacing:1px">'+c.code+'</b>'}</div>${c.used?'':`<div style="display:flex;gap:6px;margin-top:8px"><input class="form-input" type="text" inputmode="numeric" id="dcoupon-code-${c.id}" placeholder="인증번호 6자리" maxlength="6" style="flex:1;min-width:0"><button class="btn btn-sm" style="background:var(--primary);color:white;width:auto;padding:6px 12px;white-space:nowrap" onclick="useCouponByTeacher('${c.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 사용</button></div>`}</div>`).join('');}
...
function switchTab(tab){if(tab==='activity'&&G.role==='student'&&G.graduated){openMyCard();return;}document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));const sc=document.getElementById('screen-'+tab);var _ord=['home','board','attend','diary','activity','teacher','admin','my','calendar'],_ni=_ord.indexOf(tab);var _dir=(window._tabIdx==null||_ni<0)?1:(_ni>=window._tabIdx?1:-1);if(_ni>=0)window._tabIdx=_ni;try{localStorage.setItem('hd-last-tab',tab);}catch(e){}if(sc){sc.style.setProperty('--sx',_dir>0?'26px':'-26px');sc.classList.remove('tab-enter');void sc.offsetWidth;sc.classList.add('active');sc.classList.add('tab-enter');setTimeout(function(){sc.classList.remove('tab-enter');},700);}document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));const nv=document.getElementById('nav-'+tab);if(nv)nv.classList.add('active');try{if(tab==='attend')renderStudentGrowth();}catch(e){}try{if(tab==='diary')renderDiaryList();if(tab==='calendar'){
    ...
    }if(tab==='activity')renderDeptInfo();if(tab==='home'){checkImportantNotices();try{updateVerseEditUI();}catch(e){}try{checkThursdayNotice();}catch(e){}try{_refreshTeacherDesc();}catch(e){}try{_fillHomeAvatars();}catch(e){}try{setTimeout(_fillHomeAvatars,400);setTimeout(_fillHomeAvatars,1200);}catch(e){}renderHomeSchedule();try{renderTeacherWeek();}catch(e){}try{renderHomeSmart();}catch(e){}try{renderHomeWeek();}catch(e){}try{renderHomeMiniCal();}catch(e){}try{renderHomeMinutes();}catch(e){}applySeason(appConfig.season||'ordinary');try{renderBdayBannerAuto();}catch(e){}try{_homePlanRetry();}catch(e){}try{setTimeout(function(){if(!document.querySelector('#important-notice-modal.open'))checkUnreadMinutes();},900);}catch(e){}}if(tab==='teacher'){autoArchiveMinutes();renderResourceList();}}catch(e){console.error('switchTab render error:',e);}try{maybeCoach(tab);}catch(e){}}
...
function _isWeeklyLike(p){return !!(p&&p.id&&p.id.indexOf('wn-')===0);}
function _renderNoticePlain(content){var box=document.getElementById('important-notice-cards');if(!box)return;box.innerHTML='<div style="font-size:14px;color:var(--text-sub);line-height:1.75;white-space:pre-wrap;word-break:break-word">'+_esc(content||'')+'</div>';}
...
function checkImportantNotices(){try{const _jt=(typeof _joinTs==='function')?_joinTs():0;const important=posts.filter(p=>p.isImportant&&(!_jt||!p.ts||p.ts>=_jt)&&(p.target==='all'||p.target===G.role)&&(p.target!=='student'||p.grade==='all-s'||p.grade===G.gradeKey)&&!(p.popupUntil&&Date.now()>p.popupUntil)&&!(p.popupDays&&p.ts&&Date.now()>p.ts+p.popupDays*86400000));for(const p of important){const key='dismissed-important-'+p.id+'-'+G.id;let dismissed=false;try{dismissed=!!localStorage.getItem(key);}catch(e){dismissed=false;}if(!dismissed){currentImportantId=p.id;document.getElementById('important-notice-title').textContent=p.title;document.getElementById('important-notice-content').textContent=p.content||'';try{var _im=document.getElementById('important-notice-imgs');if(_im){var _imgs=p.images?p.images.map(function(x){return typeof x==='object'?x.src:x;}):(p.image?[p.image]:[]);_im.innerHTML=_imgs.length?('<div style="margin-bottom:12px">'+_imgs.map(function(s){return '<img src="'+s+'" style="width:100%;height:auto;max-height:360px;object-fit:contain;border-radius:12px;background:var(--bg);margin-bottom:8px">';}).join('')+'</div>'):'';}}catch(e){}try{if(_isWeeklyLike(p))_renderNoticeCards(p.content||'');else _renderNoticePlain(p.content||'');}catch(e){}try{var _sub=document.getElementById('important-notice-sub');if(_sub)_sub.textContent=_isWeeklyLike(p)?'이번 주 주요 일정과 안내사항을 확인해 주세요.':'꼭 확인해 주세요.';}catch(e){}openModal('important-notice-modal');break;}}}catch(e){console.error('checkImportantNotices error:',e);}}
...
function checkNewCoupons(){if(G.role!=='student'||G.graduated)return;const fresh=coupons.filter(c=>c.studentId===G.id&&!c.used&&!c.seen);if(!fresh.length)return;const c=fresh[fresh.length-1];fresh.forEach(x=>x.seen=true);const from=c.manual&&c.issuedBy?c.issuedBy+' 선생님':(c.bdayKey?'하늘의문 중고등부':'선생님');document.getElementById('crm-title').innerHTML=from+'께서<br><b style="color:var(--primary-dark)">'+c.badgeLabel+' 쿠폰</b>을 보냈어요!'+(fresh.length>1?'<div style="font-size:11px;color:var(--text-light);font-weight:500;margin-top:4px">외 '+(fresh.length-1)+'개의 새 쿠폰이 있어요</div>':'');document.getElementById('crm-reward').textContent='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg> '+(c.reward||'보상');document.getElementById('crm-date').textContent='발급일: '+c.createdAt;setTimeout(()=>openModal('coupon-received-modal'),800);}
function openCouponBox(){if(gradGuard())return;renderCouponList();openModal('coupon-modal');}
function renderCouponList(){dedupeBdayCoupons();const el=document.getElementById('coupon-list');if(!el)return;const mine=coupons.filter(c=>c.studentId===G.id);if(!mine.length){el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 8a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2 2 2 0 0 0 0 4 2 2 0 0 1-2 2H5a2 2 0 0 1-2-2 2 2 0 0 0 0-4z"/><path d="M13 6v12"/></svg></div><div class="empty-title" style="font-size:13px">아직 쿠폰이 없어요</div><div class="empty-desc">등급을 달성하면 쿠폰이 생겨요!</div></div>';return;}el.innerHTML=mine.map(c=>`<div class="card" style="margin-bottom:10px;${c.used?'opacity:.5':''}"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><div style="font-size:14px;font-weight:800;display:flex;align-items:center;gap:6px">${_treeByLabel(c.badgeLabel,30)}${(_treeByLabel(c.badgeLabel)?_labelNoEmoji(c.badgeLabel):c.badgeLabel)} 쿠폰</div><div style="font-size:12px;color:var(--primary-dark);font-weight:700;margin-top:2px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg> ${c.reward||'보상'}</div><span class="chip ${c.used?'chip-gray':'chip-coral'}">${c.used?'사용완료':'미사용'}</span></div><div style="font-size:11px;color:var(--text-light);margin-bottom:10px">발급일: ${c.createdAt}</div>${c.used?'':`<div style="display:flex;gap:8px"><input class="form-input" type="text" id="coupon-code-${c.id}" placeholder="인증번호 6자리 입력" maxlength="6" style="flex:1"><button class="btn btn-sm" style="background:var(--coral);color:white" onclick="redeemCoupon('${c.id}')">사용하기</button></div>`}</div>`).join('');}
...
function renderAttendRank(){
  ...
  if(!studs.length){el.innerHTML='<div class="empty" style="padding:32px"><div class="empty-emoji" style="font-size:32px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/></svg></div><div class="empty-title" style="font-size:13px">순위 정보가 없어요</div></div>';return;}
  var medal=['🥇','🥈','🥉'];var order=[1,0,2];
  var top='<div style="font-size:13px;font-weight:800;margin-bottom:12px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/></svg> 출석 TOP 3</div><div style="display:flex;gap:8px;justify-content:center;align-items:flex-end;margin-bottom:18px">';
...
function renderAttendHistory(){const att=(G.attendedWeeks||[]);const half=(G.halfWeeks||[]);const scans=G.qrScanAt||{};const totalEl=document.getElementById('hist-total');if(totalEl)totalEl.textContent=G.attendTotal||0;const monthEl=document.getElementById('hist-month');if(monthEl)monthEl.textContent=monthAttendCount(G);const streakEl=document.getElementById('hist-streak');if(streakEl)streakEl.textContent=G.streak||0;const listEl=document.getElementById('attend-history-list');if(!listEl)return;const all=Array.from(new Set(att.concat(Object.keys(scans)))).sort().reverse();if(!all.length){listEl.innerHTML='<div class="empty" style="padding:32px"><div class="empty-emoji" style="font-size:32px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg></div><div class="empty-title" style="font-size:13px">출석 기록이 없어요</div></div>';return;}listEl.innerHTML=all.map(function(w){var attended=att.indexOf(w)>=0,isHalf=half.indexOf(w)>=0,scan=scans[w];var av,bg,nm,dt;if(attended){av=isHalf?'◐':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg>';bg='linear-gradient(135deg,var(--mint),#3DAB99)';nm=w+(isHalf?' · 반일':'');dt=scan?('<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12" y2="18"/></svg> 내 QR 인식 '+scan):'토요일 출석';}else{av='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';bg='var(--coral)';nm=w+' · 결석 처리';dt=scan?('<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12" y2="18"/></svg> 내 QR 인식 '+scan+' · 기록 보존됨'):'결석';}return '<div class="student-row"><div class="student-avatar" style="background:'+bg+'">'+av+'</div><div class="student-info"><div class="student-name">'+nm+'</div><div class="student-detail">'+dt+'</div></div></div>';}).join('');}
...
function renderGradLetterEntry(){const el=document.getElementById('home-grad-letter-section');if(!el)return;const has=pendingList.some(u=>u.role==='student'&&u.approved&&!u.hidden&&(u.gradeLabel==='고3'||u.graduated)&&u.id!==G.id);const canWrite=gradLetterOpen&&!G.graduated&&has;el.innerHTML=canWrite?`<div onclick="openGradLetters()" style="cursor:pointer;display:flex;align-items:center;gap:14px;margin:10px 14px 0;padding:16px 18px;border-radius:var(--radius-lg);background:linear-gradient(135deg,#E8A0CC,#9B8FD4);color:#fff"><div style="flex:1"><div style="font-size:11px;opacity:.8;letter-spacing:1px">GRADUATION</div><div style="font-size:15px;font-weight:800;margin-top:2px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> 졸업 축하 편지</div><div style="font-size:11.5px;opacity:.9;margin-top:3px">고3 선배에게 마음을 전해보세요</div></div><div style="font-size:18px;opacity:.9">›</div></div>`:'';el.style.display=canWrite?'':'none';}
function openGradLetters(){const list=pendingList.filter(u=>u.role==='student'&&u.approved&&!u.hidden&&(u.gradeLabel==='고3'||u.graduated)&&u.id!==G.id);const bar=document.getElementById('grad-letter-admin-bar');const canCtl=G.role==='teacher'&&(G.type==='principal'||G.isAdmin);if(bar)bar.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;background:${gradLetterOpen?'var(--mint-light)':'var(--bg)'};border:1px solid var(--border-light);border-radius:10px;padding:8px 12px"><span style="font-size:11px;font-weight:700;color:${gradLetterOpen?'#2D9E8F':'var(--text-light)'}">${gradLetterOpen?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> 편지 쓰기 열림':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> 아직 열리지 않음'}</span>${canCtl?`<button class="btn btn-sm" style="background:${gradLetterOpen?'var(--coral-light)':'var(--lavender)'};color:${gradLetterOpen?'var(--coral)':'white'};padding:4px 12px;font-size:11px" onclick="toggleGradLetterOpen()">${gradLetterOpen?'닫기':'열기 · 알림 보내기'}</button>`:''}</div>`;const el=document.getElementById('grad-letter-students');if(!gradLetterOpen&&G.role!=='teacher'){if(el)el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><div class="empty-title" style="font-size:13px">아직 편지 쓰기가 열리지 않았어요</div></div>';openModal('grad-letter-modal');return;}if(el)el.innerHTML=list.length?list.map(u=>{const mine=letters.some(l=>l.studentId===u.id&&l.authorId===G.id);return `<div class="student-row" onclick="openGradLetterWrite('${u.id}')" style="cursor:pointer"><div class="student-avatar" style="background:linear-gradient(135deg,var(--primary),var(--lavender))">${u.name.charAt(0)}</div><div class="student-info"><div class="student-name">${u.name} ${u.baptism}${u.graduated?' <span class="chip chip-gray" style="font-size:9px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg> 졸업</span>':''}</div><div class="student-detail">${u.gradeLabel||''}${mine?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg> 내 편지 있음':''}</div></div><div style="font-size:16px;flex-shrink:0"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg></div></div>`;}).join(''):'<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg></div><div class="empty-title" style="font-size:13px">고3·졸업 학생이 없어요</div></div>';openModal('grad-letter-modal');}
function openGradLetterWrite(sid){currentGradLetterSid=sid;const u=pendingList.find(x=>x.id===sid);if(!u)return;const tt=document.getElementById('glw-title');if(tt)tt.textContent='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> '+u.name+' '+u.baptism+'에게';const list=letters.filter(l=>l.studentId===sid&&l.authorId===G.id);const cc=document.getElementById('glw-compose-card');if(cc)cc.style.display='';const sb=document.getElementById('glw-save-btn');if(sb)sb.style.display='';const el=document.getElementById('glw-list');if(el)el.innerHTML=list.length?list.map(l=>`<div class="card" style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><span style="font-size:12px;font-weight:700;color:var(--lavender)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> <span onclick="openProfileView('${l.authorId||''}')" style="cursor:pointer">${l.authorName}</span> <span class="chip ${l.authorRole==='teacher'?'chip-mint':'chip-blue'}" style="font-size:9px">${l.authorRole==='teacher'?'선생님':l.authorRole==='parent'?'학부모':'친구'}</span></span><span style="font-size:10px;color:var(--text-light)">${l.date||''}${l.authorId===G.id?' · 내 편지':''}</span></div><div style="font-size:12px;color:var(--text-sub);line-height:1.6;white-space:pre-wrap">${(l.text||'').replace(/</g,'&lt;')}</div>${l.authorId===G.id?`<div style="text-align:right;margin-top:6px"><button class="btn btn-sm" style="background:var(--coral-light);color:var(--coral);padding:4px 10px;font-size:11px" onclick="deleteGradLetter('${l.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg> 삭제</button></div>`:''}</div>`).join(''):'<div style="font-size:11px;color:var(--text-light);padding:2px 2px 10px">아직 내가 쓴 편지가 없어요. 첫 편지를 남겨보세요! (편지는 나만 볼 수 있어요)</div>';const ta=document.getElementById('glw-text');const mine=list.find(l=>l.authorId===G.id);if(ta)ta.value=mine?mine.text:'';openModal('grad-letter-write-modal');}
...
function doRegister(){
  ...
  notifications.unshift({pushed:false,id:'nt'+Date.now()+'rg',text:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> 새 가입 신청: <b>'+name+' '+baptism+'</b> ('+(user.role==='student'?'학생 · '+(user.gradeLabel||''):user.role==='parent'?'학부모':'교사 · '+(user.gradeLabel||''))+') · 승인이 필요해요.',time:'방금',ts:Date.now(),readBy:[],forTeacher:true,tap:{type:'pending'}});updateNotifDot();try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function startSession(u){
  ...
  else if(isP){const cs=(u.children||[]).map(c=>c.name||c).join(', ');G.displayName=G.name+' '+G.baptism+(cs?'('+cs+')':'')+' 학부모';document.getElementById('role-badge').textContent=G.isJabumoPresident?'학부모·자부모회장':'학부모';const phn=document.getElementById('parent-home-name');if(phn){phn.textContent=G.displayName+'님';phn.innerHTML=phn.innerHTML+'<br>안녕하세요 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 21s7-4.3 7-10V6l-7-3-7 3v5c0 5.7 7 10 7 10z"/></svg>';}filterBoardParent();renderParentChildCards(u.children||[]);}
...
function paintTeacherHome(){
  try{
    ...
    if(thn)thn.innerHTML='<span style="min-width:0">'+_esc(G.displayName+(G.grade?' T('+G.grade+')':' T'))+'</span>';
...
function setMyProfile(){try{updateVerseEditUI();}catch(e){}const AVT={student:'😊',parent:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>‍<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>‍<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',teacher:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg>'};const POS={m1:'중1 담당',m2:'중2 담당',m3:'중3 담당',h:'고등 담당',principal:'교감',admin:'교무',etc:'기타 교사'};const roleLabel=G.role==='student'?(G.graduated?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg> 졸업생':(G.grade||{m1:'중1',m2:'중2',m3:'중3',h:'고등부'}[G.gradeKey]||'')):G.role==='parent'?'':(POS[G.type]||'교사')+(G.isAdmin?' · 관리자':'');var _ma=document.getElementById('my-avatar');if(_ma){if(G.avatar){_ma.style.backgroundImage='url('+G.avatar+')';_ma.style.backgroundSize='cover';_ma.style.backgroundPosition='center';_ma.innerHTML='';}else{_ma.style.backgroundImage='';_ma.innerHTML='<svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.92)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" style="display:block;margin:auto"><circle cx="12" cy="9" r="3.4"/><path d="M4.5 20a7.5 7.5 0 0 1 15 0"/></svg>';}}var _ms=document.getElementById('my-status');if(_ms)_ms.textContent=G.statusMsg||'';document.getElementById('my-name').textContent=G.displayName;document.getElementById('my-role').textContent=roleLabel;show('my-role',G.role!=='parent');show('menu-position',G.role==='teacher');const badgeEl=document.getElementById('my-name-badge');if(G.isJabumoPresident){if(badgeEl){badgeEl.textContent='자부모회장';badgeEl.style.color='var(--yellow)';}show('my-name-badge',true);}else if(G.role==='student'&&crownCount(G)>0){if(badgeEl){const c=crownCount(G);badgeEl.textContent='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>'.repeat(Math.min(c,5))+' 명예 열매';badgeEl.style.color='var(--yellow)';}show('my-name-badge',true);}else{show('my-name-badge',false);}const mgh=document.getElementById('my-grade-hint');if(mgh)mgh.textContent=G.role==='student'?'학년·이름·세례명 변경은 교사 승인 필요':'이름·세례명 변경은 승인 필요';try{_fillHomeAvatars();}catch(e){}}
/* ── 본인 비밀번호 변경 ── */
let _pwForce=false;
...
function initStamps(){const t=G.attendTotal||0;var _nx=null;for(const L of ATTEND_LEVELS){if(t<L.n){_nx=L;break;}}var _pv=0;ATTEND_LEVELS.forEach(L=>{if(t>=L.n)_pv=L.n;});var _vacS=function(w){return isVacationDate(w)||isEduVacation(w);};
  ...
  const _pgWrap=document.getElementById('stamp-pager');if(_pgWrap)_pgWrap.style.display=(_pages>1)?'flex':'none';const _pgTxt=document.getElementById('stamp-page-txt');if(_pgTxt)_pgTxt.textContent=_pg+'번째 판';const pf=document.getElementById('stamp-fill');if(pf)pf.style.width=(_nx?Math.min(100,Math.max(0,Math.round((t-_pv)/((_nx.n-_pv)||1)*100))):100)+'%';const pt=document.getElementById('stamp-prog-txt');if(pt)pt.textContent=(_nx?(t+'/'+_nx.n+'회'):'최고 등급 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>');const st=document.getElementById('stamp-total');if(st)countUp(st,t);const sm=document.getElementById('stamp-month');if(sm)sm.textContent=monthAttendCount(G);const ss=document.getElementById('stamp-streak');if(ss)ss.textContent=(G.streak||0);try{var _re=document.getElementById('stamp-rank');if(_re){if(G.role==='student'){var _st=(pendingList||[]).filter(function(u){return u&&u.approved&&u.role==='student'&&!u.hidden&&!u.graduated;});_st.sort(function(a,b){return (b.attendTotal||0)-(a.attendTotal||0)||(b.streak||0)-(a.streak||0)||((a.name||'')>(b.name||'')?1:-1);});var _mi=-1;for(var _k=0;_k<_st.length;_k++){if(_st[_k].id===G.id){_mi=_k;break;}}var _md=['<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="15" r="6"/><path d="M8.2 10.2 5 2h5l2 4 2-4h5l-3.2 8.2"/></svg>','<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="15" r="6"/><path d="M8.2 10.2 5 2h5l2 4 2-4h5l-3.2 8.2"/></svg>','<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="15" r="6"/><path d="M8.2 10.2 5 2h5l2 4 2-4h5l-3.2 8.2"/></svg>'];_re.textContent=(_mi>=0&&_mi<3&&(G.attendTotal||0)>0)?(_md[_mi]+' 나의 순위 '+(_mi+1)+'위'):'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/></svg> 나의 순위 보기';_re.style.display='';}else{_re.style.display='none';}}}catch(e){}const mt=document.getElementById('my-total');if(mt)mt.textContent=t;let lv=(t<1)?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg> 시작 전':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg> 씨앗';for(const L of ATTEND_LEVELS)if(t>=L.n)lv=L.l;const sl=document.getElementById('stamp-level');if(sl)sl.textContent=lv;const bi=document.getElementById('my-badge-icon');if(bi){var _bs=(t<1)?-1:0;for(var _bj=0;_bj<ATTEND_LEVELS.length;_bj++)if(t>=ATTEND_LEVELS[_bj].n)_bs=_bj;bi.innerHTML=_treeSVG(_bs,32);}ATTEND_LEVELS.forEach(L=>{const tier=document.getElementById('reward-tier-'+L.n);const icon=document.getElementById('reward-icon-'+L.n);const badge=document.getElementById('reward-badge-'+L.n);const achieved=t>=L.n;if(tier)tier.style.opacity=achieved?'1':'.6';if(icon)icon.style.filter=achieved?'none':'grayscale(1)';if(badge){if(achieved){badge.style.background='var(--yellow-light)';badge.style.color='#9A7200';badge.textContent='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 달성!';}else{badge.style.background='var(--bg)';badge.style.color='var(--text-light)';badge.textContent=L.n+'회';}}});try{renderStudentGrowth();}catch(e){}}
function _treeSVG(stage,size){
  var w=Math.round(size*0.86),h=size;var soil='<ellipse cx="33" cy="73" rx="21" ry="5" fill="#E7DCC9"/>';
  var body;
  if(stage<0){ /* 아직 심기 전: 땅(흙)만 */
    body='<ellipse cx="33" cy="70" rx="20" ry="5.5" fill="#EDE6D8"/><path d="M13 70 q20 -10 40 0z" fill="#D8C4A3"/><path d="M22 67 q2 -2 4 0 M34 66 q2 -2 4 0 M44 68 q2 -2 3 0" stroke="#C2AA84" stroke-width="1.4" fill="none" stroke-linecap="round"/>';
    soil='';
  }else if(stage===0){ /* 씨앗 */
    body='<path d="M15 70 q18 -9 36 0z" fill="#C9A87E"/><ellipse cx="33" cy="63" rx="5" ry="6.5" fill="#8B5E3C"/><path d="M33 58 q2 3 0 7" stroke="#6E4327" stroke-width="1.2" fill="none"/>';
    soil='<ellipse cx="33" cy="70" rx="19" ry="5" fill="#E7DCC9"/>';
  }else if(stage===1){ /* 새싹 */
    body='<path d="M15 70 q18 -9 36 0z" fill="#C9A87E"/><path d="M33 70 q-1 -16 0 -22" stroke="#6FA05A" stroke-width="2.6" fill="none" stroke-linecap="round"/><path d="M33 52 q-11 -2 -15 -12 q11 0 15 8z" fill="#8FD09A"/><path d="M33 49 q11 -3 15 -12 q-11 -1 -15 8z" fill="#A9E0AE"/>';
    soil='<ellipse cx="33" cy="70" rx="19" ry="5" fill="#E7DCC9"/>';
  }else if(stage===2){ /* 어린나무(묘목) */
    body='<path d="M33 72 v-30" stroke="#9A6A4A" stroke-width="3" fill="none" stroke-linecap="round"/><path d="M33 56 q-9 -3 -13 -10" stroke="#9A6A4A" stroke-width="2" fill="none" stroke-linecap="round"/><path d="M33 50 q9 -3 13 -9" stroke="#9A6A4A" stroke-width="2" fill="none" stroke-linecap="round"/><ellipse cx="20" cy="44" rx="7" ry="9" fill="#7FC793" transform="rotate(-30 20 44)"/><ellipse cx="47" cy="41" rx="7" ry="9" fill="#93D6A2" transform="rotate(28 47 41)"/><ellipse cx="33" cy="34" rx="8" ry="11" fill="#8FD09A"/>';
    soil='<ellipse cx="33" cy="72" rx="19" ry="5" fill="#E7DCC9"/>';
  }else{ /* 큰나무 이상 */
    body='<path d="M33 73 v-30" stroke="#8B5E3C" stroke-width="6" fill="none" stroke-linecap="round"/><path d="M33 55 q-10 -2 -15 -9 M33 50 q11 -2 16 -10" stroke="#8B5E3C" stroke-width="3.4" fill="none" stroke-linecap="round"/><circle cx="33" cy="30" r="15" fill="#5FB374"/><circle cx="19" cy="38" r="10" fill="#7FC793"/><circle cx="47" cy="37" r="11" fill="#72C489"/><circle cx="34" cy="20" r="10" fill="#93D6A2"/>';
    if(stage===4){ /* 꽃 */
      body+='<g><circle cx="22" cy="26" r="2.1" fill="#FBD3E0"/><circle cx="24.2" cy="26" r="2.1" fill="#FBD3E0"/><circle cx="23.1" cy="28.1" r="2.1" fill="#FBD3E0"/><circle cx="23.1" cy="26.7" r="1.3" fill="#F5A9C4"/></g><g><circle cx="42" cy="32" r="2.1" fill="#FBD3E0"/><circle cx="44.2" cy="32" r="2.1" fill="#FBD3E0"/><circle cx="43.1" cy="34.1" r="2.1" fill="#FBD3E0"/><circle cx="43.1" cy="32.7" r="1.3" fill="#F5A9C4"/></g><g><circle cx="33" cy="17" r="2.1" fill="#FDE1EC"/><circle cx="35.2" cy="17" r="2.1" fill="#FDE1EC"/><circle cx="34.1" cy="19.1" r="2.1" fill="#FDE1EC"/><circle cx="34.1" cy="17.7" r="1.3" fill="#F5A9C4"/></g>';
    }else if(stage>=5){ /* 열매 */
      body+='<circle cx="23" cy="30" r="3.4" fill="#E8574C"/><circle cx="21.9" cy="28.8" r="1" fill="#fff" opacity=".6"/><circle cx="44" cy="34" r="3.4" fill="#E8574C"/><circle cx="42.9" cy="32.8" r="1" fill="#fff" opacity=".6"/><circle cx="34" cy="22" r="3.4" fill="#F06A5E"/><circle cx="32.9" cy="20.8" r="1" fill="#fff" opacity=".6"/>';
    }
    soil='<ellipse cx="33" cy="73" rx="21" ry="5" fill="#E7DCC9"/>';
  }
...
function renderStudentGrowth(){try{
  ...
  var gsub=document.getElementById('student-greet-sub');if(gsub)gsub.textContent=(appConfig&&appConfig.title)||'';
  var ggr=document.getElementById('student-greet-grade');if(ggr){ggr.textContent=G.grade||'';ggr.style.display=G.grade?'':'none';}
  ...
  if(nextEl)nextEl.innerHTML=nx?('다음 성장까지 <strong style="color:var(--primary-dark);font-weight:800">'+(nx.n-t)+'회</strong>!'):'최고 등급(열매) 달성! <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>';
  ...
  if(ag){var curLv=null;ATTEND_LEVELS.forEach(function(L){if(t>=L.n)curLv=L;});var cLabel=curLv?curLv.l:'🌱 시작 전';var mth=(typeof monthAttendCount==='function')?monthAttendCount(G):0;
    ag.innerHTML='<div class="card" style="padding:14px;text-align:left;background:linear-gradient(120deg,var(--mint-light),var(--primary-light))">'
      +'<div style="display:flex;align-items:center;gap:14px">'
        ...
        +'<div style="font-size:11px;color:var(--text-sub);margin-bottom:6px">'+(nx?('다음 등급까지 <b style="color:var(--primary-dark)">'+(nx.n-t)+'회</b>'):'최고 등급(열매) 달성 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>')+'</div>'
      ...
      +'<div style="display:flex;background:rgba(255,255,255,.72);border-radius:12px;margin-top:12px;text-align:center;padding:10px 0">'
        ...
        +'<div style="flex:1;border-left:1px solid var(--border-light);border-right:1px solid var(--border-light)"><div style="font-size:10px;color:var(--text-light);margin-bottom:2px">연속</div><div style="font-size:16px;font-weight:800;color:var(--gold)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 2s5 4 5 9a5 5 0 0 1-10 0c0-1.5.5-2.5 1-3 .5 1 1.5 1.5 2 1.5C9 7 12 5 12 2z"/></svg>'+(G.streak||0)+'주</div></div>'
      ...
      +'</div>'
      +'<div style="font-size:11.5px;color:var(--text-sub);margin-top:11px;text-align:center">'+(nx?('다음 등급까지 <b style="color:var(--primary-dark)">'+(nx.n-t)+'회</b>'):'최고 등급 달성 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg>')+'</div>'
    +'</div>';}
}catch(e){}}
...
function checkBirthdayCoupons(){dedupeBdayCoupons();const now=new Date();const m=now.getMonth()+1,d=now.getDate(),yr=now.getFullYear();let issued=false;pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated&&+u.birthMonth===m&&+u.birthDay===d).forEach(u=>{const key=u.id+'-'+yr;const cid='cpbday-'+key;if(coupons.some(c=>c.id===cid||c.bdayKey===key))return;issued=true;const reward=appConfig.bdayReward||BDAY_REWARD_DEFAULT;const code=_codeFromKey(key);coupons.push({id:cid,studentId:u.id,studentName:u.name+' '+u.baptism,badgeLabel:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg> 생일',reward,code,used:false,createdAt:now.toLocaleDateString('ko-KR'),bdayKey:key});notifications.unshift({pushed:false,id:'nt'+Date.now()+'bt'+u.id,text:`<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg> 오늘은 ${u.name} ${u.baptism} 학생의 생일! 생일 쿠폰이 발급되었어요. 인증번호: ${code}`,time:'방금',ts:Date.now(),readBy:[],forTeacher:true,tap:{type:'coupon-admin'}});notifications.unshift({pushed:false,id:'nt'+Date.now()+'bs'+u.id,text:`<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg> 생일 축하해요! <b>${reward}</b> 쿠폰이 도착했어요. 선생님께 확인받고 사용하세요!`,time:'방금',ts:Date.now(),readBy:[],forStudentId:u.id,tap:{type:'coupon'}});});if(issued){updateNotifDot();if(typeof renderAdminCouponList==='function')renderAdminCouponList();}}
...
function renderBdayComments(){const wrap=document.getElementById('bday-comments-wrap');if(!wrap)return;const list=birthdayComments.filter(c=>c.targetId===bdayTargetId);try{const _tg=document.getElementById('bday-msg-toggle');if(_tg&&_tg.style.display!=='none'&&!bdayMsgsOpen)_tg.textContent=list.length?('<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> 축하 메시지 '+list.length+'개 모두 보기'):'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg> 아직 도착한 메시지가 없어요';}catch(e){}if(!list.length){wrap.innerHTML='<div style="font-size:13px;color:rgba(255,255,255,0.8);text-align:center">아직 축하 메시지가 없어요</div>';return;}wrap.innerHTML=list.map(c=>`<div style="margin-bottom:8px"><span style="color:white;font-weight:700;font-size:12px">${c.author}</span> <span style="color:rgba(255,255,255,0.9);font-size:12px">${c.text}</span></div>`).join('');}
...
function renderBdayBanner(list){const section=document.getElementById('bday-banner-section');const slides=document.getElementById('bday-slides');const dots=document.getElementById('bday-dots');if(!section||!slides)return;const m=new Date().getMonth()+1,d=new Date().getDate();slides.innerHTML=list.map(u=>{const isBirth=+u.birthMonth===m&&+u.birthDay===d;return `<div class="bday-slide bday-slide-other" onclick="openBdayFromBanner('${u.id}')" style="color:white;display:flex;align-items:center;gap:14px;min-width:100%;padding:16px 18px;cursor:pointer"><div style="font-size:36px">${isBirth?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg>':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg>'}</div><div style="flex:1"><div style="font-size:11px;opacity:.8;margin-bottom:2px">${isBirth?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg> 생일':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg> 축일'}</div><div style="font-size:16px;font-weight:800">${u.name} ${u.baptism}</div><div style="font-size:12px;opacity:.85;margin-top:4px">오늘의 주인공을 축하해요!</div></div></div>`;}).join('');if(dots)dots.innerHTML=list.map((_,i)=>`<div class="bday-dot${i===0?' active':''}" onclick="goBdaySlide(${i})" style="cursor:pointer;padding:4px"></div>`).join('');section.style.display='block';bdaySlideIdx=0;bdaySlideTotal=list.length;const sl=document.getElementById('bday-slides');sl.style.transform='translateX(0)';if(list.length>1)initBdaySwipe(sl);}
...
function _renderJabumoLocked(){var el=document.getElementById('post-list');if(!el)return;var pres=pendingList.find(function(u){return u.isJabumoPresident;});var me=pendingList.find(function(u){return u.id===G.id;});var requested=me&&me.jabumoRequested;el.innerHTML='<div class="card" style="text-align:center;padding:30px 20px"><div style="font-size:42px;margin-bottom:12px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><div style="font-size:15px;font-weight:800;margin-bottom:8px">자부모회 회원 전용</div><div style="font-size:12.5px;color:var(--text-light);line-height:1.7;margin-bottom:20px">자부모회 게시판은 회원만 열람할 수 있어요.<br>'+(pres?('현 회장: <b>'+pres.name+' '+pres.baptism+'</b> 님'):'아직 자부모회장이 없어요')+'</div>'+(requested?'<div style="font-size:13px;font-weight:700;color:var(--primary-dark);background:var(--primary-light);border-radius:12px;padding:14px;line-height:1.6"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 가입 신청 완료<br><span style="font-size:11px;font-weight:500;color:var(--text-light)">회장님의 승인을 기다리고 있어요</span></div>':'<button class="btn btn-primary" style="width:100%" onclick="applyJabumo()">자부모회 가입 신청하기</button>')+'</div>';}
...
function renderArchivedEvents(){
  ...
  el.innerHTML=list.length?list.map(function(e){
    return '<div class="resource-card" style="align-items:center">'
      ...
      +'</div>';
  }).join(''):'<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.3 7 12 12 20.7 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg></div><div class="empty-title" style="font-size:13px">보관된 이벤트가 없어요</div></div>';
}
function buildEventCardHtml(e){
  ...
  const play=yid?'<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none"><div style="width:52px;height:52px;border-radius:50%;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center"><span style="color:#fff;font-size:22px;margin-left:3px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polygon points="6 4 20 12 6 20 6 4"/></svg></span></div></div>':'';
  if(_ec){
    if(yid){
      const capBtn=(G&&G.role==='teacher')?`<span onclick="event.stopPropagation();archiveEvent('${e.id}')" style="margin-left:8px;font-size:11px;color:var(--coral);cursor:pointer;flex-shrink:0"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.3 7 12 12 20.7 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg> 내리기</span>`:'';
      return `<div class="event-slide" ${clickAttr} style="${dim}"><div class="event-slide-img yt" style="position:relative"><img src="${_ec}">${play}${badge}</div><div class="event-yt-cap" style="display:flex;align-items:center"><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polygon points="6 4 20 12 6 20 6 4"/></svg> ${e.title}</span>${capBtn}</div></div>`;
    }
  ...
  try{
    ...
    if(box){
      ...
      if(isT&&ev&&ev.archived){
        box.innerHTML='<button class="btn btn-sm btn-outline" style="width:100%" onclick="unarchiveEvent(\''+id+'\')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg> 다시 올리기</button>';
      }else if(isT&&ev){
        box.innerHTML='<button class="btn btn-sm btn-outline" style="width:100%" onclick="archiveEvent(\''+id+'\')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.3 7 12 12 20.7 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg> 지난 이벤트로 내리기'+(ended?'':' <span style="font-weight:400;opacity:.7">(진행 중)</span>')+'</button>';
      }else box.innerHTML='';
    }
  }catch(e){}
...
async function _renderPdf(dataUrl,el){
  el.innerHTML='<div style="text-align:center;padding:40px 0;color:var(--text-light);font-size:13px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg> 문서를 여는 중…</div>';
  try{
    await loadPdfJs();
...
function renderStoryComments(scrollBottom){
  ...
  if(!comments.length){el.innerHTML='<div style="text-align:center;padding:30px 0;color:rgba(255,255,255,.5);font-size:13px">아직 댓글이 없어요<br>첫 댓글을 남겨보세요 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.4 8.4 0 0 1-3.8-.9L3 21l2-5.2A8.4 8.4 0 0 1 12 3h.5a8.4 8.4 0 0 1 8 8z"/></svg></div>';return;}
...
function renderPostComments(){const{img}=getCurImg(currentCommentPostId);const el=document.getElementById('post-comment-list');if(!el)return;const list=(img&&img.comments)||[];if(!list.length){el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.4 8.4 0 0 1-3.8-.9L3 21l2-5.2A8.4 8.4 0 0 1 12 3h.5a8.4 8.4 0 0 1 8 8z"/></svg></div><div class="empty-title" style="font-size:13px">첫 댓글을 남겨보세요</div></div>';return;}el.innerHTML=list.map(c=>`<div style="display:flex;gap:10px"><div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--lavender));display:flex;align-items:center;justify-content:center;color:white;font-size:11px;font-weight:800;flex-shrink:0">${(c.author||'?').charAt(0)}</div><div style="flex:1"><div style="font-size:12px"><strong onclick="openProfileView('${c.authorId||''}')" style="cursor:pointer">${c.author}</strong> <span style="color:var(--text-sub)">${c.text}</span></div><div style="font-size:10px;color:var(--text-light);margin-top:2px">${_timeAgo(c.ts,c.time)}</div></div></div>`).join('');}
...
function openPostDetail(id){const p=posts.find(x=>x.id===id);if(!p)return;currentPostId=id;document.getElementById('pd-title').textContent=p.title;document.getElementById('pd-meta').innerHTML=(CAT_LABEL[p.cat]||p.cat)+' · <span onclick="openProfileView(\''+(p.authorId||'')+'\')" style="cursor:pointer;color:var(--primary-dark);font-weight:700">'+_esc(p.authorName||'익명')+'</span> · '+p.date+(p.edited?' (수정됨)':'');var _pdc=document.getElementById('pd-content');if(_pdc){if(_isWeeklyLike(p)){_pdc.innerHTML='';try{_renderNoticeCards(p.content||'',_pdc);}catch(e){_pdc.textContent=p.content||'';}}else{_pdc.textContent=p.content||'';}}document.getElementById('pd-imgs').innerHTML=postImgsHTML(p);document.getElementById('pd-attach').innerHTML=postAttachHTML(p);
renderPdComments(p);
...
function openSchedList(){var el=document.getElementById('sched-list-body');if(el)el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-title" style="font-size:13px">불러오는 중…</div></div>';openModal('sched-list-modal');if(!(window.FB&&FB.enabled()&&FB.load)){_renderSchedList([]);return;}var done=false;FB.load('scheduledPosts').then(function(arr){done=true;_renderSchedList(arr||[]);}).catch(function(){done=true;_renderSchedList([]);});setTimeout(function(){if(!done){var e2=document.getElementById('sched-list-body');if(e2&&/불러오는 중/.test(e2.innerHTML))_renderSchedList([]);}},4000);}
function _renderSchedList(arr){var el=document.getElementById('sched-list-body');if(!el)return;var isFull=(G.type==='principal'||G.type==='admin'||G.isAdmin);var mine=arr.filter(function(s){return s&&!s.published&&(isFull||s.authorId===G.id);}).sort(function(a,b){return (_kstMs(a.schedDate,a.schedTime)||0)-(_kstMs(b.schedDate,b.schedTime)||0);});if(!mine.length){el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:26px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.5 5.5 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.5-6.5A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.74 1.5z"/></svg></div><div class="empty-title" style="font-size:13px">예약된 공지가 없어요</div></div>';return;}el.innerHTML=mine.map(function(s){var due=_kstMs(s.schedDate,s.schedTime);var over=due&&due<=Date.now();var kindLbl=s.kind==='weekly'?'주간공지':'공지';var ttl=(s.post&&s.post.title)||'(제목 없음)';return '<div class="card" style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div style="min-width:0"><div style="font-size:13px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+_esc(ttl)+'</div><div style="font-size:11px;color:var(--text-light);margin-top:3px"><span class="chip chip-blue" style="font-size:9px">'+kindLbl+'</span> <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> '+s.schedDate+' '+s.schedTime+(over?' · <span style="color:var(--coral)">발송 대기중</span>':'')+'</div><div style="font-size:10px;color:var(--text-light);margin-top:2px">'+_esc(s.authorName||'')+'</div></div><button class="btn btn-sm" style="background:var(--coral-light);color:#D95F50;flex-shrink:0" onclick="cancelSchedPost(\''+s.id+'\')">취소</button></div></div>';}).join('');}
function cancelSchedPost(id){if(!confirm('이 예약을 취소할까요?'))return;try{if(window.FB&&FB.enabled()&&FB.remove)FB.remove('scheduledPosts',id);}catch(e){}try{if(appConfig.wnScheduled){delete appConfig.wnScheduled;if(window.flushCfg)window.flushCfg();}}catch(e){}showToast('예약을 취소했어요');setTimeout(openSchedList,300);}
...
function _deptTabSync(dept){try{var f=deptSubFilter[dept];var _tw=document.getElementById('dept-'+dept);if(_tw){var _tabs=_tw.querySelectorAll('.tab-bar .tab-btn');_tabs.forEach(function(b){var oc=b.getAttribute('onclick')||'';b.classList.toggle('active',oc.indexOf("'"+dept+"','"+f+"'")>=0);});}}catch(e){}}
function renderDeptPosts(dept,sub){if(sub!==undefined)deptSubFilter[dept]=sub;const f=deptSubFilter[dept];_deptTabSync(dept);try{requestAnimationFrame(function(){_deptTabSync(dept);});}catch(e){}const el=document.getElementById(dept+'-posts');if(!el)return;const arr=posts.filter(p=>p.cat==='activity'&&p.dept===dept&&(f==='all'||p.subcat===f));var _ico=dept==='choir'?'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="18" r="2.6"/><circle cx="17" cy="16" r="2.6"/><path d="M8.6 18V5.2l11-2v12.6"/><path d="M8.6 8.6l11-2"/></svg>':'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="M7 8h10"/></svg>';if(!arr.length){el.innerHTML='<div class="empty"><div class="empty-emoji">'+_ico+'</div><div class="empty-title">아직 게시물이 없어요</div></div>';try{_deptFit(dept);}catch(e){}return;}const canAdmin=G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);el.innerHTML=arr.map(p=>{const isOwner=p.authorId===G.id;const moreBtn=(isOwner||canAdmin)?`<button class="post-more-btn" onclick="event.stopPropagation();openPostActions('${p.id}',${isOwner},true)">⋯</button>`:'';const subChip=`<span class="chip chip-blue">${p.subcat==='notice'?'공지':dept==='choir'?'악보/자료':'전례자료'}</span>`;return `<div class="post-card" data-pid="${p.id}" style="cursor:pointer"><div class="post-head"><div class="post-tags">${subChip}</div>${moreBtn}</div><div class="post-title">${p.title}</div><div class="post-meta"><span class="post-meta-chip">${p.authorName||'익명'}</span><span>${p.date}</span>${p.edited?'<span class="post-edited">(수정됨)</span>':''}</div>${p.image?`<img src="${p.image}" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);margin:8px 0;background:var(--bg)">`:''}${p.content?`<div class="post-preview">${p.content.slice(0,60)}${p.content.length>60?'...':''}</div>`:''}<div class="post-footer"><span class="post-stat"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.4 8.4 0 0 1-3.8-.9L3 21l2-5.2A8.4 8.4 0 0 1 12 3h.5a8.4 8.4 0 0 1 8 8z"/></svg> ${(p.comments||[]).length}</span>${p.docs&&p.docs.length?`<span class="post-stat"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg> ${p.docs.length}</span>`:''}</div></div>`;}).join('');try{_deptFit(dept);}catch(e){}}
...
function openResourceDetail(id){const r=resources.find(r=>r.id===id);if(!r)return;currentResourceId=id;const canEdit=r.authorId===G.id||G.type==='principal'||G.type==='admin'||G.isAdmin;show('rd-edit-btn',canEdit);show('rd-delete-btn',canEdit);document.getElementById('rd-title').textContent=r.title;document.getElementById('rd-meta').textContent=RESOURCE_CAT_LABEL[r.cat]+' · '+r.authorName+' · '+r.date;document.getElementById('rd-content').textContent=r.content||'';var _lk=document.getElementById('rd-link');if(_lk)_lk.innerHTML=r.link?('<a href="'+_esc(r.link)+'" class="btn btn-sm btn-primary" style="width:100%;text-decoration:none;display:flex;align-items:center;justify-content:center">'+_linkIcon(r.link)+' 문서 열기 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg></a>'):'';document.getElementById('rd-images').innerHTML=(r.images||[]).map((im,ix)=>`<img src="${_srcOf(im)}" style="width:100%;border-radius:var(--radius-sm)">${_imgActions('Res',r.id,ix,(r.title||'image'))}`).join('');document.getElementById('rd-docs').innerHTML=(r.docs||[]).map((d,ix)=>`<div style="background:var(--bg);border-radius:var(--radius-sm);padding:12px 14px"><div style="display:flex;align-items:center;gap:8px;font-size:13px"><span>${_docIcon(d)}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600">${d.name}</span>${_docSizeLabel(d)?`<span style="color:var(--text-light);font-size:10px">${_docSizeLabel(d)}</span>`:''}</div>${_docBtns('Res',r.id,ix,d)}</div>`).join('');openModal('resource-detail-modal');}
...
var _cloudBkT=0;
function _cloudBackupMinutes(force){try{
  if(!(window.FB&&FB.enabled()&&FB.save))return;
  var now=Date.now();if(!force&&now-_cloudBkT<120000)return; /* 2분 스로틀 (같은 날은 한 문서에 덮어쓰기) */
  var mins=(resources||[]).filter(function(r){return r&&r.cat==='minutes'&&!r.deleted&&((r.content||'').trim()||(r.agendaText||'').trim());}).map(function(r){return {id:r.id,mdate:r.mdate,title:r.title,content:r.content||'',agendaText:r.agendaText||'',published:!!r.published,year:r.year||''};});
  if(!mins.length)return;_cloudBkT=now;
  var d=new Date();var day=d.getFullYear()+'-'+('0'+(d.getMonth()+1)).slice(-2)+'-'+('0'+d.getDate()).slice(-2);var id='bk'+day;
  try{FB.save('minuteBackups',id,{id:id,date:day,ts:now,by:(G&&G.displayName)||'',count:mins.length,minutes:mins});}catch(e){}
  if(FB.load){FB.load('minuteBackups').then(function(arr){try{(arr||[]).sort(function(a,b){return (b.ts||0)-(a.ts||0);}).slice(14).forEach(function(x){if(x&&x.id&&FB.remove)FB.remove('minuteBackups',x.id);});}catch(e){}}).catch(function(){});}
}catch(e){}}
function openMinutesBackups(){var el=document.getElementById('min-bk-body');if(el)el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-title" style="font-size:13px">불러오는 중…</div></div>';openModal('minutes-backups-modal');
  if(!(window.FB&&FB.enabled()&&FB.load)){if(el)el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-title" style="font-size:13px">클라우드 연결이 필요해요</div></div>';return;}
  FB.load('minuteBackups').then(function(arr){arr=(arr||[]).sort(function(a,b){return (b.ts||0)-(a.ts||0);});if(!arr.length){if(el)el.innerHTML='<div class="empty" style="padding:24px;text-align:center"><svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#9AA6BC" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="margin:0 auto 6px"><path d="M20 17.6A4.5 4.5 0 0 0 18 9h-1.3A7 7 0 1 0 5 15.7"/></svg><div class="empty-title" style="font-size:13px">아직 클라우드 백업이 없어요</div></div>';return;}
    if(el)el.innerHTML=arr.map(function(b){return '<div class="card" style="margin-bottom:8px;display:flex;align-items:center;gap:8px"><div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700">'+_esc(b.date||'')+' 백업</div><div style="font-size:11px;color:var(--text-light);margin-top:2px">회의록 '+(b.count||(b.minutes?b.minutes.length:0))+'건'+(b.by?' · '+_esc(b.by):'')+'</div></div><button class="btn btn-sm btn-primary" onclick="restoreCloudBackup(\''+b.id+'\')">복원</button></div>';}).join('');
...
function _minCanDeleteAny(){return _isFullAdmin()||(resources||[]).some(function(r){return r.cat==='minutes'&&!r.deleted&&(r.authorId===G.id||r.createdById===G.id);});}
function minSelStart(id){
  if(!(G.role==='teacher'))return;
  if(!_minCanDeleteAny()){showToast('삭제 권한이 있는 회의록이 없어요');return;}   /* 삭제 가능 사용자에게만 다중선택 노출 */
  if(!_mSel){_mSel={};if(window._navPush)window._navPush('min-sel');}   /* 진입 시 히스토리 1개 → 하드웨어 뒤로가기로 취소 */
  if(id)_mSel[id]=1;
  renderMinutesHub();
}
function _minSelExit(){if(!_mSel)return;try{history.back();}catch(e){minSelCancel();}}
window._modalBack=function(id){if(id==='minutes-hub-modal'&&_mSel){minSelCancel();return true;}return false;};
function minSelToggle(id){
  if(!_mSel)return;
  if(_mSel[id])delete _mSel[id];else _mSel[id]=1;
  renderMinutesHub();
}
/* 히스토리 조작 없이 선택 모드만 해제(app-sync 뒤로가기 시스템과 충돌 방지) */
function minSelCancel(){if(!_mSel)return;_mSel=null;renderMinutesHub();}
function minSelAll(){
...
function minSelDelete(){
  ...
  appConfirm({icon:'trash',title:mine.length+'개를 휴지통으로 옮길까요?',desc:'휴지통에서 다시 복구할 수 있어요.'+(mine.length<ids.length?('\n권한이 없는 '+(ids.length-mine.length)+'개는 제외돼요.'):''),okText:'옮기기'}).then(function(ok){
    ...
    try{if(typeof flushSync==='function')flushSync();}catch(e){}
    renderResourceList();_minSelExit();
...
function renderMinutesHub(){
  ...
  var html='';
  if(!live&&list.length)html+='<div style="background:var(--bg);border-radius:var(--radius-sm);padding:9px 11px;margin-bottom:9px;font-size:11px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> '+minutesHubYear+'년 회의록은 보관되어 읽기 전용이에요.</div>';
  if(list.length)html+='<div style="display:flex;justify-content:flex-end;margin-bottom:8px"><button class="btn btn-sm btn-outline" style="width:auto" onclick="exportMinutesYear()"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/></svg> '+minutesHubYear+'년 전체 PDF</button></div>';
  var _n=_mSel?Object.keys(_mSel).length:0;
  html+=list.length?(function(){var _td=_today();var _dd=new Date();var _tdl=(_dd.getMonth()+1)+'월 '+_dd.getDate()+'일';var _shownLine=false;var out=list.map(function(r){var pre='';if(live&&!_shownLine&&(r.mdate||'')>=_td){_shownLine=true;pre='<div style="display:flex;align-items:center;gap:9px;margin:16px 2px 12px"><div style="flex:1;height:2px;background:var(--primary);border-radius:2px;opacity:.5"></div><span style="font-size:11px;font-weight:800;color:var(--primary-dark);white-space:nowrap">오늘 · '+_tdl+' ▾ 예정</span><div style="flex:1;height:2px;background:var(--primary);border-radius:2px;opacity:.5"></div></div>';}return pre+_resCard(r);}).join('');return out;})():_resEmpty('<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h6"/><path d="M14 2v6h6"/><path d="M18.4 13.6a2 2 0 0 1 3 3L16 22l-4 1 1-4z"/></svg>','회의록이 없어요',live?'＋ 새 회의록을 눌러 바로 시작해요':'이 해에는 작성된 회의록이 없어요');
...
function publishMinutes(){
  ...
  try{if(typeof flushSync==='function')flushSync();}catch(e){}try{_backupMinutes();}catch(e){}try{_cloudBackupMinutes();}catch(e){}
...
function renderMinutesAck(){
  ...
  if(!r.published){
    el.innerHTML=_isArchivedMinutes(r)?'':'<div style="font-size:11px;color:var(--text-light);text-align:center;padding:8px">아직 발행 전이에요 · 다 쓰신 뒤 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg> 발행을 눌러주세요</div>';
    return;
  }
  ...
  if(!mine&&G.role==='teacher'){
    html+='<button class="btn btn-primary" style="width:100%" onclick="ackMinutes()"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 확인했습니다</button>';
  }else{
    html+='<div style="background:var(--mint-light);color:#2D9E8F;border-radius:var(--radius-sm);padding:9px;text-align:center;font-size:12px;font-weight:700"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 확인 완료</div>';
  }
  ...
  if(isOwner&&total){
    ...
    else html+='<div style="font-size:11px;color:var(--text-light);text-align:center;margin-top:4px">모두 확인했어요 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 3l1.9 5.6L19.5 10l-5.6 1.9L12 17.5l-1.9-5.6L4.5 10l5.6-1.4z"/></svg></div>';
  }
  el.innerHTML=html;
}
  ...
  if(al){
    ...
    al.innerHTML=un.length
      ? un.map(function(r){return '<div style="background:var(--coral);color:#fff;border-radius:var(--radius);padding:13px 15px;margin-bottom:9px;cursor:pointer" onclick="openMinutesViewer(\''+r.id+'\')">'
          +'<div style="font-size:13px;font-weight:800"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg> 확인하지 않은 회의록이 있어요</div>'
...
function embedCard(key,label,url,h){
  ...
  if(_embedOpen[key]){
    return `<div style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        <span><a href="${url}" target="_blank" rel="noopener" style="font-size:11px;color:var(--primary);font-weight:700;text-decoration:none;margin-right:10px">새 창에서 열기 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg></a><a href="javascript:void(0)" onclick="toggleEmbed('${key}')" style="font-size:11px;color:var(--text-light);font-weight:700;text-decoration:none">접기 ▴</a></span>
      </div>
  ...
  return `<div style="margin-bottom:14px;background:var(--bg);border:1px solid var(--border-light);border-radius:var(--radius);padding:14px">
    ...
    <div style="display:flex;gap:8px">
      <button class="btn btn-sm btn-primary" style="width:auto;flex:1" onclick="toggleEmbed('${key}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg> 앱에서 열기</button>
      <a href="${url}" target="_blank" rel="noopener" class="btn btn-sm btn-outline" style="width:auto;flex:1;text-decoration:none;display:flex;align-items:center;justify-content:center">새 창에서 열기 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/></svg></a>
    </div>
...
function _resCard(r){
  if(r.cat==='minutes'&&!r.deleted){
    ...
    var _mark=_selMode?`<span style="width:19px;height:19px;border-radius:50%;border:2px solid ${_on?'var(--primary)':'var(--border)'};background:${_on?'var(--primary)':'transparent'};color:#fff;font-size:11px;display:flex;align-items:center;justify-content:center;flex-shrink:0">${_on?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg>':''}</span>`:`<span title="${_tip}" style="width:7px;height:7px;border-radius:50%;background:${_dot};flex-shrink:0"></span>`;
  ...
  return `<div class="resource-card" onclick="openResourceDetail('${r.id}')"><div class="resource-info"><div class="resource-title">${r.title}</div><div class="resource-meta">${oc}${r.authorName} · ${r.date}${(r.docs&&r.docs.length)?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg> '+r.docs.length+'개':''}</div></div></div>`;
}
...
function renderResourceList(){
  ...
  if(tmp.length){
    html+='<div style="display:flex;align-items:baseline;gap:6px;margin:16px 0 8px">'
      +'<span style="font-size:12px;font-weight:800;color:var(--text-sub)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg> 임시 공유 · '+tmp.length+'개</span>'
...
function _minBlockHtml(b,editable,idx){
  ...
  else if(b.t==='callout')mk='<span class="mb-mk" contenteditable="false"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M9 18h6M10 22h4"/><path d="M12 2a7 7 0 0 0-4 12.7c.6.5 1 1.3 1 2.3h6c0-1 .4-1.8 1-2.3A7 7 0 0 0 12 2z"/></svg></span>';
...
function purgeAllMinutes(){if(!_isFullAdmin()){showToast('교감·교무·관리자만 비울 수 있어요');return;}var list=(resources||[]).filter(function(r){return r&&r.cat==='minutes'&&r.deleted;});if(!list.length){showToast('휴지통이 비어 있어요');return;}appConfirm({icon:'warn',title:'휴지통을 비울까요?',desc:list.length+'개를 영구 삭제해요.\n삭제하면 복구할 수 없어요.',okText:'비우기',danger:true}).then(function(ok){if(!ok)return;var ids=list.map(function(r){return r.id;});resources=resources.filter(function(x){return ids.indexOf(x.id)<0;});try{if(window.FB&&FB.enabled()&&FB.remove){ids.forEach(function(id){try{FB.remove('resources',id);}catch(e){}});}}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}openMinutesTrash();renderResourceList();showToast(ids.length+'개를 영구 삭제했어요');});}
function _writeLock(){var id=currentMinutesId;if(!id||!(window.FB&&FB.enabled()))return;try{var pr=FB.save('minutesLocks',id,{id:id,uid:G.id,name:((G.name||'')+''+(G.baptism||'')).trim(),ts:Date.now()});if(pr&&pr.catch)pr.catch(function(){if(!window._lockWarned){window._lockWarned=true;showToast('편집 잠금 저장 실패 — Firestore 규칙에 minutesLocks 권한이 필요해요');}});}catch(e){}}
...
if(ib)ib.innerHTML=b.imgs.map((s,i)=>`<div style="position:relative;flex-shrink:0"><img src="${bufSrc(s)}" class="attach-thumb"><button onclick="removeAttach('${pfx}','imgs',${i})" style="position:absolute;top:-5px;right:-5px;width:20px;height:20px;border-radius:50%;border:none;background:rgba(0,0,0,.65);color:white;font-size:11px;line-height:1;cursor:pointer"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>`).join('');
if(db)db.innerHTML=b.docs.map((d,i)=>`<div class="attach-file-item"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg> <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${d.name}</span><button onclick="removeAttach('${pfx}','docs',${i})" style="border:none;background:none;color:var(--coral);font-weight:800;font-size:13px;cursor:pointer;padding:0 4px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>`).join('');}
...
function addAttachDocs(input,pfx){
  ...
  if(big.length)showToast(''+big[0].name+(big.length>1?('외'+(big.length-1)+'개'):'')+'는 10MB를 넘어 첨부할 수 없어요');
...
function previewDocs(input,previewId){const wrap=document.getElementById(previewId);if(!wrap)return;wrap.innerHTML='';Array.from(input.files).slice(0,5).forEach(f=>{const div=document.createElement('div');div.className='attach-file-item';div.innerHTML='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg> '+f.name+' <span style="margin-left:auto;color:var(--text-light)">'+(f.size/1024/1024).toFixed(1)+'MB</span>';wrap.appendChild(div);});}
...
function renderHomeNotices(){const el=document.getElementById('home-notices');if(!el)return;const notices=sortPostsNewest(posts.filter(p=>p.cat==='notice'&&(G.role==='teacher'?true:G.role==='parent'?_canParentSee(p):_canStudentSee(p,G.gradeKey)))).slice(0,5);if(!notices.length){el.innerHTML='<div class="empty" style="padding:24px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg></div><div class="empty-title" style="font-size:13px">아직 공지사항이 없어요</div></div>';return;}el.innerHTML=notices.map(p=>{const k=noticeKind(p);return `<div class="notice-item" onclick="openPostDetail('${p.id}')"${k?` style="border-left:3px solid ${k.fg};padding-left:9px"`:''}><div class="notice-unread"></div><div class="notice-info"><div class="notice-title">${noticeChip(p)}${p.title}</div><div class="notice-meta">${p.authorName} · ${p.date}</div></div></div>`;}).join('');}
...
function _fillHomeAvatars(){try{var u={avatar:G.avatar,name:G.displayName};['home-student-avatar','home-teacher-avatar','home-parent-avatar'].forEach(function(id){var el=document.getElementById(id);if(el)el.innerHTML=_avatarHTML(u,48);});}catch(e){}}
...
function openProfileEdit(){
  ...
  if(pv){if(me.avatar){pv.style.backgroundImage='url('+me.avatar+')';pv.innerHTML='';}else{pv.style.backgroundImage='linear-gradient(135deg,#2FA595,#9B8FD4)';pv.innerHTML='<svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.92)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="3.4"/><path d="M4.5 20a7.5 7.5 0 0 1 15 0"/></svg>';}}
...
function renderAdminCouponList(){dedupeBdayCoupons();const el=document.getElementById('admin-coupon-list');if(!el)return;if(!coupons.length){el.innerHTML='<div class="empty" style="padding:28px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 8a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2 2 2 0 0 0 0 4 2 2 0 0 1-2 2H5a2 2 0 0 1-2-2 2 2 0 0 0 0-4z"/><path d="M13 6v12"/></svg></div><div class="empty-title" style="font-size:13px">발급된 쿠폰이 없어요</div><div class="empty-desc">학생이 출석 등급을 달성하면 쿠폰이 생겨요</div></div>';return;}el.innerHTML=coupons.slice().reverse().map(c=>{const card=`<div class="card" style="margin-bottom:0"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px">${_treeByLabel(c.badgeLabel,24)}${c.studentName} <span style="font-size:11px;color:var(--text-light);font-weight:500">· ${(_treeByLabel(c.badgeLabel)?_labelNoEmoji(c.badgeLabel):c.badgeLabel)} · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg> ${c.reward||'보상'}</span></div><span class="chip ${c.used?'chip-gray':'chip-coral'}">${c.used?'사용완료':'미사용'}</span></div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><div style="font-size:11px;color:var(--text-light)">발급 ${c.createdAt}${c.manual&&c.issuedBy?' · '+c.issuedBy:''}${c.used&&c.usedAt?' · 사용 '+c.usedAt:''}</div><div style="font-size:15px;font-weight:800;letter-spacing:2px;color:var(--primary-dark)">${c.used?'· · · · · ·':c.code}</div></div>${c.used?'<div style="font-size:10px;color:var(--text-light);margin-top:6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg> 밀어서 삭제</div>':''}</div>`;
      /* 사용완료 쿠폰만 삭제 가능 — 미사용 쿠폰은 탈퇴 시에만 정리된다 */
      if(!c.used)return `<div style="margin-bottom:8px">${card}</div>`;
      return `<div style="position:relative;overflow:hidden;border-radius:var(--radius);margin-bottom:8px"><button onclick="deleteCoupon('${c.id}')" style="position:absolute;top:0;right:0;bottom:0;width:84px;border:none;background:var(--coral);color:white;font-weight:800;font-size:12px;cursor:pointer;font-family:inherit"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg> 삭제</button><div class="coupon-swipe" data-cid="${c.id}" style="position:relative;transition:transform .2s;touch-action:pan-y;background:var(--card,white)">${card}</div></div>`;}).join('');initCouponSwipe(el);}
...
function updatePendingUI(){const pending=pendingList.filter(u=>!u.approved);const posReqs=pendingList.filter(function(u){return u.posRequest&&(u.posRequest==='principal'||u.posRequest==='admin')&&_canApprovePos(u.posRequest);});const canApprove=G.type==='principal'||G.type==='admin'||G.isAdmin;const bar=document.getElementById('pending-notice-bar');const txt=document.getElementById('pending-notice-text');if(bar)bar.style.display=(pending.length&&canApprove)?'':'none';if(txt)txt.textContent=`가입 승인 대기 중인 신청이 ${pending.length}건 있어요`;const _pw=document.getElementById('pending-wrap');if(_pw)_pw.style.display=(canApprove&&(pending.length||posReqs.length))?'':'none';const el=document.getElementById('pending-list');if(!el)return;var posHtml=posReqs.map(function(u){var lbl=POS_LBL[u.posRequest]||u.posRequest;var h=_posHolder(u.posRequest,u.id);return '<div class="pending-card"><div class="pending-head"><span class="pending-name">'+u.name+' '+u.baptism+'</span><span class="chip chip-lavender">'+lbl+' 요청</span></div><div class="pending-info">현재 '+lbl+': '+(h?h.name+' 선생님':'없음')+' <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg> '+u.name+' 선생님으로 이전</div><div class="pending-actions"><button class="btn-approve" onclick="approvePosReq(\''+u.id+'\')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 수락</button><button class="btn-reject" onclick="rejectPosReq(\''+u.id+'\')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 거절</button></div></div>';}).join('');if(!pending.length&&!posReqs.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg></div><div class="empty-title">대기 중인 신청이 없어요</div></div>';return;}el.innerHTML=posHtml+pending.map(u=>`<div class="pending-card"><div class="pending-head"><span class="pending-name">${u.name} ${u.baptism}</span><span class="chip chip-${u.role==='student'?'blue':u.role==='parent'?'mint':'lavender'}">${u.role==='student'?'학생':u.role==='parent'?'학부모':'교사'}</span></div><div class="pending-info">아이디: ${u.id} · ${u.gradeLabel||''}</div><div class="pending-actions"><button class="btn-approve" onclick="approve(${pendingList.indexOf(u)})"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 승인</button><button class="btn-reject" onclick="reject(${pendingList.indexOf(u)})"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 거절</button></div></div>`).join('');}
...
function _resetOptinText(){var t=document.getElementById('push-optin-title'),d=document.getElementById('push-optin-desc');if(t)t.textContent='알림을 놓치지 마세요!';if(d)d.innerHTML='알림을 켜지 않으면 <b>쿠폰·공지·생일 축하</b>를<br>제때 받지 못해요. 앱을 닫아둬도 바로 알려드릴게요 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg>';}
...
function renderAttendList(){_gradeSync();const students=pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated);const el=document.getElementById('attend-student-list');if(!el)return;const badge=document.getElementById('attend-scope-badge');const POS={m1:'중1',m2:'중2',m3:'중3',h:'고등',principal:'전체',admin:'전체'};if(badge)badge.textContent=POS[G.type]||'전체';const isFull=G.type==='principal'||G.type==='admin'||G.isAdmin;const mine=isFull?students:students.filter(u=>u.gradeKey===G.type);const sat=attendSat();const isVac=isVacationDate(sat);const info=document.getElementById('attend-week-info');if(info)info.textContent='이번 주 토요일: '+sat+(isVac?' (방학·출석 없음)':isEduVacation(sat)?' (<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> 교리방학·출석 진행)':'');const vh=document.getElementById('vacation-hint');if(vh)vh.style.display=isFull?'block':'none';if(!mine.length){el.innerHTML='<div class="empty" style="padding:32px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg></div><div class="empty-title" style="font-size:13px">담당 학생이 없어요</div></div>';return;}el.innerHTML=mine.map(u=>{const st=attendStatus(u,sat);const qt=(u.qrScanAt||{})[sat];const SEG=(k,l)=>`<button style="font-size:10px;font-weight:700;padding:5px 0;flex:1;border:none;border-radius:7px;cursor:pointer;font-family:inherit;background:${st===k?(k==='full'?'var(--mint)':k==='half'?'var(--yellow)':'var(--coral)'):'var(--bg)'};color:${st===k?'white':'var(--text-light)'}" ${isVac?'disabled':''} onclick="setAttendStatus('${u.id}','${k}')">${l}</button>`;return `<div class="student-row${u.id===window._lastAttendUid?' just-set':''}" style="flex-wrap:wrap"><div class="student-avatar" style="background:linear-gradient(135deg,var(--primary),var(--lavender))">${u.name.charAt(0)}</div><div class="student-info" style="cursor:pointer" onclick="openStudentDetail('${u.id}')"><div class="student-name">${u.name} ${u.baptism} <span class="grade-${u.gradeKey}" style="margin-left:2px">${u.gradeLabel||''}</span></div><div class="student-detail">${u.manualReg?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.8 12.8 0 0 0 2.5.4A2 2 0 0 1 22 16.92v3A2 2 0 0 1 20 22 18 18 0 0 1 3.06 6a2 2 0 0 1 2-2h.09"/><line x1="23" y1="1" x2="1" y2="23"/></svg> ':''}누적 출석 ${u.attendTotal||0}회 · 연속 ${u.streak||0}주${qt?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12" y2="18"/></svg>QR '+qt:''}</div></div>${isVac?'':`<div style="display:flex;gap:4px;width:100%;margin-top:8px">${SEG('full','출석')}${SEG('half','반일')}${SEG('absent','결석')}</div>`}</div>`;}).join('');}
...
showToast((type==='edu'?'':'')+label+''+(type==='edu'?'교리방학':'방학')+'지정 완료! 공지·캘린더 반영');refreshAfterVacChange();}
...
function renderNotifList(){const el=document.getElementById('notif-list');if(!el)return;const list=notifications.filter(notifMatch).sort(function(a,b){return _notifTs(b)-_notifTs(a);}).slice(0,30);const _snap=_notifReadSnap;const _wasRead=(n)=>_snap?_snap.has(n.id):nRead(n);const readCnt=notifications.filter(n=>notifMatch(n)&&_wasRead(n)).length;if(!list.length){el.innerHTML='<div class="empty" style="padding:28px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/></svg></div><div class="empty-title" style="font-size:13px">알림이 없어요</div></div>';return;}el.innerHTML=(readCnt?`<div style="text-align:right;margin-bottom:4px"><button onclick="clearReadNotifs()" style="background:none;border:none;color:var(--text-light);font-size:11px;cursor:pointer;font-family:inherit;padding:4px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg> 읽은 알림 지우기 (${readCnt})</button></div>`:'')+list.map(n=>`<div class="notif-item" style="display:flex;align-items:center;gap:8px;padding:12px 4px;border-bottom:1px solid var(--border-light)${_wasRead(n)?';opacity:.65':''}"><div style="flex:1;font-size:13px;line-height:1.5;cursor:pointer" onclick="${n.tap?`notifTap('${n.id}')`:`markOneRead('${n.id}')`}">${stripNotifIcons(n.text)}<div style="font-size:10px;color:var(--text-light);margin-top:3px">${n.ts?_relTime(n.ts):(n.time||'')}</div></div>${n.tap?'<div style="color:var(--text-light);font-size:14px;flex-shrink:0">›</div>':''}<button onclick="deleteNotif('${n.id}')" style="background:none;border:none;color:var(--text-light);font-size:13px;cursor:pointer;padding:6px;flex-shrink:0"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>`).join('');updateNotifDot();}
...
function renderStorageStats(){
  ...
  el.innerHTML='<div style="display:flex;justify-content:space-between;padding:8px 2px;border-bottom:2px solid var(--border-light);font-weight:800"><span>합계</span><span>'+_fmtKB(total)+(_blobUsage?'':' <span style="font-size:10px;color:var(--coral,#E0663F);font-weight:700">(사진 미포함)</span>')+'</span></div>'
    ...
    +(_blobUsage
       ? '<div style="margin-top:10px;padding:10px;background:'+(warn?'var(--coral-light)':'var(--bg)')+';border-radius:8px"><div style="display:flex;justify-content:space-between;font-size:12px;font-weight:800;color:'+(warn?'#B0463A':'var(--text)')+'"><span>Firestore 무료 한도 1GB 중</span><span>'+pct.toFixed(1)+'% 사용</span></div><div style="height:7px;background:var(--border-light);border-radius:4px;margin-top:7px;overflow:hidden"><div style="height:100%;width:'+Math.min(100,pct).toFixed(1)+'%;background:'+(warn?'#E0663F':'var(--primary)')+'"></div></div>'+(pct>=50?'<div style="font-size:11px;margin-top:7px;font-weight:700"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h16.9a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> 50%를 넘었어요. 저장소 이전을 준비하세요.</div>':'')+'</div>'
    ...
    +(big.length?'<div style="margin-top:8px;padding:8px;background:var(--coral-light);border-radius:8px;color:#B0463A;font-weight:700"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h16.9a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> 1MB를 넘어 저장에 실패할 수 있는 게시글 '+big.length+'건</div>':'');
}
async function recalcStorage(){
  ...
  if(el)el.innerHTML='<div style="text-align:center;padding:30px 0;color:var(--text-light);font-size:13px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg> 사진·첨부파일 용량을 조회하는 중…<br><span style="font-size:11px">사진이 많으면 조금 걸려요</span></div>';
  try{
...
function renderPhotoCleanup(){
  ...
  if(!list.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div><div class="empty-title">갤러리 사진이 없어요</div></div>';return;}
...
function renderMembersList(){const el=document.getElementById('members-list');if(!el)return;let all=pendingList.filter(u=>u.approved);const total=all.length;if(memberRoleFilter!=='all')all=all.filter(u=>u.role===memberRoleFilter);if(memberRoleFilter==='student'&&memberGradeFilter!=='all-s')all=all.filter(u=>u.gradeKey===memberGradeFilter);const cEl=document.getElementById('members-count');if(cEl)cEl.textContent=(memberRoleFilter==='all'?'전체 ':'')+all.length+'명'+(memberRoleFilter==='all'?'':' / 전체 '+total+'명');if(!all.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div><div class="empty-title">해당하는 회원이 없어요</div></div>';return;}const canAdmin=G.type==='principal'||G.type==='admin'||G.isAdmin;const BTN=(bg,fg,fn,label)=>`<button style="font-size:10px;padding:4px 9px;background:${bg};color:${fg};border:none;border-radius:8px;cursor:pointer;font-weight:700;font-family:'Noto Sans KR',sans-serif;flex-shrink:0" onclick="${fn}">${label}</button>`;el.innerHTML=all.map(u=>{const isStu=u.role==='student';const status=u.graduated?' <span class="chip chip-gray" style="font-size:9px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg> 졸업</span>':u.hidden?' <span class="chip chip-gray" style="font-size:9px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg> 숨김</span>':'';let btns='';if(canAdmin){btns=BTN('var(--primary-light)','var(--primary-dark)',`openMemberEdit('${u.id}')`,'수정');if(isStu&&u.manualReg&&!u.graduated)btns+=BTN('var(--mint-light)','#2D9E8F',`mergeManualPrompt('${u.id}')`,'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.3 7 12 12 20.7 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg> 이관');if(!u.graduated)btns+=BTN('var(--bg)','var(--text-sub)',`toggleHideMember('${u.id}')`,u.hidden?'숨김 해제':'숨김');btns+=BTN('var(--coral-light)','#D95F50',`removeMember('${u.id}')`,'삭제');}return `<div class="student-row" style="flex-wrap:wrap;cursor:pointer;${u.hidden||u.graduated?'opacity:.65':''}" onclick="openMemberDetail('${u.id}')"><div onclick="event.stopPropagation();openProfileView('${u.id}')" style="flex-shrink:0">${_avatarHTML(u,40)}</div><div class="student-info"><div class="student-name">${u.name} ${u.baptism}${status}</div><div class="student-detail">${u.role==='student'?'학생':u.role==='parent'?'학부모':'교사'} · ${u.gradeLabel||''} ${u.isAdmin?'· 관리자':''} ${u.isJabumoPresident?'· 자부모회장':''}</div></div><div style="display:flex;gap:4px;flex-shrink:0;margin-left:auto" onclick="event.stopPropagation()">${btns}</div></div>`;}).join('');}
...
function openChildLink(pid,ix){
  ...
  document.getElementById('child-link-list').innerHTML=list.map(s=>{
    ...
    return `<div onclick="pickChildLink('${s.id}')" style="display:flex;align-items:center;gap:10px;padding:11px 12px;border:2px solid ${sel?'var(--primary)':'var(--border-light)'};background:${sel?'var(--primary-light)':'var(--card,white)'};border-radius:10px;margin-bottom:8px;cursor:pointer">
      ...
      ${sel?'<span style="color:var(--primary);font-weight:800"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg></span>':''}</div>`;
...
function openMemberDetail(uid){
  ...
  if(u.role==='student'){
    ...
    const _kids=(u.children||[]).map(function(c,ix){
      ...
      const tag=pin?'<span style="color:var(--mint);font-weight:700">연결됨</span>'
               :ms.length>1?'<span style="color:var(--coral);font-weight:700"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h16.9a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> '+ms.length+'명 중복</span>'
  ...
  if(u.role==='student')acts+=`<button class="btn btn-outline btn-sm" style="flex:1" onclick="closeModal('member-detail-modal');openStudentDetail('${u.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg> 학생 기록 보기</button>`;
  if(canAdmin)acts+=`<button class="btn btn-primary btn-sm" style="flex:1" onclick="closeModal('member-detail-modal');openMemberEdit('${u.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg> 정보 수정</button>`;
  acts+='</div>';
  if(canResetPw()&&u.id!==G.id&&!u.manualReg)acts+=`<button class="btn btn-outline btn-sm" style="width:100%;margin-top:8px" onclick="resetMemberPw('${u.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 2l-2 2m-3.4 3.4a5.4 5.4 0 1 0-2.6 2.6L21 4z"/><circle cx="7.5" cy="15.5" r="4.5"/></svg> 비밀번호 초기화</button>`;
...
const wantAdmin=document.getElementById('me-admin-check').checked;if(wantAdmin!==!!u.isAdmin){showToast('관리자 권한은 여기서 변경할 수 없어요. 관리 탭의 이임 또는 교사 과반 투표를 이용하세요');document.getElementById('me-admin-check').checked=!!u.isAdmin;return;}
...
function renderDeptInfo(){const teachers=pendingList.filter(u=>u.approved&&u.role==='teacher');const choirT=teachers.filter(u=>(u.dept||[]).includes('choir')).map(u=>u.name+' '+u.baptism);const liturgyT=teachers.filter(u=>(u.dept||[]).includes('liturgy')).map(u=>u.name+' '+u.baptism);const ci=document.getElementById('choir-info');if(ci)ci.textContent=choirT.length?'담당교사 '+choirT.join(', '):'담당교사 미정';const li=document.getElementById('liturgy-info');if(li)li.textContent=liturgyT.length?'담당교사 '+liturgyT.join(', '):'담당교사 미정';renderDeptPosts('choir');renderDeptPosts('liturgy');renderDeptWeekInfo();var _ad=(document.getElementById('dt-liturgy')&&document.getElementById('dt-liturgy').classList.contains('active'))?'liturgy':'choir';try{_deptFit(_ad);setTimeout(function(){_deptFit(_ad);},120);}catch(e){}
  if(!teachers.length){renderDeptInfo._n=(renderDeptInfo._n||0)+1;if(renderDeptInfo._n<=4){setTimeout(function(){var sa=document.getElementById('screen-activity');if(sa&&sa.classList.contains('active'))renderDeptInfo();},1200);}}else{renderDeptInfo._n=0;}
}
function _dlCanvas(cv,fn){try{var a=document.createElement('a');a.download=fn;a.href=cv.toDataURL('image/png');document.body.appendChild(a);a.click();a.remove();showToast('이미지를 저장했어요');}catch(e){showToast('이미지 저장 실패');}}
function _drawInfoCard(opt){
  var dpr=2,W=700,pad=44,rowH=62,rows=opt.rows||[];
  var H=150+rows.length*rowH+(opt.footer?66:0)+34;
  var cv=document.createElement('canvas');cv.width=W*dpr;cv.height=H*dpr;var x=cv.getContext('2d');x.scale(dpr,dpr);
  x.fillStyle='#FFFFFF';x.fillRect(0,0,W,H);
  x.fillStyle='#E7F4EF';x.fillRect(0,0,W,112);
  x.fillStyle='#1E7D70';x.font='700 30px "Noto Sans KR",sans-serif';x.textBaseline='alphabetic';x.fillText(opt.title||'',pad,60);
  x.fillStyle='#5A6A8A';x.font='16px "Noto Sans KR",sans-serif';x.fillText(opt.subtitle||'',pad,90);
  var y=150;
  rows.forEach(function(r){
    x.fillStyle='#9AAAC5';x.font='700 15px "Noto Sans KR",sans-serif';x.textAlign='left';x.fillText(r.label||'',pad,y);
    x.fillStyle='#1A2340';x.font='700 20px "Noto Sans KR",sans-serif';x.fillText(r.value||'',pad+110,y);
    if(r.sub){x.fillStyle='#1E7D70';x.font='700 16px "Noto Sans KR",sans-serif';x.textAlign='right';x.fillText(r.sub,W-pad,y);}
    x.textAlign='left';x.strokeStyle='#EAF0FA';x.lineWidth=1;x.beginPath();x.moveTo(pad,y+20);x.lineTo(W-pad,y+20);x.stroke();
    y+=rowH;
  });
  if(opt.footer){x.fillStyle='#5A6A8A';x.font='16px "Noto Sans KR",sans-serif';x.fillText(opt.footer,pad,y+22);}
  _dlCanvas(cv,opt.filename||'card.png');
}
function _exportLiturgyImg(){var l=(appConfig.liturgyWeek)||{};var sat=currentSaturday();var d=sat.split('-');var rows=[];[['제1독서',l.reading1,l.r1],['제2독서',l.reading2,l.r2],['복음',l.gospel,l.rg]].forEach(function(r){if(r[1])rows.push({label:r[0],value:r[1],sub:r[2]||''});});if(!rows.length){showToast('저장할 독서 내용이 없어요');return;}_drawInfoCard({title:(l.title||((typeof _dayLabel==='function'&&_dayLabel(sat))||'이번 주 전례')),subtitle:(+d[1])+'월 '+(+d[2])+'일'+(l.team?'  ·  전례부원 '+l.team:''),rows:rows,footer:l.note?('※ '+l.note):'',filename:'전례부_'+sat+'.png'});}
function _exportChoirImg(){var c=(appConfig.choirWeek)||{};var sat=currentSaturday();var d=sat.split('-');var rows=[];[['입당',c.ipdang],['봉헌',c.bonghen],['성체1',c.seongche1],['성체2',c.seongche2],['파견',c.pagyeon]].forEach(function(r){if(r[1])rows.push({label:r[0],value:r[1]});});if(c.teuksong)rows.push({label:'특송',value:c.teuksong});if(!rows.length){showToast('저장할 성가곡이 없어요');return;}_drawInfoCard({title:(c.title||'이번 주 성가곡'),subtitle:(+d[1])+'월 '+(+d[2])+'일',rows:rows,footer:c.note||'',filename:'성가대_'+sat+'.png'});}
function _imgBtn(fn){return '<button onclick="'+fn+'" style="background:var(--bg);border:1px solid var(--border-light);border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--text-sub);margin-right:6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg> 이미지</button>';}
...
var editBtn=function(fn){return canEdit?'<button onclick="'+fn+'()" style="background:var(--bg);border:1px solid var(--border-light);border-radius:8px;padding:4px 11px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--text-sub)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg> 편집</button>':'';};
...
var _wkMd=_sat?((+_sat.split('-')[1])+'월 '+(+_sat.split('-')[2])+'일'):'';
var _wkSea=(typeof _dayLabel==='function'&&_sat)?_dayLabel(_sat):'';
var _wkSub=[_wkSea,_wkMd].filter(Boolean).join(' · ');
try{var _bd=_sat?((+_sat.split('-')[1])+'.'+(+_sat.split('-')[2])):'';var _cl=document.getElementById('choir-week-label');if(_cl)_cl.textContent='THIS WEEK'+(_bd?' · '+_bd:'')+' · 성가대';var _ll=document.getElementById('liturgy-week-label');if(_ll)_ll.textContent='THIS WEEK'+(_bd?' · '+_bd:'')+' · 전례부';}catch(e){}
...
var cc=document.getElementById('choir-week-card');if(cc){var crows=[['입당',c.ipdang],['봉헌',c.bonghen],['성체1',c.seongche1],['성체2',c.seongche2],['파견',c.pagyeon]].filter(function(r){return r[1];});var cEmpty=!crows.length&&!c.teuksong;
  var h='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px"><div style="min-width:0"><div style="font-size:10.5px;letter-spacing:.5px;color:#C9785F;font-weight:700">WEEKLY CHOIR</div><div style="font-size:16px;font-weight:800;color:var(--text);margin-top:1px">이번 주 성가곡</div>'+(_wkSub?'<div style="font-size:12.5px;color:var(--text-sub);margin-top:3px;font-weight:600">'+E(_wkSub)+'</div>':'')+'</div>'+monthBtn()+'</div>';
  if(cEmpty){h+='<div style="font-size:12px;color:var(--text-light);padding:4px 0">아직 등록된 성가곡이 없어요'+(canEdit?' · 편집을 눌러 입력하세요':'')+'</div>';}
  else{h+=crows.map(function(r,i){return '<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border-light)"><span style="font-size:14px;color:#C9785F;font-weight:800;width:20px;flex-shrink:0">'+('0'+(i+1)).slice(-2)+'</span><span style="flex:1;min-width:0;font-size:14px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+E(r[0])+'</span><span style="font-size:14px;color:var(--text-sub);font-weight:600;flex-shrink:0">'+E(r[1])+'</span></div>';}).join('');
    if(c.teuksong)h+='<div style="display:flex;align-items:center;gap:12px;padding:10px 0"><span style="font-size:12px;color:#C9785F;font-weight:800;width:20px;flex-shrink:0">특송</span><span style="flex:1;font-size:14px;font-weight:700">'+E(c.teuksong)+'</span></div>';
    if(c.note)h+='<div style="margin-top:9px;font-size:12.5px;color:#C9785F;font-weight:700">'+E(c.note)+'</div>';}
  cc.innerHTML=h;}
...
function openLiturgyWeekEdit(){var l=(appConfig.liturgyWeek)||{};['title','reading1','reading2','gospel','r1','r2','rg','team','note'].forEach(function(k){var el=document.getElementById('lw-'+k);if(el)el.value=l[k]||'';});openModal('liturgy-week-modal');}
function saveLiturgyWeek(){var v=function(k){return (document.getElementById('lw-'+k)||{}).value||'';};appConfig.liturgyWeek={title:v('title').trim(),reading1:v('reading1').trim(),reading2:v('reading2').trim(),gospel:v('gospel').trim(),r1:v('r1').trim(),r2:v('r2').trim(),rg:v('rg').trim(),team:v('team').trim(),note:v('note').trim()};renderDeptWeekInfo();closeModal('liturgy-week-modal');try{if(window.flushCfg)window.flushCfg();}catch(e){}showToast('이번 주 독서가 저장되었어요');}

function renderStudentCards(gf){_gradeSync();try{_lastStudentFilter=gf||'all';}catch(e){}const el=document.getElementById('student-card-list');if(!el)return;const isFull=G.type==='principal'||G.type==='admin'||G.isAdmin;const isGrade=['m1','m2','m3','h'].includes(G.type);const fb=document.getElementById('student-grade-filter');if(fb)fb.style.display=isFull?'':'none';if(!isFull&&!isGrade){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="10"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/></svg></div><div class="empty-title">해당사항이 없어요</div><div class="empty-desc">담당 학년이 지정된 교사와 교감·교무·관리자만 이용할 수 있어요</div></div>';return;}let students=pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated);if(!isFull)students=students.filter(u=>u.gradeKey===G.type);else if(gf!=='all')students=students.filter(u=>u.gradeKey===gf);if(!students.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div><div class="empty-title">등록된 학생이 없어요</div></div>';return;}el.innerHTML=students.map(u=>{let lv='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg>';const t=u.attendTotal||0;for(const L of ATTEND_LEVELS)if(t>=L.n)lv=L.l.split(' ')[0];return `<div class="student-card-item" onclick="openStudentDetail('${u.id}')"><div class="student-card-avatar" style="background:${GCOLOR[u.gradeKey]||GCOLOR.m1}">${u.name.charAt(0)}</div><div class="student-card-info"><div class="student-card-name">${u.name} ${u.baptism}${crownMark(u)}</div><div class="student-card-sub">${u.gradeLabel||''}${u.manualReg?' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.8 12.8 0 0 0 2.5.4A2 2 0 0 1 22 16.92v3A2 2 0 0 1 20 22 18 18 0 0 1 3.06 6a2 2 0 0 1 2-2h.09"/><line x1="23" y1="1" x2="1" y2="23"/></svg> 수동':''}</div></div><div class="student-card-stats"><div class="student-card-stat"><div class="student-card-stat-n">${t}</div><div class="student-card-stat-l">출석</div></div><div class="student-card-stat"><div class="student-card-stat-n">${lv}</div><div class="student-card-stat-l">등급</div></div></div></div>`;}).join('');}
...
function renderPastDates(){
  ...
  el.innerHTML='<div style="font-size:11px;color:var(--text-light);margin-bottom:7px">칸을 눌러 출석 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg> 반출석 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg> 결석으로 바꿔주세요</div>'
    +'<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:6px;margin-bottom:8px">'+cells+'</div>'
    +'<div style="font-size:11px;color:var(--text-sub);text-align:center;margin-bottom:12px">○ '+o+' · ◐ '+h+' · <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> '+x+' <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg> 출석 <b>'+(o+h*0.5)+'회</b></div>';
}
...
function renderDetailAttend(){
  ...
  if(pick!=='now'){
    ...
    el.innerHTML='<div class="card" style="padding:14px">'
      ...
      +'<button onclick="delPastRecord('+(u.history.length-1-parseInt(pick,10))+')" style="width:100%;margin-top:12px;padding:10px;border:1px solid var(--coral);background:var(--coral-light);color:#D95F50;border-radius:10px;font-size:12px;font-weight:700;font-family:inherit;cursor:pointer"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg> 이 학년도 기록 삭제</button>'
      +'</div>';
    return;
  }
  ...
  var cells=weeks.map(function(w){
    ...
    return '<div style="text-align:center"><div class="att-cell"'+ev+' style="position:relative;background:'+bg+';color:'+fg+';border-radius:9px;padding:7px 0;font-size:14px;font-weight:800;line-height:1'+(can?';cursor:pointer':'')+'">'+mark
      +(ed?'<span style="position:absolute;top:1px;right:3px;font-size:8px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg></span>':'')+'</div>'
  ...
  el.innerHTML='<div class="card" style="padding:14px">'
    ...
    +'<div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin-top:11px;font-size:10px;color:var(--text-light)">'
      +'<span>○ 출석 '+cnt.o+'</span><span>◐ 반출석 '+cnt.h+'</span><span><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 결석 '+cnt.x+'</span><span>– 방학 '+cnt.v+'</span>'
    +'</div>'
...
function openLitRoster(ds){
  ...
  +'<button class="btn btn-primary" style="width:100%;margin-top:4px" onclick="sendLitNotify(\''+ds+'\')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg> 배정 학생에게 알림 보내기</button>';
...
function sendLitNotify(ds){
  ...
  Object.keys(byStu).forEach(function(sid){
    ...
    notifications.unshift({pushed:false,id:'nt'+Date.now()+'lit'+cnt+Math.random().toString(36).slice(2,5),
      text:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg> <b>'+when+'</b> 미사 전례 봉사 안내 — 이번 주 <b>'+_esc(roleTxt)+'</b>(을)를 맡았어요. 미리 준비해 주세요 <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1-1.1a5.5 5.5 0 1 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8z"/></svg>',
...
function saveLitMonth(){
  /* 전례(독서·전례봉사 배정)는 교사가 편집·저장 가능. 연간계획 그리드(yearplan)와 달리 관리자 전용/잠금 대상이 아님 */
  if(G.role!=='teacher'){showToast('교사만 전례를 수정할 수 있어요');return;}
  if(_litBuf){
  ...
  showToast(''+(litMonth+1)+'월 전례를 저장했어요');
}
...
function hymnCardHtml(dateStr,compact){
  ...
  if(compact){
    return '<div style="font-size:11px;color:var(--text-sub);margin-top:3px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg> '
  ...
  return '<div class="card" style="margin-top:8px;padding:12px 14px">'
    +'<div style="font-size:12px;font-weight:800;margin-bottom:7px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg> 성가'+(h.label?' <span style="font-weight:600;color:var(--text-light)">'+_esc(h.label)+'</span>':'')+'</div>'
...
function renderCalDayEvents(dateStr){const el=document.getElementById('cal-event-list');if(!el)return;try{_hydrateYP();}catch(e){}const events=calEvents.filter(e=>e.date===dateStr&&(e.isRecurring||e.visibility!=='private'||e.authorId===G.id));const isFull=G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);const isT=G.role==='teacher';const [y,m,d]=dateStr.split('-');const isSat=new Date(dateStr+'T12:00:00').getDay()===6;const isVac=isVacationDate(dateStr);const isEdu=isEduVacation(dateStr);const anyVac=isVac||isEdu;let html=`<div style="display:flex;align-items:center;justify-content:space-between;margin:10px 0 8px"><span style="font-size:13px;font-weight:800">${parseInt(m)}월 ${parseInt(d)}일 일정${isEdu?' <span class="chip chip-blue"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> 교리방학</span>':isVac?' <span class="chip chip-yellow"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg> 방학</span>':''}</span><span style="display:flex;gap:6px">${isFull&&isSat?`<button class="btn btn-sm" style="width:auto;background:${anyVac?'var(--bg)':'var(--yellow-light)'};color:${anyVac?'var(--text-sub)':'#9A6A00'};font-weight:700" onclick="setVacationDate('${dateStr}',${!anyVac})">${anyVac?'방학 해제':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg> 방학 지정'}</button>`:''}</span></div>`;if(isSat){var _lp=litFor(dateStr)||{};var _ph=_dayLabel(dateStr);var _bits=[];if(_ph)_bits.push(['전례시기',_ph]);if(_lp.optype)_bits.push(['운영구분',_lp.optype]);if(_lp.form)_bits.push(['교리',_lp.form]);if(_lp.owner)_bits.push(['교안담당',_lp.owner]);if(_lp.detail)_bits.push(['교리내용',_lp.detail]);if(_lp.liturgyTeam)_bits.push(['전례부',_lp.liturgyTeam]);if(_lp.choir)_bits.push(['성가대',_lp.choir]);if(_lp.progress)_bits.push(['진행',_lp.progress]);if(_bits.length){html+='<div '+(isT?'onclick="openYearPlan()" ':'')+'style="background:var(--primary-light);border-radius:10px;padding:10px 12px;margin-bottom:8px'+(isT?';cursor:pointer':'')+'"><div style="font-size:11px;font-weight:800;color:var(--primary-dark);margin-bottom:5px;display:flex;justify-content:space-between;align-items:center"><span><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg> 연간 운영 계획</span>'+(isT?'<span style="font-weight:700;color:var(--primary)">전체 보기 ›</span>':'')+'</div>'+_bits.map(function(b){return '<div style="font-size:12px;display:flex;gap:6px;margin-top:2px"><span style="color:var(--text-light);width:60px;flex-shrink:0">'+b[0]+'</span><span style="font-weight:600;color:var(--text-sub)">'+_esc(b[1])+'</span></div>';}).join('')+'</div>';}}if(!events.length){html+='<div class="empty" style="padding:20px"><div class="empty-emoji" style="font-size:24px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div class="empty-title" style="font-size:12px">등록된 일정이 없어요</div></div>';}else{html+=events.map(e=>{const teachers=getApprovedTeachers();const _R=_evResp(e);const yesCount=teachers.filter(t=>_R[t.id]==='yes').length;const noCount=teachers.filter(t=>_R[t.id]==='no').length;const noResp=teachers.filter(t=>!_R[t.id]);const myResp=_R[G.id];const isShared=e.visibility==='shared';const borderColor=e.isRecurring?'var(--mint)':(isShared?'var(--primary)':'var(--lavender)');const badge=e.isRecurring?'':(isShared?`<span class="chip chip-blue"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> 전체 공유</span>`:`<span class="chip chip-lavender"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> 나만 보기</span>`);const _isPast=dateStr<toDateStr(new Date());
    let respHtml='';if((e.isRecurring||e.vote||e.visibility==='shared')&&G.role==='teacher'){respHtml=(_isPast?'':`<div style="display:flex;gap:6px;margin-top:10px">
        <button class="filter-chip${myResp==='yes'?' active':''}" onclick="setCalResponse('${e.id}','yes')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 참석</button>
        <button class="filter-chip${myResp==='no'?' active':''}" onclick="setCalResponse('${e.id}','no')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 불참</button>
      </div>`)+`<div style="display:flex;gap:6px;margin-top:9px">
        <button class="resp-cnt" onclick="openRespModal('${e.id}','yes')"><b>${yesCount}</b> <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 참석</button>
        <button class="resp-cnt" onclick="openRespModal('${e.id}','no')"><b>${noCount}</b> <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> 불참</button>
        <button class="resp-cnt" onclick="openRespModal('${e.id}','none')"><b>${noResp.length}</b> <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> 미응답</button>
      </div>${(!_isPast&&isFull&&noResp.length)?`<button class="btn btn-sm" style="width:100%;margin-top:6px;background:var(--yellow-light);color:#9A6A00;font-weight:700" onclick="notifyUnrespondedTeachers()"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/></svg> 미응답 ${noResp.length}명에게 투표 알림</button>`:''}`;}let checkHtml='';if(!e.isRecurring&&isShared){const checked=(e.checkedBy||[]).includes(G.id);checkHtml=`<div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding-top:8px;border-top:1px solid var(--border-light)"><span style="font-size:10px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 확인 ${((e.checkedBy||[]).length)}명</span><button class="btn btn-sm ${checked?'':'btn-outline'}" style="${checked?'background:var(--mint);color:white':''}" onclick="toggleCalCheck('${e.id}')">${checked?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> 확인함':'확인하기'}</button></div>`;}return `<div class="card" style="margin-bottom:8px;border-left:4px solid ${borderColor}"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><div style="font-size:14px;font-weight:800">${e.title}</div>${badge?`<div style="margin-top:4px">${badge}</div>`:''}</div>${e.time?`<span style="font-size:11px;color:var(--text-light)">${e.time}</span>`:''}</div>${e.place?`<div style="font-size:11px;color:var(--text-sub);margin-top:4px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 21s-6-5.7-6-10a6 6 0 0 1 12 0c0 4.3-6 10-6 10z"/><circle cx="12" cy="11" r="2.2"/></svg> ${e.place}</div>`:''}${e.content?`<div style="font-size:12px;color:var(--text-sub);margin-top:6px;line-height:1.5">${e.content}</div>`:''}${e.authorName&&!e.isRecurring?`<div style="font-size:10px;color:var(--text-light);margin-top:6px">작성자: ${e.authorName}${e.edited?' · 수정됨':''}</div>`:''}${canEditCal(e)?`<div style="display:flex;gap:6px;margin-top:8px"><button class="btn btn-sm btn-outline" style="width:auto;flex:1" onclick="openCalEditModal('${e.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg> 수정</button><button class="btn btn-sm" style="width:auto;flex:1;background:var(--coral-light);color:#B0463A;font-weight:700" onclick="deleteCalEvent('${e.id}')"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg> 삭제</button></div>`:''}${respHtml}${checkHtml}</div>`;}).join('');}
  ...
  try{
    ...
    if(_rms.length){
      html+='<div class="card" style="margin-top:8px;padding:12px 14px">'
        +'<div style="font-size:11px;font-weight:800;color:var(--text-sub);margin-bottom:8px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 21s-6-5.7-6-10a6 6 0 0 1 12 0c0 4.3-6 10-6 10z"/><circle cx="12" cy="11" r="2.2"/></svg> 할 일</div>'
        +_rms.map(function(r){
  ...
  try{
    ...
    if(_bf.birth.length||_bf.feast.length){
      ...
      html+='<div class="card" style="margin-top:8px;padding:12px 14px">'
        +(_bf.birth.length?('<div style="font-size:11px;font-weight:800;color:var(--text-sub);margin-bottom:6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg> 생일</div>'+_bf.birth.map(function(u){return _chip(u,'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M4 21h16"/><path d="M5 21v-8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8"/><path d="M4 15c1.5 0 1.5 1 3 1s1.5-1 3-1 1.5 1 3 1 1.5-1 3-1 1.5 1 3 1"/><path d="M12 8v-.5a2 2 0 1 0-2 0"/></svg>','var(--coral-light)','#D95F50');}).join('')):'')
        +(_bf.feast.length?('<div style="font-size:11px;font-weight:800;color:var(--text-sub);margin:'+(_bf.birth.length?'8px':'0')+' 0 6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg> 축일</div>'+_bf.feast.map(function(u){return _chip(u,'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg>','var(--gold-light)','#B37A00');}).join('')):'')
        +'</div>';
    }
  }catch(e){}
...
var _SVG_PLAN='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="3" width="8" height="4" rx="1"/><path d="M9 5H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-3"/><path d="M8 12h8M8 16h5"/></svg>';
...
function renderHomeSmart(){
  ...
  var left = isFull
    ? sub('openYearPlan()',_SVG_PLAN,'연간계획',0)
...
function renderHomeSchedule(){const el=document.getElementById('teacher-schedule');if(!el)return;ensureWeeklyEvents();const n=new Date();const t0=n.getFullYear()+'-'+pad2(n.getMonth()+1)+'-'+pad2(n.getDate());const sat=currentSaturday();const end=sat>=t0?sat:t0;const evs=calEvents.filter(e=>e.date>=t0&&e.date<=end&&(e.isRecurring||e.visibility!=='private'||e.authorId===G.id)).sort((a,b)=>a.date<b.date?-1:a.date>b.date?1:0);if(!evs.length){el.innerHTML='<div class="empty" style="padding:20px"><div class="empty-emoji" style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div class="empty-title" style="font-size:13px">이번 주 일정이 없어요</div></div>';return;}el.innerHTML=evs.map(e=>{const[y,m,d]=e.date.split('-');const dow=['일','월','화','수','목','금','토'][new Date(e.date+'T12:00:00').getDay()];return `<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border-light);cursor:pointer" onclick="switchTab('calendar');selectedCalDate='${e.date}';renderCalendar()"><span class="chip ${e.isRecurring?'chip-mint':'chip-blue'}" style="flex-shrink:0">${parseInt(m)}/${parseInt(d)} (${dow})</span><div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${e.title}</div>${(e.time||e.place)?`<div style="font-size:11px;color:var(--text-light)">${e.time||''}${e.place?' · '+e.place:''}</div>`:''}</div></div>`;}).join('');}
...
function notifyUnrespondedTeachers(){
  ...
  teachers.forEach(t=>{
    ...
    notifications.unshift({pushed:false,id:nid,text:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> '+(tm+1)+'월 주일학교 참석 여부를 아직 응답하지 않았어요: <b>'+label+'</b><br>눌러서 참석/불참을 알려주세요!',time:'방금',ts:Date.now(),readBy:[],hiddenBy:[],forTeacherId:t.id,tap:{type:'cal-vote',date:miss[0].date}});
    sent++;
  });
  updateNotifDot();
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
...
function _sealIfRolled(u){if(u.role!=='student')return false;var cur=_curSchoolYr();if(u.curTermYr==null){u.curTermYr=cur;return true;}if(u.curTermYr>=cur)return false;var t=u.attendTotal||0;if(t>0||(u.attendedWeeks||[]).length){var lv='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg> 씨앗';for(var i=0;i<ATTEND_LEVELS.length;i++)if(t>=ATTEND_LEVELS[i].n)lv=ATTEND_LEVELS[i].l;u.history=u.history||[];u.history.push({year:u.curTermYr,term:u.curTermYr+'학년도',grade:_gradeLabelForYear(u,u.curTermYr),attendTotal:t,level:lv,weeks:(u.attendedWeeks||[]).slice(),halfWeeks:(u.halfWeeks||[]).slice()});}u.attendedWeeks=[];u.halfWeeks=[];u.qrScanAt={};u.attendTotal=0;u.streak=0;u.earnedLevels=[];u.absentAckWeek=null;u.absentAckBy=null;u.lastContactAt=null;u.lastContactBy=null;u.curTermYr=cur;return true;}
...
function renderYearPlan(){var edit=_canEditPlan();_hydrateYP();
  ...
  if(thead)thead.innerHTML='<div style="display:flex;gap:6px;padding:9px 8px 9px 0;font-size:11px;font-weight:800;color:var(--text-sub)"><span style="width:56px;flex-shrink:0;box-sizing:border-box;padding:0 6px 0 10px;align-self:stretch;display:flex;align-items:center;position:sticky;left:0;background:#fff;z-index:2;box-shadow:-40px 0 0 0 #fff,3px 0 6px -2px rgba(0,0,0,0.12)">날짜</span><span style="flex:1.2">전례시기</span><span style="flex:.9">운영구분</span><span style="flex:1.2">교리</span><span style="flex:.9">교안담당</span><span style="flex:1.5">교리내용</span><span style="flex:1.2">전례부</span><span style="flex:1.2">성가대</span><span style="flex:1.2">비고</span><span style="flex:1.8">회의 안건</span></div>';
  ...
  var rows=sats.map(function(ds){var l=_ypView(ds);var d=ds.split('-');
    ...
    var mnCell=mn?'<button onclick="closeModal(\'yearplan-modal\');openMinutesViewer(\''+mn.id+'\')" style="width:30px;background:none;border:none;cursor:pointer;font-size:15px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h6"/><path d="M14 2v6h6"/><path d="M18.4 13.6a2 2 0 0 1 3 3L16 22l-4 1 1-4z"/></svg></button>':(edit?'<button onclick="newMinutesForDate(\''+ds+'\')" style="width:30px;background:none;border:none;cursor:pointer;font-size:16px;color:var(--text-light)">＋</button>':'<span style="width:30px;display:inline-block"></span>');
    return '<div class="yp-row" style="display:flex;gap:6px;align-items:center;padding:7px 8px 7px 0;border-bottom:1px solid var(--border-light)"><span style="width:56px;flex-shrink:0;box-sizing:border-box;padding:0 6px 0 10px;align-self:stretch;display:flex;align-items:center;font-size:12px;font-weight:700;color:var(--text-sub);position:sticky;left:0;background:#fff;z-index:6;box-shadow:-40px 0 0 0 #fff,3px 0 6px -2px rgba(0,0,0,0.12)">'+(+d[1])+'/'+(+d[2])+'</span>'+cell('label',l.label||_dayLabel(ds),'1.2','전례시기')+cell('optype',l.optype,'.9','운영구분')+cell('form',l.form,'1.2','교리')+cell('owner',l.owner,'.9','교안담당')+cell('detail',l.detail,'1.5','교리내용')+cell('liturgyTeam',l.liturgyTeam,'1.2','전례부')+cell('choir',l.choir,'1.2','성가대')+cell('note2',l.note2,'1.2','비고')+cell('agenda',l.agenda,'1.8','회의 안건')+'</div>';}).join('');
...
function renderReminderList(){const el=document.getElementById('reminder-list');if(!el)return;if(!reminderData.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 21s-6-5.7-6-10a6 6 0 0 1 12 0c0 4.3-6 10-6 10z"/><circle cx="12" cy="11" r="2.2"/></svg></div><div class="empty-title">리마인더가 없어요</div></div>';return;}el.innerHTML=reminderData.map(r=>`<div class="card" style="margin-bottom:8px;display:flex;align-items:flex-start;gap:10px${r.done?';opacity:.5':''}"><input type="checkbox" ${r.done?'checked':''} style="width:18px;height:18px;accent-color:var(--primary);flex-shrink:0;margin-top:2px" onchange="toggleReminder('${r.id}',this)"><div style="flex:1"><div style="font-size:13px;font-weight:600${r.done?';text-decoration:line-through':''}">${r.content}</div><div style="font-size:10px;color:var(--text-light);margin-top:3px">${r.shared?'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> 교사 공유':'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> 나만 보기'}${r.date?' · '+r.date:''}${r.time?' <svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> '+r.time:''}</div></div><button class="post-more-btn" onclick="openReminderActions('${r.id}')">⋯</button></div>`).join('');}
...
function renderTeacherWeek(){
  ...
  if(!html)html='<div style="padding:22px;text-align:center"><div style="font-size:28px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div style="font-size:13px;color:var(--text-light);margin-top:6px">이번 주는 일정도 할 일도 없어요</div></div>';
  el.innerHTML=html;
...
function openDiaryWriteModal(){editingDiaryId=null;const tt=document.getElementById('diary-modal-title');if(tt)tt.textContent='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> 다이어리 작성';const sel=document.getElementById('diary-teacher-select');if(sel){const teachers=pendingList.filter(u=>u.approved&&u.role==='teacher');sel.innerHTML='<option value="">교사를 선택하세요</option>'+teachers.map(t=>{const POS={m1:'중1',m2:'중2',m3:'중3',h:'고등',principal:'교감',admin:'교무',etc:'기타'};return `<option value="${t.id}">${t.name} ${t.baptism} (${POS[t.teacherType]||t.teacherType})</option>`;}).join('');}document.getElementById('diary-title').value='';document.getElementById('diary-content').value='';const priv=document.querySelector('input[name="diary-scope"][value="private"]');if(priv)priv.checked=true;onScopeChange();openModal('diary-write-modal');}
...
function renderAdminDiaryList(){const el=document.getElementById('admin-diary-list');if(!el)return;const list=diaryData.filter(d=>{if(d.scope==='private')return false;const stu=pendingList.find(u=>u.id===d.studentId);return stu&&canViewDiary(d,stu);});if(!list.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div><div class="empty-title">공유된 다이어리가 없어요</div><div class="empty-desc">학생이 선생님께 공유하면 여기에 표시돼요</div></div>';return;}const SCOPE={teacher:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>‍<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="9" y1="7" x2="9" y2="7"/><line x1="15" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="9" y2="11"/><line x1="15" y1="11" x2="15" y2="11"/><path d="M9 22v-4h6v4"/></svg> 학년교사',specific:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg> 특정교사'};el.innerHTML=list.map(d=>{const stu=pendingList.find(u=>u.id===d.studentId);const nm=stu?stu.name+' '+stu.baptism:'(알 수 없음)';const gl=stu?stu.gradeLabel||'':'';const n=(d.comments||[]).length;const prev=(d.content||'').slice(0,60);return `<div class="diary-card" style="margin-bottom:10px;cursor:pointer" onclick="openDiaryDetail('${d.id}')"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><span style="font-size:12px;font-weight:800"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg> ${nm} <span style="font-size:10px;color:var(--text-light);font-weight:500">(${gl})</span></span><span style="font-size:10px;color:var(--text-light)">${d.date||''}</span></div><div style="font-size:12px;font-weight:700;margin-bottom:3px">${d.title||'무제'}</div><div style="font-size:12px;color:var(--text-sub);line-height:1.6;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical">${prev}${(d.content||'').length>60?'...':''}</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><span style="font-size:10px;color:var(--lavender);font-weight:600">${SCOPE[d.scope]||''}${d.scope==='specific'&&d.teacherName?' ('+d.teacherName+')':''}</span><span style="font-size:11px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.4 8.4 0 0 1-3.8-.9L3 21l2-5.2A8.4 8.4 0 0 1 12 3h.5a8.4 8.4 0 0 1 8 8z"/></svg> ${n} · 자세히 보기 ›</span></div></div>`;}).join('');}
...
function renderDiaryList(){const el=document.getElementById('diary-list');if(!el)return;const mine=diaryData.filter(d=>d.studentId===G.id);if(!mine.length){el.innerHTML='<div class="empty"><div class="empty-emoji"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div><div class="empty-title">아직 기록이 없어요</div><div class="empty-desc">첫 번째 신앙 일기를 작성해보세요!</div></div>';return;}const SCOPE={private:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> 나만',teacher:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>‍<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="9" y1="7" x2="9" y2="7"/><line x1="15" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="9" y2="11"/><line x1="15" y1="11" x2="15" y2="11"/><path d="M9 22v-4h6v4"/></svg> 학년교사',specific:'<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M10 2h4v6h6v4h-6v10h-4V12H4V8h6z"/></svg> 특정교사'};el.innerHTML=mine.map(d=>{const n=(d.comments||[]).length;const prev=(d.content||'').slice(0,60);return `<div class="diary-card" onclick="openDiaryDetail('${d.id}')" style="cursor:pointer"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="flex:1;min-width:0;font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${d.title||'무제'}</span><span style="font-size:10px;color:var(--text-light);white-space:nowrap;flex-shrink:0">${d.date}</span><button onclick="event.stopPropagation();editDiary('${d.id}')" title="수정" style="flex-shrink:0;background:none;border:none;cursor:pointer;font-size:13px;line-height:1;padding:4px;border-radius:6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg></button><button onclick="event.stopPropagation();deleteDiary('${d.id}')" title="삭제" style="flex-shrink:0;background:none;border:none;cursor:pointer;font-size:13px;line-height:1;padding:4px;border-radius:6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg></button></div><p style="font-size:12px;color:var(--text-sub);line-height:1.6;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical">${prev}${(d.content||'').length>60?'...':''}</p><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><span style="font-size:10px;color:var(--lavender);font-weight:600">${SCOPE[d.scope]||''}${d.scope==='specific'&&d.teacherName?' ('+d.teacherName+')':''}</span><span style="font-size:11px;color:var(--text-light)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.4 8.4 0 0 1-3.8-.9L3 21l2-5.2A8.4 8.4 0 0 1 12 3h.5a8.4 8.4 0 0 1 8 8z"/></svg> ${n} · 자세히 보기 ›</span></div></div>`;}).join('');}
...
function openMyCard(){const GRADE={m1:'중1',m2:'중2',m3:'중3',h:'고등부'};const GCHIP={m1:'<span class="grade-m1">중1</span>',m2:'<span class="grade-m2">중2</span>',m3:'<span class="grade-m3">중3</span>',h:'<span class="grade-h">고등</span>'};let lv='<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/></svg> 씨앗';const t=G.attendTotal||0;for(const L of ATTEND_LEVELS)if(t>=L.n)lv=L.l;const el=document.getElementById('mycard-content');if(!el)return;el.innerHTML=`<div style="background:linear-gradient(135deg,var(--mint-light),var(--primary-light));border-radius:var(--radius-lg);padding:18px;margin-bottom:14px"><div style="display:flex;align-items:center;gap:12px;margin-bottom:14px"><div style="width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--lavender));display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800;color:white">${G.name.charAt(0)}</div><div><div style="font-size:16px;font-weight:800">${G.displayName}${G.graduated?' <span class="chip chip-gray" style="font-size:9px;vertical-align:2px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg> 졸업생</span>':''}</div><div style="margin-top:4px">${GCHIP[G.gradeKey]||''}</div></div></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px"><div style="background:white;border-radius:10px;padding:10px;text-align:center"><div style="font-size:20px;font-weight:800;color:var(--primary)">${t}</div><div style="font-size:9px;color:var(--text-light)">누적 출석</div></div><div style="background:white;border-radius:10px;padding:10px;text-align:center"><div style="font-size:20px;font-weight:800;color:var(--mint)">${G.streak||0}</div><div style="font-size:9px;color:var(--text-light)">연속 출석</div></div><div style="background:white;border-radius:10px;padding:10px;text-align:center"><div>${_treeSVG(t<1?-1:ATTEND_LEVELS.reduce((s,L,j)=>t>=L.n?j:s,0),30)}</div><div style="font-size:9px;color:var(--text-light)">현재 등급</div></div></div></div><div class="card" style="margin-bottom:14px"><div style="display:flex;padding:9px 0;border-bottom:1px solid var(--border-light)"><div style="font-size:12px;color:var(--text-sub);width:76px">세례명</div><div style="font-size:13px;font-weight:600">${G.baptism}</div></div><div style="display:flex;padding:9px 0;border-bottom:1px solid var(--border-light)"><div style="font-size:12px;color:var(--text-sub);width:76px">학년</div><div style="font-size:13px;font-weight:600">${G.grade||GRADE[G.gradeKey]||'-'}</div></div><div style="display:flex;padding:9px 0"><div style="font-size:12px;color:var(--text-sub);width:76px">등급</div><div style="font-size:13px;font-weight:600">${lv}</div></div><div style="display:flex;padding:9px 0;border-top:1px solid var(--border-light)"><div style="font-size:12px;color:var(--text-sub);width:76px">담당교사</div><div style="font-size:13px;font-weight:600">${myTeacherNames(G.gradeKey)}</div></div></div><div id="diary-history"></div>`;renderDiaryHistory();openModal('mycard-modal');}
function openBadgeInfo(){const el=document.getElementById('badge-detail');if(!el)return;const t=G.attendTotal||0;let cur=null,next=null;for(const L of ATTEND_LEVELS){if(t>=L.n)cur=L;else if(!next)next=L;}el.innerHTML=`<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:8px 0">${ATTEND_LEVELS.map((L,i)=>`<div style="text-align:center;${t>=L.n?'':'opacity:.45'}"><div style="${t>=L.n?'':'filter:grayscale(.85)'}">${_treeSVG(i,52)}</div><div style="font-size:10px;font-weight:700;margin-top:2px">${L.l.split(' ')[1]}</div><div style="font-size:9px;color:${t>=L.n?'var(--mint)':'var(--text-light)'}">${t>=L.n?'달성!':L.n+'회'}</div></div>`).join('')}</div><div class="card" style="margin:8px 0;background:var(--primary-light)"><div style="font-size:13px;font-weight:800">${cur?cur.l:'등급 없음'} <span style="font-size:11px;font-weight:500;color:var(--text-sub)">· 누적 출석 ${t}회</span></div><div style="font-size:11px;color:var(--text-sub);margin-top:6px;line-height:1.6">토요일 주일학교에 출석하면 누적 1회(반일출석은 0.5회)가 쌓여요. 누적 횟수가 기준에 도달하면 등급이 올라가고 보상 쿠폰이 발급됩니다. 등급은 내려가지 않아요!</div>${next?`<div style="font-size:12px;font-weight:700;margin-top:8px;color:var(--primary-dark)">다음 등급 ${next.l}까지 <b>${next.n-t}회</b> 남았어요 (${next.n}회 달성 시)</div>`:'<div style="font-size:12px;font-weight:700;margin-top:8px;color:var(--primary-dark)"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><circle cx="12" cy="8" r="6"/><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5"/></svg> 최고 등급(열매) 달성! 꾸준한 발걸음이 최고예요</div>'}</div><div style="font-size:12px;font-weight:800;margin:10px 0 6px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></svg> 등급별 보상</div>${ATTEND_LEVELS.map((L,i)=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 4px;border-bottom:1px solid var(--border-light);font-size:12px"><span style="display:inline-flex;align-items:center;gap:6px;font-weight:700;${t>=L.n?'':'opacity:.5'}">${_treeSVG(i,26)}${L.l.split(' ')[1]} <span style="font-weight:500;color:var(--text-light)">(${L.n}회)</span></span><span style="${t>=L.n?'color:var(--mint);font-weight:700':'color:var(--text-sub)'}">${L.r||'-'}</span></div>`).join('')}`;openModal('badge-modal');}
function toggleHideMember(uid){const u=pendingList.find(x=>x.id===uid);if(!u)return;if(u.isAdmin){showToast('관리자 계정은 숨길 수 없어요. 먼저 이임하세요');return;}u.hidden=!u.hidden;showToast(u.hidden?'숨김 처리되었어요 (목록·알림·통계 제외)':'숨김이 해제되었어요');renderMembersList();}
function removeMember(uid){const u=pendingList.find(x=>x.id===uid);if(!u)return;if(u.isAdmin){showToast('관리자 계정은 삭제할 수 없어요. 먼저 이임하세요');return;}if(!confirm(u.name+''+u.baptism+'회원을 삭제할까요?\n계정과 모든 기록이 삭제되며 되돌릴 수 없습니다.\n\n※ 일시 중지는 삭제 대신 숨김을 사용하세요.'))return;if(!confirm('정말 삭제합니다. 마지막 확인입니다.'))return;purgeMemberData(uid);showToast('회원과 관련 기록이 모두 삭제되었습니다');renderMembersList();try{renderAdminCouponList();}catch(e){}}
/* 회원 삭제 시 그 사람의 흔적을 한 번에 정리 (쿠폰이 남아 처리 불가해지는 문제 방지) */
function purgeMemberData(uid){
  try{if(window.FB&&FB.enabled()&&FB.remove){
    try{FB.remove('members',uid);}catch(e){}
    (coupons||[]).forEach(function(c){if(c&&c.studentId===uid&&c.id){try{FB.remove('coupons',c.id);}catch(e){}}});
    (diaryData||[]).forEach(function(d){if(d&&(d.studentId===uid||d.authorId===uid)&&d.id){try{FB.remove('diaries',d.id);}catch(e){}}});
  }}catch(e){}
...
function renderMyPosts(filter){const el=document.getElementById('my-posts-list');if(!el)return;let mine=posts.filter(p=>p.authorId===G.id);var myRes=[];try{myRes=(resources||[]).filter(function(r){return r.authorId===G.id;}).map(function(r){return {_res:true,resId:r.id,title:r.title,date:r.updatedAt||r.date||'',cat:r.cat,image:(r.images&&r.images[0]&&(r.images[0].src||r.images[0]))||''};});}catch(e){}if(filter==='board')mine=mine.filter(p=>!p.isActivity&&!p.isResource);else if(filter==='activity')mine=mine.filter(p=>p.isActivity);else if(filter==='resource')mine=myRes;else if(filter==='jabumo')mine=mine.filter(p=>p.cat==='jabumo');else mine=mine.concat(myRes);if(!mine.length){el.innerHTML='<div class="empty" style="padding:32px"><div class="empty-emoji" style="font-size:32px"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.14em;flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg></div><div class="empty-title" style="font-size:13px">작성한 게시물이 없어요</div></div>';return;}el.innerHTML=mine.map(p=>{var click=p._res?`openMyResourceDetail('${p.resId}')`:`openMyPostDetail('${p.id}')`;var catLbl=p._res?(RESOURCE_CAT_LABEL&&RESOURCE_CAT_LABEL[_catNorm?_catNorm(p.cat):p.cat]||'자료실'):(CAT_LABEL[p.cat]||'-');return `<div class="post-card" style="cursor:pointer" onclick="${click}">${p.image?`<img src="${p.image}" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);margin-bottom:8px;background:var(--bg)">`:''}<div class="post-title">${p.title}</div><div class="post-meta"><span class="post-meta-chip">${catLbl}</span><span>${p.date}</span><span style="margin-left:auto;color:var(--primary);font-weight:700">보기 ›</span></div></div>`;}).join('');}
...
function _deptFit(dept){var w=document.getElementById('dept-stage-wrap');var p=document.getElementById(dept==='choir'?'dept-choir':'dept-liturgy');if(w&&p)w.style.height=p.offsetHeight+'px';}
function selDept(dept,btn){var tc=document.getElementById('dt-choir'),tl=document.getElementById('dt-liturgy');if(tc)tc.classList.toggle('active',dept==='choir');if(tl)tl.classList.toggle('active',dept==='liturgy');var ind=document.getElementById('dept-ind');if(ind){ind.style.transform='translateX('+(dept==='liturgy'?'100%':'0')+')';ind.style.background=dept==='liturgy'?'#DAD1F3':'#F2C5AF';ind.style.borderColor=dept==='liturgy'?'#B4A6E0':'#E3A488';}var st=document.getElementById('dept-stage');if(st)st.style.transform='translateX('+(dept==='liturgy'?'-50%':'0')+')';renderDeptPosts(dept);_deptFit(dept);setTimeout(function(){_deptFit(dept);},80);}
renderCalendar();
