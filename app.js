...
function submitWeeklyNotice(){
  ...
  var _wnPopupDays=parseInt((document.getElementById('wn-popup-days')||{}).value||'7')||7;
  ...
  if(_wsdt){
    ...
    var _wp={id:id,title:title,content:body,cat:'notice',target:'all',grade:'all',isImportant:true,popupDays:_wnPopupDays,authorId:G.id,authorName:G.displayName,comments:[],edited:false};
  ...
  if(ex){ ex.title=title; ex.content=body; ex.edited=true; ex.ts=Date.now(); ex.isImportant=true; ex.popupDays=_wnPopupDays; if(_wimgs.length){ex.images=_wimgs.map(function(s){return {src:s};});ex.image=_wimgs[0];}else{delete ex.images;ex.image='';} }
  else{
    var _np={id:id,ts:Date.now(),pushed:false,title:title,content:body,cat:'notice',target:'all',grade:'all',isImportant:true,popupDays:_wnPopupDays,
...
function _resetSchedUI(prefix){try{var on=document.getElementById(prefix+'-sched-on');if(on)on.checked=false;var f=document.getElementById(prefix+'-sched-fields');if(f)f.style.display='none';var d=document.getElementById(prefix+'-sched-date');if(d)d.value='';var t=document.getElementById(prefix+'-sched-time');if(t)t.value='';}catch(e){}}
