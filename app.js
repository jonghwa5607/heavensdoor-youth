...
function openNotifModal(){
  /* 창을 여는 시점에 '이미 읽은 알림'만 기억해둔다 — 새 알림이 함께 지워지는 문제 방지 */
  _notifReadSnap=new Set(notifications.filter(n=>notifMatch(n)&&nRead(n)).map(n=>n.id));
  renderNotifList();markNotifsRead();openModal('notif-modal');
}
