...
function selParentCat(cat,btn){selCatTab(btn);currentBoardCat=cat;show('fab-board',cat==='jabumo'&&!!G.isJabumoPresident);if(cat==='jabumo')renderBoardList(cat,posts.filter(p=>p.cat==='jabumo'));else if(cat==='event')renderBoardList(cat,posts.filter(p=>p.cat==='event'));else renderBoardList(cat,posts.filter(p=>p.cat===cat&&(p.target==='all'||p.target==='parent')));}
