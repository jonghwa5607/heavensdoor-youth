...
function renderMinutesHub(){
  ...
  html+=list.length?_minTable(list):_resEmpty('📝','회의록이 없어요',live?'＋ 새 회의록을 눌러 바로 시작해요':'이 해에는 작성된 회의록이 없어요');
...
function openMinutesViewer(id){const r=resources.find(r=>r.id===id);if(!r)return;currentMinutesId=id;_minEditing=false;_minSlashClose();document.getElementById('minutes-viewer-title').value=r.title;var _md=document.getElementById('min-mdate');if(_md)_md.value=r.mdate||'';var _mf=document.getElementById('min-form');if(_mf)_mf.value=r.form||'';var _mp=document.getElementById('min-progress');if(_mp)_mp.value=r.progress||'';var _mph=document.getElementById('min-phase');if(_mph)_mph.textContent=(r.mdate&&_dayLabel(r.mdate))||'—';_minRender(_minParse(r.content),false);_renderMinutesMeta(r);setMinutesEditing(false);_updateMinutesLockUI();renderMinutesAck();clearInterval(_minViewTimer);_minViewTimer=setInterval(_minViewSync,2500);openModal('minutes-viewer-modal');}
function saveMinutesMeta(){var r=resources.find(function(x){return x.id===currentMinutesId;});if(!r)return;r.mdate=(document.getElementById('min-mdate').value||'');r.form=(document.getElementById('min-form').value||'').trim();r.progress=(document.getElementById('min-progress').value||'').trim();r.updatedAt=_minDateStr();r.updatedBy=G.displayName;var ph=document.getElementById('min-phase');if(ph)ph.textContent=(r.mdate&&_dayLabel(r.mdate))||'—';try{if(typeof flushSync==='function')flushSync();}catch(e){}try{renderMinutesHub();}catch(e){}}
function _minTable(list){var head='<div style="display:flex;gap:6px;padding:7px 8px;font-size:10px;font-weight:700;color:var(--text-light);border-bottom:1px solid var(--border-light)"><span style="flex:2">📝 회의록</span><span style="flex:1">날짜</span><span style="flex:2">전례시기</span><span style="flex:2">형태</span><span style="flex:1">진행</span></div>';var rows=list.map(function(r){var d=r.mdate?(function(p){return (+p[1])+'/'+(+p[2]);})(r.mdate.split('-')):'—';var ph=(r.mdate&&_dayLabel(r.mdate))||'—';var st=(r.published&&G.role==='teacher'&&!_isAcked(r,G.id))?'🔴 ':(r.published?'✅ ':'');return '<div onclick="openMinutesViewer(\''+r.id+'\')" style="display:flex;gap:6px;align-items:center;padding:11px 8px;border-bottom:1px solid var(--border-light);cursor:pointer;font-size:12px"><span style="flex:2;font-weight:700;color:var(--primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+st+_esc(r.title)+'</span><span style="flex:1;color:var(--text-sub)">'+d+'</span><span style="flex:2;color:var(--text-sub);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(ph)+'</span><span style="flex:2;color:var(--text-sub);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(r.form||'')+'</span><span style="flex:1;color:var(--text-sub);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(r.progress||'')+'</span></div>';}).join('');return '<div style="background:var(--card);border:1px solid var(--border-light);border-radius:var(--radius-sm);overflow:hidden">'+head+rows+'</div>';}
