...
function _canWriteWeekly(){return G.role==='teacher';}
function _isWeeklyWatcher(){return G.role==='teacher';}
function openWeeklyNotice(){
  if(!_isWeeklyWatcher()){showToast('교사만 작성할 수 있어요');return;}
...
function submitWeeklyNotice(){
  if(!_canWriteWeekly()){showToast('교사만 올릴 수 있어요');return;}
...
el.innerHTML='<div style="font-size:13px;font-weight:800;margin-bottom:10px">📜 나의 신앙 발자취</div>'+summary+'<div style="position:relative;padding-left:28px"><div style="position:absolute;left:10px;top:12px;bottom:12px;width:2px;background:linear-gradient(var(--mint),var(--lavender))"></div>'+entries.map(x=>{const at=x.attendTotal||0;const pct=Math.min(100,Math.round(at/45*100));const emoji=(x.level||'🌱 씨앗').split(' ')[0];return `<div style="position:relative;margin-bottom:12px"><div style="position:absolute;left:-28px;top:12px;width:22px;height:22px;border-radius:50%;background:white;border:2px solid ${x.cur?'var(--mint)':'var(--lavender)'};display:flex;align-items:center;justify-content:center;font-size:11px;box-shadow:0 1px 4px rgba(0,0,0,.08)">${emoji}</div><div class="card" style="cursor:pointer${x.cur?';border:1.5px solid var(--mint)':''}" onclick="showHistoryWeeks(${x.idx})"><div style="display:flex;justify-content:space-between;align-items:center;gap:6px"><div style="font-size:13px;font-weight:800">${x.year}년${x.grade?' · '+x.grade:''} ${x.cur?'<span class="chip chip-mint" style="font-size:9px">진행 중</span>':''}</div><span class="chip chip-blue" style="flex-shrink:0">${x.level}</span></div><div style="margin-top:9px;height:6px;background:var(--bg);border-radius:4px;overflow:hidden"><div style="width:${pct}%;height:100%;background:linear-gradient(90deg,var(--mint),var(--primary));border-radius:4px"></div></div><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-light);margin-top:5px"><span>출석 ${at}회</span><span>${at>=45?'🍎 열매 달성!':'🍎까지 '+(45-at)+'회'}</span></div></div></div>`;}).join('')+'</div><div style="font-size:10px;color:var(--text-light);text-align:center;margin:2px 0 8px">카드를 누르면 그해의 출석 발자국을 볼 수 있어요 👣</div>';}
...
function streakBonusOf(streak){return {amount:_streakWeekly(streak),milestone:false};}
/* 누적 출석 횟수(지각 포함, 각 주 1회) */
function _cumAttendCount(u){return ((u&&u.attendedWeeks)||[]).length;}
/* 다음 누적출석 보너스 목표 */
function _nextCumMilestone(count){var m=ptCfg().milestones;var ks=Object.keys(m).map(Number).sort(function(a,b){return a-b;});for(var i=0;i<ks.length;i++){if(ks[i]>(count||0))return {count:ks[i],pt:m[ks[i]]};}return null;}
/* 누적출석 보너스 재조정 — 누적 횟수 기준으로 임계값 도달분을 지급/회수 (성장점수 반영) */
function _reconcileMilestones(u,by){
  var ms=ptCfg().milestones,count=_cumAttendCount(u);u.milestonesAwarded=u.milestonesAwarded||{};
  var granted=0;
  Object.keys(ms).forEach(function(k){
    var T=+k,bonus=ms[k],should=count>=T,has=!!u.milestonesAwarded[k];
    if(should&&!has){u.currentPoints=(u.currentPoints||0)+bonus;u.yearTotalPoints=(u.yearTotalPoints||0)+bonus;u.milestonesAwarded[k]=true;_phPush(u,'earn',bonus,'누적 '+T+'회 출석 보너스',by||'시스템','mile-'+T);granted+=bonus;}
...
function awardAttendance(u,sat,status,by){
  ...
  /* 1) 이번 주 기본+연속 보너스 재계산 (상태 변경 시 이전 지급분 되돌림) */
  var prev=u.weekAward[sat]||{pts:0};
  u.currentPoints=(u.currentPoints||0)-prev.pts;
  u.yearTotalPoints=(u.yearTotalPoints||0)-prev.pts;
  var cfg=ptCfg(),addPts=0,reason='',streak=0,wk=0,base=0;
  if(status!=='absent'){
    streak=computeStreak(u);u.maxStreak=Math.max(u.maxStreak||0,streak);if(u.id===G.id)G.maxStreak=u.maxStreak;
    if(status==='half'){base=cfg.half;wk=0;reason='지각 출석';}          /* 지각: 연속 보너스 없음, 연속기록은 유지 */
    else{base=cfg.full;wk=_streakWeekly(streak);reason='주일 출석'+(wk?(' · 연속 '+streak+'주 보너스'):'');}
    addPts=base+wk;
  }
  u.currentPoints=(u.currentPoints||0)+addPts;
  u.yearTotalPoints=(u.yearTotalPoints||0)+addPts;
  u.weekAward[sat]={pts:addPts};
...
/* 초기 예시 상품 없음 — 상점은 비어서 시작하고, 관리자가 추가한 상품만 표시·동기화됨 (자동 생성/초기화 안 함) */
const DEFAULT_SHOP_ITEMS=[];
var _shopCat='all';
function _shopInited(){return true;}
function _shopMarkInited(){}
function _shopList(){return shopItems||[];}
...
function _shopSeed(){return false;}   /* 예시 상품 자동 생성 제거 — 아무 것도 만들지 않음 */
...
/* 교환 취소 — 포인트 환불 + 재고 복구 (교사/본인) */
function cancelShopOrder(id){
  var idx=(shopOrders||[]).findIndex(function(x){return x.id===id;});if(idx<0)return;var o=shopOrders[idx];
  var self=(o.uid===G.id);
  if(!self&&G.role!=='teacher'){showToast('본인 또는 교사만 취소할 수 있어요');return;}
  appConfirm({icon:'warn',title:'교환을 취소할까요?',desc:o.itemName+'\n'+(o.price||0).toLocaleString()+'P 환불 · 재고 복구',okText:'취소하기'}).then(function(ok){
    if(!ok)return;
    var buyer=(pendingList||[]).find(function(x){return x.id===o.uid;});
    if(buyer){buyer.currentPoints=(buyer.currentPoints||0)+(o.price||0);_phPush(buyer,'earn',(o.price||0),'교환 취소 환불 - '+o.itemName,G.displayName,'cancel-'+o.id);_ptSyncG(buyer);try{saveMemberNow(buyer);}catch(e){}}
    var real=(shopItems||[]).find(function(x){return x.id===o.itemId;});if(real)real.stock=(real.stock||0)+1;
    shopOrders.splice(idx,1);
    try{notifications.unshift({pushed:false,id:'nt'+Date.now()+'cxl',text:'↩️ <b>'+_esc(o.itemName)+'</b> 교환이 취소되어 '+(o.price||0).toLocaleString()+'P가 환불되었어요',time:'방금',ts:Date.now(),readBy:[],forStudentId:o.uid});updateNotifDot();}catch(e){}
    try{if(typeof flushSync==='function')flushSync();}catch(e){}
    try{renderShopAdmin();}catch(e){}try{renderShopOrders();}catch(e){}try{renderShop();}catch(e){}try{renderHomePoints();}catch(e){}
    showToast('교환을 취소하고 환불했어요');
  });
}
/* 재고 빠른 추가 (교사) */
function restockItem(id){
  if(G.role!=='teacher'){showToast('교사만 재고를 관리할 수 있어요');return;}
  var it=(shopItems||[]).find(function(x){return x.id===id;});if(!it){if(!(shopItems&&shopItems.length)){_shopSeed();it=(shopItems||[]).find(function(x){return x.id===id;});}if(!it)return;}
  var v=prompt(it.name+' 재고를 몇 개 추가할까요?','10');if(v===null)return;var n=parseInt(v,10);if(!n||n<1){showToast('1 이상의 숫자를 입력해주세요');return;}
  it.stock=(it.stock||0)+n;try{if(window.FB&&FB.enabled()&&FB.save)FB.save('shopItems',it.id,it);}catch(e){}try{if(typeof flushSync==='function')flushSync();}catch(e){}try{renderShopAdmin();}catch(e){}try{renderShop();}catch(e){}showToast('재고 '+n+'개 추가 · 현재 '+it.stock+'개');
}
/* ── 교사 상점 관리 ── */
...
function renderShopAdmin(){
  ...
  oh+= pend.length? pend.map(function(o){return '<div style="display:flex;align-items:center;gap:10px;padding:9px 2px;border-bottom:1px solid var(--border-light)"><div style="flex:1;min-width:0"><div style="font-size:12.5px;font-weight:700">'+_esc(o.name||'')+' · '+_esc(o.itemName)+'</div><div style="font-size:10px;color:var(--text-light)">'+_esc(o.dateStr||'')+' · '+(o.price||0).toLocaleString()+'P</div></div><div style="display:flex;gap:5px;flex-shrink:0"><button class="btn btn-sm" style="width:auto;padding:6px 10px;background:var(--coral-light);color:#B0463A;font-weight:800" onclick="cancelShopOrder(\''+o.id+'\')">취소</button><button class="btn btn-sm" style="width:auto;padding:6px 12px;background:var(--mint);color:#fff;font-weight:800" onclick="markOrderDone(\''+o.id+'\')">수령완료</button></div></div>';}).join('') : '<div style="font-size:12px;color:var(--text-light);padding:8px 2px 14px">대기중인 교환이 없어요</div>';
...
function saveShopItem(){
  if(!_isShopAdmin()){showToast('교사만 상품을 관리할 수 있어요');return;}
  if(!(shopItems&&shopItems.length))_shopSeed();
  var id=document.getElementById('sim-id').value;
  var name=(document.getElementById('sim-name').value||'').trim();
  ...
  var _saved;
  if(id){var it=shopItems.find(function(x){return x.id===id;});if(it){it.name=name;it.price=price;it.stock=stock;it.cat=cat;it.sec=sec;it.active=active;it.img=img;_saved=it;}}
  else{_saved={id:'si'+Date.now().toString(36),name:name,price:price,stock:stock,cat:cat,sec:sec,active:active,img:img};shopItems.push(_saved);}
  try{if(_saved&&window.FB&&FB.enabled()&&FB.save)FB.save('shopItems',_saved.id,_saved);}catch(e){}
  try{if(typeof flushSync==='function')flushSync();}catch(e){}
  closeModal('shop-item-modal');renderShopAdmin();showToast('저장했어요 · 모두에게 반영돼요');
}
...
/* ── 축일 포인트 (성장점수 미반영, 보유 포인트만) ── */
function checkFeastPoints(){
  try{
...
function renderHomePoints(){
  try{
    var yt=G.yearTotalPoints||0;var li=levelInfo(yt);
    var s=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};
    s('home-cur-points',(G.currentPoints||0).toLocaleString());
    s('home-year-pts',yt.toLocaleString());s('home-year-pts2',yt.toLocaleString());
    ...
    var st=G.streak||0,cumCnt=(G.attendedWeeks||[]).length,nm=_nextCumMilestone(cumCnt);
    ...
    if(mel){
      ...
      else{var nx=st+1,wk=_streakWeekly(nx),nextCnt=cumCnt+1,cumBonus=(cfg2.milestones[nextCnt]||0),tot=cfg2.full+wk+cumBonus;
        parts.push('이번 주 정상 출석 시 <b>+'+tot+'P</b>'+(cumBonus>0?(' · 누적 '+nextCnt+'회 달성 +'+cumBonus+'P!'):''));}
      if(nm){parts.push('🔥 누적 '+nm.count+'회 출석까지 '+(nm.count-cumCnt)+'회! 달성 시 <b>+'+nm.pt+'P</b> 보너스');}
...
function showAttendDone(aw,streak){
  ...
  try{
    ...
    var s=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};
    s('attdone-streak',streak||0);s('attdone-maxstreak',Math.max(streak||0,G.maxStreak||0));
    var sub=document.getElementById('attdone-sub');
    if(sub)sub.innerHTML=isHalf?'지각도 연속출석 기록과 누적 횟수는 인정돼요. (이번 주 연속 보너스만 미지급)':'';
...
var _CI={
  home:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',
  star:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15 9 22 9.3 16.5 14 18.5 21 12 17 5.5 21 7.5 14 2 9.3 9 9"/></svg>',
  check:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>',
  book:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>',
  chat:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',
  user:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
  admin:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="21" x2="4" y2="14"/><line x1="4" y1="10" x2="4" y2="3"/><line x1="12" y1="21" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="20" y1="21" x2="20" y2="16"/><line x1="20" y1="12" x2="20" y2="3"/><line x1="1" y1="14" x2="7" y2="14"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="17" y1="16" x2="23" y2="16"/></svg>',
  music:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>',
  folder:'<svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>'
};
const COACH={student:{
home:[
  {e:_CI.home,t:'하늘의문에 오신 걸 환영해요!',d:'아래 탭으로 홈·커뮤니티·출석·다이어리·부서활동·MY를 이용해요. 공지·오늘의 말씀·생일 축하는 홈에서 확인할 수 있어요.'},
  {e:_CI.star,t:'출석하면 포인트가 쌓여요',d:'토요일 QR 출석으로 포인트를 모아 레벨을 올려요. 정상 +200P·지각 +100P에 연속·누적 출석 보너스가 더해지고, 모은 포인트는 포인트 상점에서 쓸 수 있어요.'}
],
attend:[{e:_CI.check,t:'출석 체크 & 포인트',d:'선생님이 보여주는 QR을 찍으면 출석 완료! 포인트가 자동 적립되고 연속·누적 출석 보너스가 더해져요. 이 화면에서 출석 내역과 월별 포인트 순위도 볼 수 있어요.'}],
diary:[{e:_CI.book,t:'신앙 다이어리',d:'기도·묵상·오늘의 마음을 기록해요. 나만 보기 / 학년 교사 / 특정 교사로 공유 범위를 정할 수 있고, 선생님께 공유하면 답장을 받을 수 있어요.'}],
board:[{e:_CI.chat,t:'커뮤니티',d:'친구들과 소식을 나누는 공간이에요. 글쓰기 버튼으로 글을 남길 수 있어요. 서로 존중하는 말로 함께해요!'}],
my:[{e:_CI.user,t:'MY',d:'내 정보와 활동, 보유 포인트·레벨을 한눈에 볼 수 있어요. 프로필 사진과 상태 메시지도 설정할 수 있어요.'}]
},
parent:{
home:[{e:_CI.home,t:'환영합니다!',d:'자녀의 신앙생활을 함께 응원하는 공간이에요. 홈에서 공지·행사·오늘의 말씀과 자녀 출석 현황을 볼 수 있어요. (자녀 세례명이 등록돼 있어야 정확히 연동돼요)'}],
board:[{e:_CI.chat,t:'커뮤니티',d:'부서 소식과 학부모 소통 글을 볼 수 있어요. 위쪽 카테고리로 나눠서 확인할 수 있어요.'}],
my:[{e:_CI.user,t:'MY',d:'내 정보와 자녀 정보를 확인할 수 있어요.'}]
},
teacher:{
home:[{e:_CI.home,t:'환영합니다, 선생님!',d:'홈에서 공지·일정·오늘의 말씀을 확인하고, 관리 탭에서 학생·부서를 관리해요. 오른쪽 위 알림으로 가입 승인·연속 결석 등을 확인할 수 있어요.'}],
board:[{e:_CI.chat,t:'커뮤니티 & 글쓰기',d:'학생·학부모와 소통하는 게시판이에요. 글쓰기 버튼으로 공지·소식을 올리고 대상(학년·전체)도 정할 수 있어요.'}],
activity:[{e:_CI.music,t:'부서활동',d:'성가대·전례 등 부서 활동 자료와 일정을 관리하는 공간이에요.'}],
teacher:[{e:_CI.folder,t:'자료실',d:'교리·부서활동 자료와 회의록을 올리고 함께 열람할 수 있어요.'}],
admin:[{e:_CI.admin,t:'관리',d:'관리 홈이 메인이에요. 회원 승인·출석 체크·상점/특별포인트·출석 통계·앱 설정을 여기서 관리해요. 빨간 배지는 처리할 일이 있다는 표시고, 출석 QR은 출석관리에서 만들 수 있어요(교감·교무·관리자).'}],
my:[{e:_CI.user,t:'MY',d:'내 정보와 설정을 확인할 수 있어요.'}]
}
};
...
function renderCoachCard(st){document.getElementById('coach-ring').style.display='none';document.getElementById('coach-tip').style.display='none';document.getElementById('coach-card-wrap').style.display='flex';document.getElementById('coach-emoji').innerHTML=st.e||'';document.getElementById('coach-title').textContent=st.t||'';document.getElementById('coach-desc').textContent=st.d||'';document.getElementById('coach-next').textContent=coachIdx>=coachSteps.length-1?'시작하기':'다음';_coachDots('coach-dots',false);_coachAnim(document.getElementById('coach-card-inner'),'coach-cardpop');}
function renderCoachSpot(st){const el=document.getElementById(st.sel);if(!_coachVisible(el)){coachIdx++;return renderCoach();}document.getElementById('coach-card-wrap').style.display='none';el.scrollIntoView({block:'center'});setTimeout(function(){if(!_coachVisible(el)){coachIdx++;return renderCoach();}const r=el.getBoundingClientRect(),p=8,vw=innerWidth,vh=innerHeight;const ring=document.getElementById('coach-ring');ring.style.display='block';ring.style.top=(r.top-p)+'px';ring.style.left=(r.left-p)+'px';ring.style.width=(r.width+2*p)+'px';ring.style.height=(r.height+2*p)+'px';const tip=document.getElementById('coach-tip');document.getElementById('coach-tip-emoji').innerHTML=st.e||'';document.getElementById('coach-tip-title').textContent=st.t||'';document.getElementById('coach-tip-desc').textContent=st.d||'';document.getElementById('coach-tip-next').textContent=coachIdx>=coachSteps.length-1?'시작하기':'다음';_coachDots('coach-tip-dots',true);tip.style.display='block';tip.style.visibility='hidden';const tw=tip.offsetWidth,th=tip.offsetHeight;let left=Math.min(Math.max(r.left+r.width/2-tw/2,10),vw-tw-10);const below=(vh-r.bottom)>(th+28);let top=below?(r.bottom+p+12):(r.top-p-12-th);top=Math.min(Math.max(top,10),vh-th-10);tip.style.left=left+'px';tip.style.top=top+'px';const arw=document.getElementById('coach-arrow');const ax=Math.min(Math.max(r.left+r.width/2-left-7,14),tw-28);arw.style.left=ax+'px';arw.style.top=below?'-7px':(th-8)+'px';tip.style.visibility='visible';_coachAnim(tip,'coach-pop');},240);}
...
function _monthEarned(u){var ym=currentYM();var s=0;((u&&u.pointHistory)||[]).forEach(function(x){if(x&&x.type==='earn'&&(x.amount>0)&&_ymOf(x.ts)===ym&&String(x.ref||'').indexOf('cancel-')!==0)s+=x.amount;});return s;}
function _monthSats(){var now=new Date();var y=now.getFullYear(),mo=now.getMonth();var d=new Date(y,mo,1);while(d.getDay()!==6)d.setDate(d.getDate()+1);var out=[];while(d.getMonth()===mo){var s=toDateStr(d);if(!(typeof isVacationDate==='function'&&isVacationDate(s)))out.push(s);d.setDate(d.getDate()+7);}return out;}
...
function renderAttendHistory(){
  ...
  // 다음 연속출석 목표
  var _cc2=(G.attendedWeeks||[]).length,nm=_nextCumMilestone(_cc2);
  if(nm){html+='<div class="card" style="margin-top:13px;display:flex;align-items:center;gap:13px"><div style="width:40px;height:40px;border-radius:12px;background:var(--mint-light);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">🎯</div><div><div style="font-size:14px;font-weight:800">누적 '+nm.count+'회 출석까지 '+(nm.count-_cc2)+'회</div><div style="font-size:12px;color:var(--text-sub);margin-top:2px">누적 '+nm.count+'회 출석 시 +'+nm.pt+'P 지급</div></div></div>';}
  // 명언 카드
...
async function doLogin(){
  ...
  if(id===ADMIN.id){
    ...
    /* ⚠️ 서버에서 회원 데이터가 아직 안 왔으면(캐시 삭제 직후 등) 잠깐 기다린다.
       빈 임시 문서로 로그인하면 클라우드의 진짜 프로필(사진)을 덮어써 사라지기 때문. */
    if(window.FB&&FB.enabled()&&!window._membersServerLoaded){showToast('서버에 연결 중이에요. 잠시 후 다시 시도해주세요');return;}
...
function _setStoryAuthor(p){
  ...
  var authorId=(post&&post.authorId)||'';
  ...
  _setAvatarEl(av,avatar);
  if(av){if(authorId){av.style.cursor='pointer';av.onclick=function(e){if(e)e.stopPropagation();openStoryProfile(authorId);};}else{av.style.cursor='';av.onclick=null;}}
  var de=document.getElementById('story-viewer-date');
  if(de){var rt=_relTime(ts);de.textContent=(name?name+' · ':'')+(rt||p.date||'');
    if(authorId){de.style.cursor='pointer';de.onclick=function(e){if(e)e.stopPropagation();openStoryProfile(authorId);};}else{de.style.cursor='';de.onclick=null;}}
}
...
var _PERSON_SVG='<svg viewBox="0 0 24 24" width="58%" height="58%" fill="none" stroke="#fff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:block"><circle cx="12" cy="8.5" r="3.6"/><path d="M5 20c0-3.6 3.1-6.2 7-6.2s7 2.6 7 6.2"/></svg>';
function _setAvatarEl(el,src){if(!el)return;if(src){el.style.backgroundImage="url('"+src+"')";el.style.backgroundSize='cover';el.style.backgroundPosition='center';el.innerHTML='';}else{el.style.backgroundImage='';el.innerHTML=_PERSON_SVG;}}
function _cmtAvatarHTML(c,sz){sz=sz||30;var u=(pendingList||[]).find(function(x){return x.id===c.authorId;});var av=u&&u.avatar;var base='width:'+sz+'px;height:'+sz+'px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;overflow:hidden;cursor:pointer;';if(av)return '<div style="'+base+'background:#4BAF9E url(\''+_esc(av)+'\') center/cover" onclick="openStoryProfile(\''+(c.authorId||'')+'\')"></div>';return '<div style="'+base+'background:linear-gradient(135deg,#2FA595,#9B8FD4)" onclick="openStoryProfile(\''+(c.authorId||'')+'\')">'+_PERSON_SVG+'</div>';}
...
var _storyProfilePaused=false;
function _initStoryProfileObserver(){
  var pv=document.getElementById('profile-view-modal');if(!pv||pv._spObs)return;pv._spObs=true;
  try{new MutationObserver(function(){
    if(_storyProfilePaused&&!pv.classList.contains('open')){_storyProfilePaused=false;
      var sm=document.getElementById('story-viewer-modal');if(sm&&sm.classList.contains('open')){try{resumeStory();}catch(e){}}}
  }).observe(pv,{attributes:true,attributeFilter:['class']});}catch(e){}
}
function openStoryProfile(uid){var u=_fillProfileView(uid);if(!u)return;var av=document.getElementById('pv-avatar');if(av){av.style.cursor='zoom-in';av.onclick=function(e){if(e)e.stopPropagation();openStoryAvatar(u.avatar||'');};}var cb=document.getElementById('pv-close-btn');if(cb)cb.onclick=function(){closeModal('profile-view-modal');};
  var sm=document.getElementById('story-viewer-modal');if(sm&&sm.classList.contains('open')){_initStoryProfileObserver();try{pauseStory();}catch(e){}_storyProfilePaused=true;}
...
/* 키보드가 떠도 사진이 가려지지 않도록 VisualViewport 높이에 뷰어를 맞춤 */
var _storyVVh=null;
function _storyKbBind(){var vv=window.visualViewport;if(!vv)return;if(!_storyVVh){_storyVVh=function(){var stage=document.getElementById('story-stage');var m=document.getElementById('story-viewer-modal');if(!stage||!m||!m.classList.contains('open')){return;}stage.style.height=Math.round(vv.height)+'px';};}vv.addEventListener('resize',_storyVVh);vv.addEventListener('scroll',_storyVVh);_storyVVh();}
function _storyKbUnbind(){var vv=window.visualViewport;var stage=document.getElementById('story-stage');if(stage)stage.style.height='';if(vv&&_storyVVh){vv.removeEventListener('resize',_storyVVh);vv.removeEventListener('scroll',_storyVVh);}}
function _stDoCloseStoryFull(){clearStoryTimer();try{_storyKbUnbind();}catch(e){}try{collapseStoryContent();}catch(e){}var sheet=document.getElementById('story-comment-sheet');if(sheet)sheet.style.transform='translateY(100%)';var stage=document.getElementById('story-stage');if(stage)stage.classList.remove('cmt-open');_sheetOpen=false;_storyReplyTo=null;var rb=document.getElementById('story-reply-bar');if(rb)rb.style.display='none';closeModal('story-viewer-modal');try{renderStoryRow();}catch(e){}}
/* 댓글 시트 드래그(아래로 밀어 닫기) */
...
function onWriteTopChange(){const v=document.getElementById('write-top').value;show('write-mid-wrap',v==='student'||v==='parent');const mid=document.getElementById('write-mid');if(v==='student')mid.innerHTML='<option value="">선택</option><option value="all-s">전체</option><option value="m1">중1</option><option value="m2">중2</option><option value="m3">중3</option><option value="h">고등</option>';else if(v==='parent')mid.innerHTML='<option value="">선택</option><option value="all-p">전체</option>';}
...
function renderDeptPosts(dept,sub){if(sub!==undefined)deptSubFilter[dept]=sub;const f=deptSubFilter[dept];_deptTabSync(dept);try{requestAnimationFrame(function(){_deptTabSync(dept);});}catch(e){}const el=document.getElementById(dept+'-posts');if(!el)return;const arr=posts.filter(p=>p.cat==='activity'&&p.dept===dept&&(f==='all'||p.subcat===f));var _ico=dept==='choir'?'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="18" r="2.6"/><circle cx="17" cy="16" r="2.6"/><path d="M8.6 18V5.2l11-2v12.6"/><path d="M8.6 8.6l11-2"/></svg>':'<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="var(--text-light)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="M7 8h10"/></svg>';if(!arr.length){el.innerHTML='<div class="empty"><div class="empty-emoji">'+_ico+'</div><div class="empty-title">아직 게시물이 없어요</div></div>';try{_deptFit(dept);}catch(e){}return;}const canAdmin=G.role==='teacher'&&(G.type==='principal'||G.type==='admin'||G.isAdmin);el.innerHTML=arr.map(p=>{const isOwner=p.authorId===G.id;const moreBtn=(isOwner||canAdmin)?`<button class="post-more-btn" onclick="event.stopPropagation();openPostActions('${p.id}',${isOwner},true)">⋯</button>`:'';const subChip=`<span class="chip chip-blue">${p.subcat==='notice'?'공지':dept==='choir'?'악보/자료':'전례자료'}</span>`;return `<div class="post-card" data-pid="${p.id}" style="cursor:pointer"><div class="post-head"><div class="post-tags">${subChip}</div>${moreBtn}</div><div class="post-title">${p.title}</div><div class="post-meta"><span class="post-meta-chip">${p.authorName||'익명'}</span><span>${p.date}</span>${p.edited?'<span class="post-edited">(수정됨)</span>':''}</div>${p.image?`<img src="${p.image}" style="width:100%;height:auto;max-height:420px;object-fit:contain;border-radius:var(--radius-sm);margin:8px 0;background:var(--bg)">`:''}${p.content?`<div class="post-preview">${p.content.slice(0,60)}${p.content.length>60?'...':''}</div>`:''}<div class="post-footer"><span class="post-stat">💬 ${(p.comments||[]).length}</span>${p.docs&&p.docs.length?`<span class="post-stat">📎 ${p.docs.length}</span>`:''}</div></div>`;}).join('');try{_deptFit(dept);}catch(e){}}
...
/* 번호 목록 매기기: 같은 섹션 안에서는 사이에 글머리·문단이 끼어도 번호가 이어짐.
   제목(h1~h3)·구분선(div)에서만 초기화. 들여쓰기 단계별로 따로 셈. */
function _minNumberOf(blocks){
  var counters={},out=[];
  for(var i=0;i<blocks.length;i++){
    var b=blocks[i],t=b.t,ind=parseInt(b.ind,10)||0;
    if(t==='h1'||t==='h2'||t==='h3'||t==='div'){counters={};out.push(0);continue;}
    if(t==='ol'){Object.keys(counters).forEach(function(k){if(+k>ind)delete counters[k];});counters[ind]=(counters[ind]||0)+1;out.push(counters[ind]);}
    else out.push(0);
  }
  return out;
}
function _minRender(blocks,editable){
  var ed=document.getElementById('minutes-viewer-content');if(!ed)return;
  var nums=_minNumberOf(blocks);
  ed.innerHTML=blocks.map(function(b,i){ return _minBlockHtml(b,editable,nums[i]); }).join('');
...
function _minRenumber(){
  var ed=document.getElementById('minutes-viewer-content');if(!ed)return;var counters={};
  Array.prototype.forEach.call(ed.querySelectorAll('.mb'),function(b){
    var t=b.dataset.t,ind=parseInt(b.dataset.ind,10)||0;
    if(t==='h1'||t==='h2'||t==='h3'||t==='div'){counters={};return;}
    if(t==='ol'){Object.keys(counters).forEach(function(k){if(+k>ind)delete counters[k];});counters[ind]=(counters[ind]||0)+1;var m=b.querySelector('.mb-mk');if(m)m.textContent=counters[ind]+'.';}
  });
}
...
function _resizeImg(file,cb){var r=new FileReader();r.onload=function(){var img=new Image();img.onload=function(){var mx=640,w=img.width,h=img.height;if(w>h){if(w>mx){h=Math.round(h*mx/w);w=mx;}}else{if(h>mx){w=Math.round(w*mx/h);h=mx;}}var c=document.createElement('canvas');c.width=w;c.height=h;var cx=c.getContext('2d');try{cx.imageSmoothingEnabled=true;cx.imageSmoothingQuality='high';}catch(e){}cx.drawImage(img,0,0,w,h);cb(c.toDataURL('image/jpeg',0.82));};img.src=r.result;};r.readAsDataURL(file);}
...
function ensureAdminMember(){
  ...
  try{
    ...
    /* ⚠️ 서버에서 회원 데이터가 실제로 도착하기 전에는 (아바타·프로필이 빠진) 임시 문서를 만들지 않는다.
       그렇지 않으면 캐시 삭제 후 로그인 시 빈 문서가 클라우드의 진짜 프로필(사진 등)을 덮어써 사라짐. */
    if(window.FB&&FB.enabled()&&!window._membersServerLoaded&&!_loggedInAsAdmin)return;
    ...
    pendingList.push({
      ...
      avatar:(typeof G!=='undefined'&&G.id===ADMIN.id&&G.avatar)||ADMIN.avatar||'',
      statusMsg:(typeof G!=='undefined'&&G.id===ADMIN.id&&G.statusMsg)||ADMIN.statusMsg||'',
...
function renderAdminGrid(isFull){const el=document.getElementById('admin-grid');if(!el)return;isFull=(G.type==='principal'||G.type==='admin'||G.isAdmin);const isAdminUser=G.type==='principal'||G.isAdmin;const canApprove=G.type==='principal'||G.type==='admin'||G.isAdmin;show('admin-settings-tabbtn',isAdminUser);const students=pendingList.filter(u=>u.approved&&u.role==='student'&&!u.hidden&&!u.graduated&&(isFull||u.gradeKey===G.type));const absentCount=students.filter(u=>computeAbsentStreak(u)>=3).length;const items=[
    ...
    {icon:'<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>',title:'상점 관리',sub:'상품·교환 관리',badge:(shopOrders||[]).filter(o=>o&&o.status!=='done').length,action:"showAdminTab('shop')"},
...
function showAdminTab(tab){try{if(tab==='main')renderAdminGrid(G.type==='principal'||G.type==='admin'||G.isAdmin);}catch(e){}['main','attend','members','students','diary','shop','stats','settings'].forEach(t=>{const el=document.getElementById('admin-'+t+'-tab');if(el)el.style.display=t===tab?'':'none';});const bb=document.getElementById('admin-back-btn');if(bb)bb.style.display=tab==='main'?'none':'flex';const bar=document.querySelector('#screen-admin .tab-bar');if(bar)bar.querySelectorAll('.tab-btn').forEach(b=>b.classList.toggle('active',b.getAttribute('onclick').includes(`showAdminTab('${tab}')`)));if(tab==='pending')updatePendingUI();if(tab==='attend')renderAttendList();if(tab==='members')renderMembersList();if(tab==='students')renderStudentCards('all');if(tab==='diary')renderAdminDiaryList();if(tab==='main')renderGovSection();if(tab==='settings')renderSeasonBtns();if(tab==='reminder')renderReminderList();if(tab==='shop')renderShopAdmin();if(tab==='settings')loadAppConfigForm();if(tab==='stats'){renderAttendStats();renderAbsentAlerts();}window.scrollTo(0,0);}
function renderAdminCouponList(){dedupeBdayCoupons();const el=document.getElementById('admin-coupon-list');if(!el)return;if(!coupons.length){el.innerHTML='<div class="empty" style="padding:28px"><div class="empty-emoji" style="font-size:28px">🎟️</div><div class="empty-title" style="font-size:13px">발급된 쿠폰이 없어요</div><div class="empty-desc">학생이 출석 등급을 달성하면 쿠폰이 생겨요</div></div>';return;}el.innerHTML=coupons.slice().reverse().map(c=>{const card=`<div class="card" style="margin-bottom:0"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px">${_treeByLabel(c.badgeLabel,24)}${c.studentName} <span style="font-size:11px;color:var(--text-light);font-weight:500">· ${(_treeByLabel(c.badgeLabel)?_labelNoEmoji(c.badgeLabel):c.badgeLabel)} · 🎁 ${c.reward||'보상'}</span></div><span class="chip ${c.used?'chip-gray':'chip-coral'}">${c.used?'사용완료':'미사용'}</span></div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><div style="font-size:11px;color:var(--text-light)">발급 ${c.createdAt}${c.manual&&c.issuedBy?' · '+c.issuedBy:''}${c.used&&c.usedAt?' · 사용 '+c.usedAt:''}</div><div style="font-size:15px;font-weight:800;letter-spacing:2px;color:var(--primary-dark)">${c.used?'· · · · · ·':c.code}</div></div>${c.used?'<div style="font-size:10px;color:var(--text-light);margin-top:6px">← 밀어서 삭제</div>':''}</div>`;
...
function _imgBtn(fn){return '<button onclick="'+fn+'" style="background:var(--bg);border:1px solid var(--border-light);border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;color:var(--text-sub);margin-right:6px">🖼 이미지</button>';}
...
renderCalendar();
/* ══ 쿠폰 UI 폐지 — 포인트제 전환 오버라이드 ══ */
try{checkNewCoupons=function(){};}catch(e){}
try{renderCouponList=function(){};}catch(e){}
try{renderAdminCouponList=function(){};}catch(e){}
try{renderDetailCoupons=function(u){try{renderDetailPoints(u);}catch(e){}};}catch(e){}
try{checkBirthdayCoupons=function(){try{checkBirthdayPoints();}catch(e){}};}catch(e){}
