...
function doRegister(){
  ...
  if(G.regRole==='student'){const s=document.getElementById('reg-school').value;if(s==='prep'){user.cohort=_curSchoolYr()+1;}else{const g=(document.getElementById('reg-grade-num').value||'').replace('학년','');if(!s||!g){showToast('학교급과 학년을 선택해주세요');return;}user.cohort=_curSchoolYr()-(s==='middle'?(+g-1):(+g-1+3));}user.gradeOffset=0;try{applyStudentGrade(user);}catch(e){}user.attendTotal=0;user.streak=0;}
...
function _gradeSync(){try{var admin=_canEditPlan();if(admin)_migrateCohorts();var ch=false;(pendingList||[]).forEach(function(u){if(u.role!=='student')return;if(admin&&_sealIfRolled(u))ch=true;if(applyStudentGrade(u))ch=true;});if(ch&&admin){try{if(typeof flushSync==='function')flushSync();}catch(e){}}}catch(e){}}
