...
window.FB=(function(){
  ...
  return {
    ...
    watch:function(col,cb){ if(db) db.collection(col).onSnapshot(function(s){cb(s.docs.map(function(d){return d.data();}));},function(err){ try{console.error('[FB] 읽기 실패(규칙 확인 필요):',col,err&&err.code);}catch(e){} try{if(err&&err.code==='permission-denied'&&window._fbWarnDenied)window._fbWarnDenied(col);}catch(e){} }); }
  };
})();
...
function fbRefreshBoard(){ try{ if(typeof currentBoardCat==='undefined'||!currentBoardCat||!(G&&G.id))return; if(currentBoardCat==='activity'){ if(typeof renderDeptPosts==='function'){renderDeptPosts('choir');renderDeptPosts('liturgy');} return; } if(G.role==='teacher'){ if(typeof applyTeacherFilter==='function')applyTeacherFilter(); } else if(G.role==='student'){ renderBoardList(currentBoardCat,posts.filter(function(x){return x.cat===currentBoardCat&&(x.target==='all'||(x.target==='student'&&(x.grade==='all-s'||x.grade===G.gradeKey)));})); } else { renderBoardList(currentBoardCat,posts.filter(function(x){return x.cat===currentBoardCat&&(currentBoardCat==='jabumo'||x.target==='all'||x.target==='parent');})); } }catch(e){} }
var _fbDeniedSeen={};
...
(function(){
  ...
  specs.forEach(function(sp){ FB.watch(sp.c,function(cloud){ try{
    ...
    cloud.forEach(function(cd){ var id=jid(cd); if(!id)return; if(sp.dec){try{cd=sp.dec(cd);}catch(e){}} cids[id]=1; var lc=by[id];
      if(!lc && (id in pp)){ /* 이전에 동기화됐던 문서가 로컬에서 사라짐 = 이 기기에서 삭제함 → 클라우드에서도 삭제 (부활 방지) */ try{FB.remove(sp.c,id);}catch(e){} delete pp[id]; return; }
      if(lc&&!lc.demo&&JSON.stringify(lc)!==pp[id]){ out.push(lc); } else { out.push(cd); pp[id]=JSON.stringify(cd); } });
    local.forEach(function(x){ if(!x||x.demo)return; var id=jid(x); if(!id||cids[id])return; if(id in pp){ if(JSON.stringify(x)!==pp[id])out.push(x); } else { out.push(x); } });
