...
(function(){
  ...
  var specs=[
    ...
    {c:'shopItems',g:function(){return shopItems;},s:function(v){shopItems=v;}},
    {c:'shopOrders',g:function(){return shopOrders;},s:function(v){shopOrders=v;}},
  ...
  function rerender(){ clearTimeout(_rt); _rt=setTimeout(function(){ if(!(G&&G.id))return; ['renderMembersList','renderAttendList','renderAdminGrid','renderAbsentAlerts','renderAttendStats','renderCouponList','renderCalendar','renderHomeNotices','renderStoryRow','renderReminderList','renderHomeReminders','renderEventBanner','renderHomeSchedule','initStamps','renderAttendHistory','renderResourceList','renderPushNudgeBar','renderShopOrders','renderShopAdmin','renderHomePoints','renderBdayComments','renderBdayLike','renderBdayBannerAuto','renderDiaryList','updateNotifDot','renderNotifList','renderTeacherWeek','renderHomeMinutes','renderMinutesHub','renderDeptWeekInfo','renderGradLetterEntry','_syncStudentCards','_syncDetailScreen','updatePendingUI','syncSessionFromRec','setMyProfile','paintTeacherHome','updateWeeklyBtn'].forEach(function(fn){ try{ if(typeof window[fn]==='function')window[fn](); }catch(e){} }); try{fbRefreshBoard();}catch(e){} }, 150); }
  specs.forEach(function(sp){ FB.watch(sp.c,function(cloud,fromCache){ try{
    ...
    if(sp.c==='members'&&G&&G.id){ for(var i=0;i<out.length;i++){ if(out[i].id===G.id){ var _u=out[i]; if(!previewMode)currentLoginUser=_u; G.role=_u.role;G.name=_u.name;G.baptism=_u.baptism;if(_u.id===ADMIN.id&&appConfig&&appConfig.adminPos&&appConfig.adminPos.t&&(_u.teacherType||'')!==appConfig.adminPos.t){_u.teacherType=appConfig.adminPos.t;_u.gradeLabel=appConfig.adminPos.l||appConfig.adminPos.t;try{saveMemberNow(_u);}catch(e){}}G.type=_u.teacherType||'';G.gradeKey=_u.gradeKey||'';G.grade=_u.gradeLabel||'';G.isAdmin=_u.isAdmin||false;G.isJabumo=_u.isJabumo||false;G.isJabumoPresident=_u.isJabumoPresident||false;G.graduated=_u.graduated||false;G.birthMonth=_u.birthMonth||0;G.birthDay=_u.birthDay||0;G.feastMonth=_u.feastMonth||0;G.feastDay=_u.feastDay||0;G.attendTotal=_u.attendTotal||0;G.streak=_u.streak||0;G.history=_u.history||[];G.attendedWeeks=_u.attendedWeeks||[];G.halfWeeks=_u.halfWeeks||[];G.qrScanAt=_u.qrScanAt||{};G.avatar=_u.avatar||'';G.statusMsg=_u.statusMsg||'';G.currentPoints=_u.currentPoints||0;G.yearTotalPoints=_u.yearTotalPoints||0;G.level=_u.level||1;G.pendingStreakRewards=_u.pendingStreakRewards||0; try{setMyProfile();}catch(e){} try{paintTeacherHome();}catch(e){} break; } } }
    if(sp.c==='members'){window._membersLoaded=true;if(!fromCache)window._membersServerLoaded=true;try{ensureAdminMember();}catch(e){} try{if(typeof _tryRestoreSession==='function')_tryRestoreSession();}catch(e){} try{syncSessionFromRec();}catch(e){} try{checkPwVersion();}catch(e){} try{cleanOrphanCoupons();}catch(e){}}
